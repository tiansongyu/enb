...
int trx_usrp_start(openair0_device *device)
{
...
struct timespec time_finish = {0,0};
int uhd_send_count = 0;
int uhd_recv_succ = 0;
int uhd_recv_fail = 0;
...
static int trx_usrp_write(openair0_device *device, openair0_timestamp timestamp, void **buff, int nsamps, int cc, int flags)
{
  ...
  if( write_flag )
  {
      ...
      if( time_finish.tv_nsec > time_start.tv_nsec)
      {
          duration = (time_finish.tv_nsec - time_start.tv_nsec);
      }
      else
      {
          duration = 1000000000 - ( time_start.tv_nsec - time_finish.tv_nsec );
      }
      duration /= 100;
      if( duration < 100 )
      {
          printf("%2d.%1d", duration/10, duration%10);
      }
      else
      {
          printf("%3d.%1d", duration/10, duration%10);
      }

      //printf("send:%d recv:%d fail:%d\n", uhd_send_count, uhd_recv_succ, uhd_recv_fail);
      write_flag = 0;
      uhd_recv_succ = 0;
      uhd_recv_fail = 0;
      uhd_send_count = 0;
...
static int trx_usrp_read(openair0_device *device, openair0_timestamp *ptimestamp, void **buff, int nsamps, int cc)
{
  ...
  if (cc>1) {
    ...
    if( s == 0 )
    {
        printf("trx_usrp_read: s is null\n");
        return 0;
    }
    if( s->rx_stream == 0 )
    {
        printf("trx_usrp_read: s->rx_stream is null\n");
        return 0;
    }
    /*if( s->rx_stream->recv == 0 )
    {
        printf("trx_usrp_read: s->rx_stream->recv is null\n");
        return 0;
    }*/
    ...
    {
        uhd_recv_fail++;
        printf( "@@@@@trx_usrp_read:samples_received=%d nsamps=%d!!!!\n",samples_received, nsamps);
        samples_received = nsamps;
        //samples_received += s->rx_stream->recv(&buff_tmp[0][samples_received], nsamps-samples_received, s->rx_md);
    }
    else
    {
		uhd_recv_succ ++;
  ...
  *ptimestamp = s->rx_timestamp;
  ...
  if( (++count ) % 75000 == 0 )
  {
      ...
      if( count2 % 60 == 0 )
      {
          //printf("\n#%6d#% 08llx--",count2, s->rx_timestamp);
          printf("\n#%03d# ",count2/60);
      }
      else if( count2 % 60 == 30 )
      {
          //printf("\n#%6d#% 08llx--",count2, s->rx_timestamp);
          printf("\n      ");
      }
...
int trx_usrp_set_gains(openair0_device* device, 
  ...
  printf("trx_usrp_set_gains:Setting USRP TX gain to %f\n", 
    openair0_cfg[0].tx_gain[0]);
  ...
  printf("trx_usrp_set_gains:Setting USRP RX gain to %f(%f-%f)\n", 
...
int openair0_dev_init_usrp(openair0_device* device, openair0_config_t *openair0_cfg, char *pUsrpName)
{
  ...
  std::string args = boost::str(boost::format("name=%s") % pUsrpName);


  uhd::device_addrs_t device_adds = uhd::device::find(args);
  size_t i;

  printf("openair0_dev_init_usrp:Checking for USRP device of name %s\n", pUsrpName);

  if(device_adds.size() == 0)
  {
    printf("openair0_dev_init_usrp:Checking for USRP device of type b200\n");
  ...
  if(device_adds.size() == 0)
  {
    printf("openair0_dev_init_usrp:Line=%d\n", __LINE__);
...
//    s->usrp->set_clock_source("internal");
  ...
  for(i=0;i<s->usrp->get_rx_num_channels();i++) 
  {
    if (i<openair0_cfg[0].rx_num_channels) {
      ...
      printf("openair0_dev_init_usrp:Setting rx freq/gain on channel %lu/%lu rx freq=%f rx gain=%f(%f-%f)\n",
        i,
        s->usrp->get_rx_num_channels(),
        openair0_cfg[0].rx_freq[i],
  ...
  for(i=0;i<s->usrp->get_tx_num_channels();i++) {
    if (i<openair0_cfg[0].tx_num_channels) {
      ...
      printf("openair0_dev_init_usrp:Setting tx freq/gain on channel %lu/%lu tx freq=%f tx gain=%f\n",
        i,
        s->usrp->get_tx_num_channels(),
        openair0_cfg[0].tx_freq[i],
  ...
  if(is_equal(s->sample_rate, (double)15.36e6))
    s->tx_forward_nsamps = 90;//115?
  if(is_equal(s->sample_rate, (double)7.68e6))
    s->tx_forward_nsamps = 50;

  printf("openair0_dev_init_usrp:END\n");
