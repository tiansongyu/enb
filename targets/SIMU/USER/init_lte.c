                           ...
                           uint8_t abstraction_flag)
{
  ...
  PHY_vars_eNB->lte_frame_parms.Nid_cell = frame_parms->Nid_cell;//((Nid_cell/3)*3)+((eNB_id+Nid_cell)%3);
  ...
  LOG_I(PHY,"init eNB: Nid_cell %d/%d/%d eNB_id=%d\n", 
    frame_parms->Nid_cell, PHY_vars_eNB->lte_frame_parms.Nid_cell, Nid_cell, eNB_id);
                         ...
                         uint8_t transmission_mode)
{
  ...
  for (i=0; i<NUMBER_OF_CONNECTED_eNB_MAX; i++) {
    ...
    printf("init_lte_UE PHY_vars_UE->ulsch_ue[%d]=%llx PHY_vars_UE=%llx\n",i,PHY_vars_UE->ulsch_ue[i],PHY_vars_UE);
