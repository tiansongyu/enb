...
static char                    *conf_config_file_name2 = NULL;
#if defined(ENABLE_ITTI)
...
void reset_stats(FL_OBJECT *button, long arg)
{
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
    ...
    //xhd_oai_enb
    phy_vars_eNB->eNB_UE_stats[i].timing_advance_update=0;
    phy_vars_eNB->eNB_UE_stats[i].timing_advance_count=0;
    phy_vars_eNB->eNB_UE_stats[i].timing_advance_active=0;
...
extern void print_frame25(uint32_t *pData, int len, int start, int frame, int hw_subframe);
extern void print_frame25F(uint32_t *pData, int len, int start, int frame, int hw_subframe);
...
void do_OFDM_mod_rt(int subframe,PHY_VARS_eNB *phy_vars_eNB)
{
  ...
  if ((subframe_select(&phy_vars_eNB->lte_frame_parms,subframe)==SF_DL)||
    ...
    //xhd_oai_ue
    static uint8_t ucLogFlag = 0;
    if( 0 == subframe && ucLogFlag == 0)
    {
        ucLogFlag = 1;
        //print_frame25FT(
        //    (uint32_t *)&phy_vars_eNB->lte_eNB_common_vars.txdataF[0][aa][slot_offset_F], 
        //    (uint32_t *)&dummy_tx_b[0], 
        //    0, 0);
    }
...
int main_cpuid = 255;
int main_enb_cpuid = 255;
int tx_thread_cpuid[10]={255,255,255,255,255,255,255,255,255,255};
int rx_thread_cpuid[10]={255,255,255,255,255,255,255,255,255,255};

void get_thread_cpu(int *cpuid)
{
    if(*cpuid==255)
    {
        *cpuid = sched_getcpu();
...
void set_thread_cpu( char *threadname, int i)
{
    cpu_set_t mask;
    ...
    if (pthread_setaffinity_np(pthread_self(), sizeof(mask), &mask) < 0) {
        printf("set thread %s to cpu %d fail\n", threadname, i);
    }
    else
    {
        //printf("set thread %s to cpu %d success\n", threadname, i);
    }
}
...
{
/*
    printf("Cpuid: main %d enb %d Tx %d %d %d %d %d  %d %d %d %d %d  Rx %d %d %d %d %d  %d %d %d %d %d\n",
        main_cpuid, main_enb_cpuid,
        tx_thread_cpuid[0], tx_thread_cpuid[1], tx_thread_cpuid[2], tx_thread_cpuid[3], tx_thread_cpuid[4],
        tx_thread_cpuid[5], tx_thread_cpuid[6], tx_thread_cpuid[7], tx_thread_cpuid[8], tx_thread_cpuid[9],
        rx_thread_cpuid[0], rx_thread_cpuid[1], rx_thread_cpuid[2], rx_thread_cpuid[3], rx_thread_cpuid[4],
        rx_thread_cpuid[5], rx_thread_cpuid[6], rx_thread_cpuid[7], rx_thread_cpuid[8], rx_thread_cpuid[9]
        );
*/    
    main_cpuid=main_enb_cpuid=255;
    for( int i = 0; i < 10; i++)
    {
        tx_thread_cpuid[i] = 255;
        rx_thread_cpuid[i] = 255;
    }
}
 ...
 */
 int txThreadCpu[10]={0};

unsigned int tx_run_subframe[10]={0};
unsigned int rx_run_subframe[10]={0};
uint8_t g_print_flag = 0;
unsigned int g_tx_pos = 0;
unsigned int g_rx_pos = 0;

uint8_t gucThreadNumTx = 0;
uint8_t gucThreadNumTx1 = 0;
uint8_t gucThreadNumTx2 = 0;
uint8_t gucThreadNumRx = 0;

//#if 0
#define ENB_LOCK_SIZE 5

typedef struct{
pthread_cond_t enb_cond[ENB_LOCK_SIZE];
pthread_mutex_t enb_cond_m[ENB_LOCK_SIZE];
pthread_mutex_t enb_mutex;
int enb_wait_head;
int enb_wait_tail;
}stEnbLock;
stEnbLock gEnbLock[10];
...
void enb_lock_init()
{
    ...
    for( int i = 0; i<2; i++)
    {
        stEnbLock *pLock = &gEnbLock[i];
        pthread_mutex_init( &pLock->enb_mutex, NULL);
        for(int i=0;i<ENB_LOCK_SIZE;i++)
        {
            pthread_cond_init( &pLock->enb_cond[i], NULL);
            pthread_mutex_init( &pLock->enb_cond_m[i], NULL);
        }
        pLock->enb_wait_head = 0;
        pLock->enb_wait_tail = 0;

    }

}
void enb_lock_get(uint8_t txFlag)
{
    return; //xhd_oai_ue
    
    stEnbLock *pLock = &gEnbLock[txFlag];

    if( g_print_flag ) printf("get%d",pLock->enb_wait_tail);

    if (pthread_mutex_lock(&pLock->enb_mutex) != 0) {
      LOG_E( PHY, "[SCHED][eNB] error locking mutex for eNB TX proc\n" );
      exit_fun("nothing to add");
    }
   
    if (pLock->enb_wait_head != pLock->enb_wait_tail) 
    {
        int old_tail = pLock->enb_wait_tail;
        if(++pLock->enb_wait_tail == ENB_LOCK_SIZE)pLock->enb_wait_tail=0;

        if (pthread_mutex_unlock(&pLock->enb_mutex) != 0) {
          LOG_E(PHY,"[SCHED][eNB] error unlocking mutex for eNB tail=%d\n",old_tail);
          exit_fun("nothing to add");
        }
        if( g_print_flag ) printf("wait%d",old_tail);
        pthread_mutex_lock(&pLock->enb_cond_m[old_tail]);
        pthread_cond_wait( &pLock->enb_cond[old_tail], &pLock->enb_cond_m[old_tail] ); // this unlocks mutex_tx while waiting and then locks it again
        pthread_mutex_unlock(&pLock->enb_cond_m[old_tail]);
        if( g_print_flag ) printf("do%d",old_tail);
    }
    else
    {
        if( g_print_flag ) printf("do%d",pLock->enb_wait_head);
        if(++pLock->enb_wait_tail == ENB_LOCK_SIZE)pLock->enb_wait_tail=0;

        if (pthread_mutex_unlock(&pLock->enb_mutex) != 0) {
          LOG_E(PHY,"[SCHED][eNB] error unlocking mutex for eNB\n");
          exit_fun("nothing to add");
        }
    }
...
void enb_lock_release(uint8_t txFlag)
{
    return; //xhd_oai_ue
    
    stEnbLock *pLock = &gEnbLock[txFlag];

    if( g_print_flag ) printf("release%d ",pLock->enb_wait_head);
    ...
    if( g_print_flag ) printf("done%d ",pLock->enb_wait_head);
    
    if(++pLock->enb_wait_head == ENB_LOCK_SIZE)pLock->enb_wait_head=0;
    
    if( pLock->enb_wait_head != pLock->enb_wait_tail)
    {
        if( g_print_flag ) printf("sig%d ",pLock->enb_wait_head);
        if (pthread_cond_signal(&pLock->enb_cond[pLock->enb_wait_head]) != 0) 
        {
          LOG_E( PHY, "[eNB] ERROR pthread_cond_signal for eNB RX thread\n" );
...
#if 0
typedef struct{
pthread_cond_t enb_cond;
pthread_mutex_t enb_cond_m;
pthread_mutex_t enb_mutex;
int enb_wait_count;
}stEnbLock;
stEnbLock gEnbLock[10];
...
void enb_lock_init()
{
    for( int i = 0; i<2; i++)
    {
        stEnbLock *pLock = &gEnbLock[i];
        pthread_mutex_init( &pLock->enb_mutex, NULL);
        pthread_mutex_init( &pLock->enb_cond_m, NULL);
        pthread_cond_init( &pLock->enb_cond, NULL);
        pLock->enb_wait_count = 0;
    }
...
void enb_lock_get(uint8_t txFlag)
{
    ...
    if( g_print_flag ) printf("get%d",txFlag);
    ...
    if (pLock->enb_wait_count<0) 
    {
        pthread_mutex_unlock(&pLock->enb_mutex);
        
        pthread_mutex_lock(&pLock->enb_cond_m);
        pthread_cond_wait( &pLock->enb_cond, &pLock->enb_cond_m ); // this unlocks mutex_tx while waiting and then locks it again
        pthread_mutex_unlock(&pLock->enb_cond_m);
...
void enb_lock_release(uint8_t txFlag)
{
    ...
    if( g_print_flag ) printf("release%d ",txFlag);
    ...
    if(pLock->enb_wait_count<0)
    {
        if( g_print_flag ) printf("sig%d ",txFlag);
        if (pthread_cond_signal(&pLock->enb_cond) != 0) 
        {
          LOG_E( PHY, "[eNB] ERROR pthread_cond_signal for eNB RX thread\n" );
          exit_fun( "ERROR pthread_cond_signal" );
        }
...
//#if 0
  ...
  if (runtime > 1000000 ){
    LOG_W(HW,"eNB_thread_tx:estimated runtime %d is larger than 1ms, adjusting\n",runtime);
    runtime = (0.97 * 100) * 10000; 
  }
  deadline = (0.98 * 100) * 10000; 
  ...
  if (sched_setattr(0, &attr, flags) < 0 ) {
    ...
    printf("Start eNB Tx:%d failed: runtime=%ld deadline=%ld period=%ld cpuid=%d !!!!!\n", 
        proc->subframe, runtime, deadline, period, sched_getcpu());
    return &eNB_thread_tx_status[proc->subframe];
  }
  else
  {
      printf("Start eNB Tx:%d success: runtime=%ld deadline=%ld period=%ld  cpuid=%d \n", 
          proc->subframe, runtime, deadline, period, sched_getcpu());
  }
...
//#endif
  ...
  //xhd_oai_50
  set_thread_cpu("TX",proc->subframe+16);

  while (!oai_exit) 
  {

    //xhd_oai_50
    get_thread_cpu(&tx_thread_cpuid[proc->subframe]);
    ...
    //xhd_oai_60m  tx_handle
    uint8_t ucSubFrameRecv = (proc->subframe_tx + 6)%10;
    enb_lock_get(ucSubFrameRecv);

    uint8_t ucStartPos = proc->subframe_tx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet;
    uint8_t ucDuration = PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet;

    if( ucStartPos < gulRxPos + 96  && ucStartPos > gulRxPos + 64)
    {
        tx_pos_set[5][proc->subframe_tx]++;
        static int test_count = 0;
        if( test_count++ > 100)
        {
            printf(" \n$$$$ subframe_tx=%d g_tx_pos=%d TXPos=%d RXPos=%d $$$$\n",proc->subframe_tx, g_tx_pos, ucStartPos, gulRxPos);
            test_count = 0;
        }
    }
    ...
    if( g_print_flag )
    {
        if( openair0_cfg[0].samples_per_packet == 1920 
        ...
        {
            printf("eNB_thread_tx: tx_sf=%d  pos %d.%d samples_per_tti=%d.%d\n",
                proc->subframe_tx,
                proc->subframe_tx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet,
                proc->subframe_tx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti%openair0_cfg[0].samples_per_packet,
                PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet,
                PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti%openair0_cfg[0].samples_per_packet
                );
        }
    }
    ...
    if (((PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.frame_type == TDD) &&
      ...
      if(gucThreadNumTx>1)
      {
        //printf("####more than 2 tx:%d-%d-%d subframe=%d\n", 
        //    gucThreadNumTx, 
        //    gucThreadNumTx1, 
        //    gucThreadNumTx2, 
        //    proc->subframe_tx);
    ...
    tx_run_subframe[proc->subframe_tx]=1;

    gucThreadNumTx2++;
    do_OFDM_mod_rt( proc->subframe_tx, PHY_vars_eNB_g[0][proc->CC_id] );
    gucThreadNumTx2--;
    
    if(proc->subframe_tx == 0 && ((proc->subframe_rx & 0x7f) == 0))
    {
        print_frame25TSymbol(
           &PHY_vars_eNB_g[0][proc->CC_id]->lte_eNB_common_vars.txdata[0][0][0],
           PHY_vars_eNB_g[0][proc->CC_id]->rx_offset,
           PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti * 10,
           proc->frame_tx,proc->subframe_tx);
    
       print_frame25FSymbol(
            &PHY_vars_eNB_g[0][proc->CC_id]->lte_eNB_common_vars.txdataF[0][0][0],
            proc->frame_tx,proc->subframe_tx);
    }

    
    tx_run_subframe[proc->subframe_tx]=0;
    ...
    int gulTxPos = g_tx_pos;
    int gulRxPosx = gulRxPos;
    if( g_print_flag )
    {
        printf(" TX%d-%d END ",ucStartPos, gulTxPos);
    }
    if( proc->subframe_tx < 10)
    {
        tx_pos_set[0][proc->subframe_tx]++;
        if( gulTxPos >= ucStartPos)
        {
            if( gulTxPos < ucStartPos + 8 )
            {
                tx_pos_set[1][proc->subframe_tx]++;
            }
            else if( gulTxPos < ucStartPos + 16 )
            {
                tx_pos_set[2][proc->subframe_tx]++;
            }
            else if( gulTxPos < ucStartPos + 24 )
            {
                tx_pos_set[3][proc->subframe_tx]++;
            }
            else if( gulTxPos < ucStartPos + 32 )
            {
                tx_pos_set[4][proc->subframe_tx]++;
            }
        //printf(" \n$$$$ subframe_tx=%d g_tx_pos=%d ucStartPos=%d $$$$\n",proc->subframe_tx, g_tx_pos, ucStartPos);
        }
    }
    enb_lock_release(ucSubFrameRecv);
 ...
 */   
  if (runtime > 1000000 ){
    LOG_W(HW,"eNB_thread_rx:estimated runtime %d is larger than 1ms, adjusting\n",runtime);
    runtime = (0.97 * 100) * 10000; 
  }
  deadline = (0.98 * 100) * 10000; 
  ...
  if (sched_setattr(0, &attr, flags) < 0 ) {
    ...
    printf("Start eNB Rx:%d failed: runtime=%ld deadline=%ld period=%ld cpuid=%d  !!!!\n", 
        proc->subframe, runtime, deadline, period, sched_getcpu() );
    return &eNB_thread_rx_status[proc->subframe];
  }
  else
  {
    printf("Start eNB Rx:%d success: runtime=%ld deadline=%ld period=%ld cpuid=%d \n", 
        proc->subframe, runtime, deadline, period, sched_getcpu() );
  }
//#endif
  ...
  if( proc->subframe < 5)
  {
	  set_thread_cpu("RX",proc->subframe+20);
  }
  else
  {
	  set_thread_cpu("RX",proc->subframe+20);
  ...
  while (!oai_exit) 
  {
    ...
    //xhd_oai_50
    get_thread_cpu(&rx_thread_cpuid[proc->subframe]);
    ...
    //uint8_t ucSubFrameSend = (proc->subframe_rx + 4)%10;

    enb_lock_get(proc->subframe_rx);

    //xhd_oai_60m  rx_handle
    int gulRxPosx =(proc->subframe_rx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti
                  - PHY_vars_eNB_g[0][proc->CC_id]->N_TA_offset)/openair0_cfg[0].samples_per_packet; 
    ...
    if( g_print_flag)
    {
            ...
            || openair0_cfg[0].samples_per_packet == 960 )
        {
            printf(" RX%d-%d ",
                gulRxPosx,
                g_rx_pos //PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet
                );
        }
        else
        {
            printf("eNB_thread_rx: tx_sf=%d rx_sf=%d pos %d.%d samples_per_tti=%d.%d\n",
                proc->subframe_tx,proc->subframe_rx,
                (proc->subframe_rx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti
                  - PHY_vars_eNB_g[0][proc->CC_id]->N_TA_offset)/openair0_cfg[0].samples_per_packet,
                (proc->subframe_rx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti
                  - PHY_vars_eNB_g[0][proc->CC_id]->N_TA_offset)%openair0_cfg[0].samples_per_packet,
                PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti/openair0_cfg[0].samples_per_packet,
    ...
    rx_run_subframe[proc->subframe]=1;
    ...
    rx_run_subframe[proc->subframe]=0;
    gulRxPos = gulRxPosx;
    if( g_print_flag)
    {
        printf(" RX%d-%dEND ",
            (proc->subframe_rx*PHY_vars_eNB_g[0][proc->CC_id]->lte_frame_parms.samples_per_tti
              - PHY_vars_eNB_g[0][proc->CC_id]->N_TA_offset)/openair0_cfg[0].samples_per_packet,
            g_rx_pos //PHY_vars_eNB_g[0][proc->CC_id]->N_TA_offset
            );
    }
    if (pthread_mutex_lock(&proc->mutex_rx) != 0) {
      LOG_E( PHY, "[SCHED][eNB] error locking mutex for eNB RX proc %d\n", proc->subframe );
    ...
    enb_lock_release(proc->subframe_rx);

    //xhd_oai_60m
    uint8_t tx_subframe = proc->subframe_rx + 2;
    if( tx_subframe >= 10)
    {
        tx_subframe -= 10;
    }
    //eNB_call_tx(tx_subframe);
...
////#endif
      ...
      PHY_vars_eNB_g[0][CC_id]->proc[i].frame_tx0 = 0;
      PHY_vars_eNB_g[0][CC_id]->proc[i].frame_rx = 0;
      PHY_vars_eNB_g[0][CC_id]->proc[i].frame_rx0 = 0;
#ifdef EXMIMO
    ...
    PHY_vars_eNB_g[0][CC_id]->proc[9].frame_tx = 1;
    PHY_vars_eNB_g[0][CC_id]->proc[8].frame_tx0 = 1;
    PHY_vars_eNB_g[0][CC_id]->proc[9].frame_tx0 = 1;
 ...
 */
int radio_offset=0;
static void* eNB_thread_ver( void* arg )
{
  UNUSED(arg);
...
#endif
  //xhd_oai_50
  set_thread_cpu("main_enb", 4);
  get_thread_cpu(&main_enb_cpuid);
...
//LOG_D(HW,"eNB Frame %d : hw_slot %d, time %llu\n",frame,hw_slot,rt_get_time_ns());
	...
	openair0.trx_write_func(&openair0,
				(timestamp+(openair0_cfg[card].tx_delay*spp)-openair0_cfg[card].tx_forward_nsamps+radio_offset),
				txp,
				spp,
...
void eNB_call_tx(int hw_subframe)
{
       ...
       for (int CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) 
       {
         ...
         else 
         {
           LOG_W( PHY,"[eNB] Frame %d, eNB TX thread %d busy!! (cnt_tx %i)\n", 
               PHY_vars_eNB_g[0][CC_id]->proc[hw_subframe].frame_tx, hw_subframe, cnt_tx );
           //xhd_oai_enb
           //exit_fun( "TX thread busy" );
           break;
         }
...
//#if 0
  ...
  //xhd_oai_60m
  attr.sched_runtime  = (0.9 * 100) * 10000;
  attr.sched_deadline = (0.95 * 100) * 10000;
  attr.sched_period   = 1 * 1000000;
...
//#endif
  ...
  while (!oai_exit) 
  {
    ...
    if(frame%4096 == 0)
    {
        //g_print_flag=1; by LLG, removed for clear test
        //printf("eNB_thread: "); by LLG, removed for clear test
    }
    else
    {
        g_print_flag=0;
    ...
    //eNB_call_tx((hw_subframe==9)?0:(hw_subframe+1));
    eNB_call_tx(hw_subframe);
    ...
    while (rx_pos < ((1+hw_subframe)*PHY_vars_eNB_g[0][0]->lte_frame_parms.samples_per_tti)) 
    {
      ...
      openair0_timestamp timestamp;
      openair0_timestamp timestamp_old;
      int i=0;
      ...
      if(g_print_flag)
      {
        ...
        {
            g_rx_pos = rx_pos/spp;
            //printf("r%d ", rx_pos/spp);
        }
        else
        {
            printf("r%d.%d ", rx_pos/spp, rx_pos%spp);
        }
      }
      ...
      //if( timestamp != timestamp_old + spp )
      {
        //printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@eNB_thread: Fatal error!timestamp=%d.%d timestamp_old=%d.%d@@@@@@@@@@@@@@@@@@@@@@@@\n", 
        //    timestamp/spp, timestamp%spp, timestamp_old/spp, timestamp_old%spp);
      }
      ...
      if(g_print_flag)
      {
        ...
        {
            printf("t%d.%d ", tx_pos/spp, tx_pos%spp);
        }
      }
      ...
      if ((tx_launched == 0) &&
          (rx_pos >= (((32*hw_subframe)+27)*PHY_vars_eNB_g[0][0]->lte_frame_parms.samples_per_tti>>5))) 
      {
        tx_launched = 1;
        //eNB_call_tx((hw_subframe==9)?0:(hw_subframe+1));
      ...
      #if 0
        if ((rx_launched == 0) &&
            (rx_pos >= (hw_subframe*PHY_vars_eNB_g[0][0]->lte_frame_parms.samples_per_tti)) )
        {
          rx_launched = 1;

          eNB_call_rx((hw_subframe==0)?9:(hw_subframe-1));
...
uint16_t gusUsrpSample = 0;
uint16_t gusUsrpDelay = 0;
uint16_t gusUsrpForward= 0;
uint16_t gusUeAmp= 0;
#define ZSET(a,value) ((a==0)?value:a)
...
static void get_options (int argc, char **argv)
{
  ...
  enum long_option_e {
    ...
    LONG_OPTION_USRP_SAMPLE,
    LONG_OPTION_USRP_DELAY,
    LONG_OPTION_USRP_FORWARD,
    LONG_OPTION_UE_AMP
  };
  ...
  static const struct option long_options[] = {
    ...
    {"usrp-sample",   required_argument,  NULL, LONG_OPTION_USRP_SAMPLE},
    {"usrp-delay",   required_argument,  NULL, LONG_OPTION_USRP_DELAY},
    {"ue-amp",   required_argument,  NULL, LONG_OPTION_UE_AMP},
    {NULL, 0, NULL, 0}
  };

  while ((c = getopt_long (argc, argv, "C:dK:g:F:G:hqO:N:m:SUVRM:r:P:Ws:t:x:",long_options,NULL)) != -1) {
    switch (c) {
   ...
   //xhd_oai_60m
   case LONG_OPTION_USRP_SAMPLE:
      gusUsrpSample = atoi(optarg);
      printf("USRP SAMPLE %d\n",gusUsrpSample);
      break;
   case LONG_OPTION_USRP_DELAY:
      gusUsrpDelay = atoi(optarg);
      printf("USRP DELAY %d\n",gusUsrpDelay);
      break;
   case LONG_OPTION_USRP_FORWARD:
      gusUsrpForward = atoi(optarg);
      printf("USRP FORWARD %d\n",gusUsrpForward);
      break;
  case LONG_OPTION_UE_AMP:
     gusUeAmp = atoi(optarg);
     printf("Ue Amp %d\n",gusUeAmp);
     break;
  ...
  printf("conf_config_file_name:%s\n",conf_config_file_name);
  ...
  if ( (conf_config_file_name != NULL)) {
    ...
    enb_properties = enb_config_init1(conf_config_file_name);


    /* xhd_oai */
    if( conf_config_file_name2 != NULL )
    {
        enb_properties = enb_config_init2(conf_config_file_name2);
    }
...
# if defined(ENABLE_USE_MME)
  ...
  else if (UE_flag == 1) 
  {
    if (conf_config_file_name != NULL) 
    {
      printf("UE:strcpy(uecap_xer,conf_config_file_name)\n");
...
extern int trx_usrp_start(openair0_device *device);
...
int main( int argc, char **argv )
{
  ...
  //xhd_oai_50
  set_thread_cpu("main", 3);
  get_thread_cpu(&main_cpuid);
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    downlink_frequency[CC_id][0] = 3580000000; //2680000000; // Use float to avoid issue with frequency over 2^31.
    downlink_frequency[CC_id][1] = downlink_frequency[CC_id][0];
    downlink_frequency[CC_id][2] = downlink_frequency[CC_id][0];
    downlink_frequency[CC_id][3] = downlink_frequency[CC_id][0];
    //printf("Downlink for CC_id %d frequency set to %u\n", CC_id, downlink_frequency[CC_id][0]);
...
//#endif
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    ...
    printf("==================main:AAA RIV_max50=%d====================\n",RIV_max50);
  }
  printf("==================main:BBB RIV_max50=%d====================\n",RIV_max50);
    ...
    for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
      ...
      if (UE[CC_id]->lte_frame_parms.frame_type == TDD) {
        if (UE[CC_id]->lte_frame_parms.N_RB_DL == 100)
          ...
          UE[CC_id]->N_TA_offset = (624+58)/4;
      } else {
        UE[CC_id]->N_TA_offset = 0; //50 xhd_oai_debug
      }
#endif
    }
  ...
  for (card=0; card<MAX_CARDS; card++) {
    ...
    if(frame_parms[0]->N_RB_DL == 100) {
      ...
      openair0_cfg[card].samples_per_packet = ZSET(gusUsrpSample,1920);//1920;//2048;
      openair0_cfg[card].samples_per_frame = 307200; 
      openair0_cfg[card].tx_bw = 10e6;
      openair0_cfg[card].rx_bw = 10e6;
      // from usrp_time_offset
      openair0_cfg[card].tx_forward_nsamps = ZSET(gusUsrpForward,175);//80;//175;  USRP发送到空口的时延
      openair0_cfg[card].tx_delay = ZSET(gusUsrpDelay,8);//8;    基带到USRP 传输的时延
      ...
      //INTEL NUC测试
      //1920 5 10     1M  1次接入，出现U
      //1920 5 60     <1M  1次接入，出现UU
      //1920 5 175    未接入 出现UUU
      //1920 6 10     3M  2次接入
      //1920 6 175    4M  3次接入
      //1920 6 300    未接入
      //1920 7 10     3M
      ...
      openair0_cfg[card].tx_forward_nsamps = ZSET(gusUsrpForward,60);//95;
      openair0_cfg[card].tx_delay = ZSET(gusUsrpDelay,5);//5;
      //radio_offset=50;
      //5, 1920, -50 8M
      //5, 1920,  0  16M，但经常会回落。
      //5, 1920, 10  17M
      //5, 1920, 50  一开始保持在16m，但是后来又回落
      //5, 1920, 90  16M,中间有速率回落现象
      //5, 1920, 95  偶尔能达到16m，但是保持时间很短
      //5, 1920,100  16m, 两分钟之后达到16m
      //5, 1920,105  16m, 不稳定，单个流由于有丢包，达不到满速率，开多个TCP流可达到21M 
      //5, 1920,110  11m, 运行很久之后能在16m稳定一段时间
      //5, 1920,112  16m,接入较慢，低位运行很久之后升到16M
      //5, 1920,115  16m, 接入较慢  
      //5, 1920,118  无法接入
      //5, 1920,120  16m,接入较慢，但是能摸高到19m  
      //5, 1920,125  无法接入  
      //5, 2048, 95  6m 
      //6, 1920,  0  10~15M之间波动
      //6, 1920, 50  5~10M之间波动
      ...
      openair0_cfg[card].samples_per_packet = 960;//1024;//1024;
      //5*1024-70=5050
#ifdef OAI_USRP
    if (UE_flag==1) 
    {
        openair0_cfg[card].tx_forward_nsamps = 60;//70;//70;
    }
    else
    {
        openair0_cfg[card].tx_forward_nsamps = 70;//70;
    }
...
openair0_cfg[card].num_rb_dl=frame_parms[0]->N_RB_DL;
  ...
  if (UE_flag==1)
  {
      sprintf(ausUsrpName, "%s", "wico_zhuzh_3");
  }
  else
  {
      sprintf(ausUsrpName, "%s", "MyB210");
  }
  
  if ((mode!=loop_through_memory) && 
      (openair0_device_init(&openair0, &openair0_cfg[0], ausUsrpName) <0)) {
  ...
  */
  ...
  //pthread_attr_setstacksize(&attr_UE_thread,5*PTHREAD_STACK_MIN);//5*PTHREAD_STACK_MIN);
  ...
  pthread_attr_setschedpolicy (&attr_UE_thread, SCHED_FIFO); //added by xhd_oai_ue
  ...
  if (mode!=loop_through_memory)
  {
    ...
    {
        printf("@@@@@ openair0.trx_start_func is NULL @@@@@\n");
        //if (trx_usrp_start(&openair0) != 0 ) 
        {
          printf("Could not start the device\n");
          //LOG_E(HW,"Could not start the device\n");
        }
