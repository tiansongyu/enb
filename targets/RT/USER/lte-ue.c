...
static const eutra_band_t eutra_bands[] = {
  ...
  {42, 3400    * MHz, 3500    * MHz, 3500    * MHz, 3600    * MHz, FDD},
...
static void *UE_thread_synch(void *arg)
{
  ...
  else if  (UE->UE_scan == 1) 
  {
    ...
    for (card=0; card<MAX_CARDS; card++) {
      for (i=0; i<openair0_cfg[card].rx_num_channels; i++) {
        ...
        printf( "[UE_thread_synch]UE_SCAN: setting RX gain (%d,%d) to %f\n", card, i, openair0_cfg[card].rx_gain[i] );
#endif
      }
    }
  ...
  while (oai_exit==0) {
    ...
    case pss:
      ...
      for (card=0; card<MAX_CARDS; card++) 
      {
        for (i=0; i<openair0_cfg[card].rx_num_channels; i++) 
        {
          ...
          printf("[UE_thread_synch]PSS: setting RX gain (%d,%d) to %f\n",card,i,openair0_cfg[card].rx_gain[i]);
#endif
	...
	if (UE->UE_scan_carrier == 1) {
      ...
      printf("[UE_thread_synch]PBCH: rx_gain[%d]=%f\n",i,openair0_cfg[card].rx_gain[i]);
  ...
  else 
  {
	...
	if (UE->UE_scan_carrier == 1) 
    {
	  ...
	  if (abs(freq_offset) > 7500*8 /*3600*/ /*7500*/) {
	...
	else 
    {
        ...
        LOG_I( PHY, "[initial_sync] trying carrier off %d Hz, rxgain %d (DL %u, UL %u) freq_offset=%d time=%d\n", 
	       freq_offset,freq_offset/7500,
               UE->rx_total_gain_dB,
               downlink_frequency[0][0]+freq_offset%7500,
               downlink_frequency[0][0]+uplink_frequency_offset[0][0]+freq_offset%7500,
               freq_offset);

        for (card=0; card<MAX_CARDS; card++) {
          for (i=0; i<openair0_cfg[card].rx_num_channels; i++) {
            openair0_cfg[card].rx_freq[i] = downlink_frequency[card][i]+freq_offset%7500;
            openair0_cfg[card].tx_freq[i] = downlink_frequency[card][i]+uplink_frequency_offset[card][i]+freq_offset%7500;
#ifdef OAI_USRP
            openair0_cfg[card].rx_gain[i] = UE->rx_total_gain_dB;//-USRP_GAIN_OFFSET;
...
#endif
          printf("[initial_sync]ZZZ: rx_gain[%d]=%f\n",i,openair0_cfg[card].rx_gain[i]);
...
static void *UE_thread_tx(void *arg)
{
  ...
  attr.sched_flags = 0;
  attr.sched_nice = -20;
  attr.sched_priority = sched_get_priority_max(SCHED_DEADLINE);//0;
  ...
  while (!oai_exit) {
    ...
    if (pthread_mutex_lock(&UE->mutex_tx_cnt) != 0) {
    ...
    {
        LOG_TX_LINE();
        if (pthread_mutex_unlock(&UE->mutex_tx_cnt) != 0) {
    ...
    PERF_TIME(PERF_UE_THREAD_TX, PERF_START);

    my_printf("===>UE_thread_tx start...\n");
    if ((subframe_select( &UE->lte_frame_parms, UE->slot_tx>>1 ) == SF_UL) ||
        (UE->lte_frame_parms.frame_type == FDD)) 
    {
        LOG_TX_LINE();
        PERF_TIME(PERF_UE_phy_procedures_UE_TX, PERF_START);
        phy_procedures_UE_TX( UE, 0, 0, UE->mode, no_relay );
        PERF_TIME(PERF_UE_phy_procedures_UE_TX, PERF_ONCE);
        LOG_TX_LINE();
    }
    ...
    PERF_TIME(PERF_UE_THREAD_TX, PERF_ONCE);
    LOG_TX_LINE();
    if (pthread_mutex_lock(&UE->mutex_tx_cnt) != 0) {
...
static void *UE_thread_rx(void *arg)
{
  ...
  while (!oai_exit) {
    ...
    if (pthread_mutex_lock(&UE->mutex_rx_cnt) != 0) {
    ...
    {
        int cnt_rx = UE->instance_cnt_rx;
        
        LOG_RX_LINE();
        if (pthread_mutex_unlock(&UE->mutex_rx_cnt) != 0) {
    ...
    LOG_RX_LINE();
    PERF_TIME(PERF_UE_THREAD_RX, PERF_START);

    my_printf("===>UE_thread_rx start...\n");
	...
	*/
        LOG_RX_LINE();
        PERF_TIME(PERF_UE_phy_procedures_UE_RX, PERF_START);
        phy_procedures_UE_RX( UE, 0, 0, UE->mode, no_relay, NULL );
        PERF_TIME(PERF_UE_phy_procedures_UE_RX, PERF_ONCE);
        LOG_RX_LINE();
      }
	...
	*/
      ...
      {
          LOG_RX_LINE();
          PERF_TIME(PERF_UE_ue_scheduler, PERF_START);
          ret = mac_xface->ue_scheduler(UE->Mod_id,
                                      ...
                                      0,
                                      0/*FIXME CC_id*/);
          PERF_TIME(PERF_UE_ue_scheduler, PERF_ONCE);
    ...
    LOG_RX_LINE();
    PERF_TIME(PERF_UE_THREAD_RX, PERF_ONCE);
 ...
 * \returns a pointer to an int. The storage is not on the heap and must not be freed.
 */
char gacRxFunction[200]={0};
char gacTxFunction[200]={0};
int gulRxLine=0;
int gulTxLine=0;

void setRxFunctionLine(char *func, int line)
{
    sprintf(gacRxFunction, "%s", func);
    gulRxLine = line;
}
void setTxFunctionLine(char *func, int line)
{
    sprintf(gacTxFunction, "%s", func);
    gulTxLine = line;
}
...
void *UE_thread(void *arg)
{
  ...
  attr.size = sizeof(attr);
  attr.sched_flags = 0;
  attr.sched_nice = -20; //xhd_oai_ue modified from 0 
  ...
  while (!oai_exit) 
  {
    ...
    LOG_D(PHY,"====> UE_thread:frame=%d, hw_subframe=%d rx_offset=%d, first_rx=%d rx_off_diff=%d rxpos=%d txpos=%d\n",
        UE->frame_rx,hw_subframe,UE->rx_offset,first_rx,rx_off_diff,rxpos,txpos);
    PERF_TIME(PERF_UE_UE_thread, PERF_START);
    ...
    while (rxpos < (1+hw_subframe)*UE->lte_frame_parms.samples_per_tti) 
    {
      ...
      */
      ...
      if (tx_enabled==1)  
      {
        ...
        {
            //printf("T%d ",txpos);
            //print_buff(txp[0],spp - ((first_rx==1) ? rx_off_diff : 0),32, 
            //    "txpos=%d size=%d\n",txpos,spp - ((first_rx==1) ? rx_off_diff : 0));
        }
        //#endif
    ...
    PERF_TIME(PERF_UE_UE_thread, PERF_ONCE);
    ...
    if (UE->is_synchronized == 1)  
    {
      ...
      if (start_rx_stream == 1) 
      {
        ...
        {
          LOG_E( PHY, "[SCHED][UE] UE RX thread busy (IC %d function:%s line:%d)!!\n", instance_cnt_rx, gacRxFunction, gulRxLine);

          //xhd_oai_ue
    	  if (instance_cnt_rx > 20) {
    	    exit_fun("instance_cnt_rx > 20");
    	    return &UE_thread_retval;
    	  }
          if (pthread_cond_signal(&UE->cond_rx) != 0) {
            LOG_E( PHY, "[SCHED][UE] ERROR pthread_cond_signal for UE RX thread\n" );
        ...
        if ((tx_enabled==1)&&(UE->mode != loop_through_memory)) {
          ...
          if (instance_cnt_tx == 0) {
            ...
            LOG_E( PHY, "[SCHED][UE] UE TX thread busy (IC %d function:%s line:%d)!!\n",instance_cnt_tx, gacTxFunction, gulTxLine );
            //xhd_oai_ue
    	    if (instance_cnt_tx>20) {
    	      exit_fun("instance_cnt_tx > 20");
    	      return &UE_thread_retval;
    	    }
            if (pthread_cond_signal(&UE->cond_tx) != 0) {
              LOG_E( PHY, "[SCHED][UE] ERROR pthread_cond_signal for UE TX thread\n" );
    ...
    if (hw_subframe==10) 
    {
	  ...
	  //	  LOG_I(PHY,"UE->rx_offset %d\n",UE->rx_offset);
          ...
          else if ((UE->rx_offset + UE->rx_offset_diff <(FRAME_LENGTH_COMPLEX_SAMPLES/2)) &&
		     (UE->rx_offset + UE->rx_offset_diff> RX_OFF_MIN) && 
		     (start_rx_stream==1) && 
		     (rx_correction_timer == 0)) 
		  {
            rx_off_diff = -UE->rx_offset -UE->rx_offset_diff + RX_OFF_MIN;
	        LOG_D(PHY,"UE->rx_offset %d + UE->rx_offset_diff %d  > %d, diff %d\n",
                UE->rx_offset,UE->rx_offset_diff,RX_OFF_MIN,rx_off_diff);
            rx_correction_timer = 5;
            if( rx_off_diff < 0)
            {
                //printf("UE_thread:rx_off_diff ERROR 1 !!!! rx_off_diff=%d\n",rx_off_diff);
                //rx_off_diff = 0;
            }
            //UE->rx_offset=0;
          } 
          else if ((UE->rx_offset + UE->rx_offset_diff >(FRAME_LENGTH_COMPLEX_SAMPLES/2)) && 
		     (UE->rx_offset + UE->rx_offset_diff < (FRAME_LENGTH_COMPLEX_SAMPLES-RX_OFF_MIN)) &&
		     (start_rx_stream==1) && 
		     (rx_correction_timer == 0)) 
		  {   

            // moving to the left so drop rx_off_diff samples
            rx_off_diff = FRAME_LENGTH_COMPLEX_SAMPLES - RX_OFF_MIN - UE->rx_offset - UE->rx_offset_diff;
	        LOG_D(PHY,"UE->rx_offset %d + rx_offset_diff %d < %d, diff %d\n",
                UE->rx_offset,UE->rx_offset_diff,FRAME_LENGTH_COMPLEX_SAMPLES-RX_OFF_MIN,rx_off_diff);

            rx_correction_timer = 5;
            //UE->rx_offset=0;
            if( rx_off_diff < 0)
            {
                //printf("UE_thread:rx_off_diff ERROR 2!!!! rx_off_diff=%d\n",rx_off_diff);
                //rx_off_diff = 0;
            }
  ...
  while (!oai_exit)  {
    ...
    if (UE->is_synchronized) {
	  ...
	  int instance_cnt_rx = ++UE->instance_cnt_rx;
          ...
          pthread_mutex_unlock(&UE->mutex_rx_cnt);
	  VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_UE_INST_CNT_RX, instance_cnt_rx);
        ...
        if (pthread_mutex_lock(&UE->mutex_tx_cnt) != 0) {
          ...
          pthread_mutex_unlock(&UE->mutex_tx_cnt);
	  VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_UE_INST_CNT_TX, instance_cnt_tx);
 ...
 */
extern    pthread_attr_t                  attr_UE_thread;
...
void init_UE_threads(void)
{
  ...
  pthread_mutex_init(&UE->mutex_tx_cnt,NULL);
  pthread_mutex_init(&UE->mutex_rx_cnt,NULL);
  ...
  //pthread_create(&UE->thread_tx,&attr_UE_thread,UE_thread_tx,(void*)UE);
  pthread_create(&UE->thread_tx,NULL,UE_thread_tx,(void*)UE);
  pthread_setname_np( UE->thread_tx, "UE_thread_tx" );
  //pthread_create(&UE->thread_rx,&attr_UE_thread,UE_thread_rx,(void*)UE);
...
int setup_ue_buffers(PHY_VARS_UE **phy_vars_ue, openair0_config_t *openair0_cfg, openair0_rf_map rf_map[MAX_NUM_CCs])
{
  ...
  printf("@@@@@inside setup_ue_buffers\n");
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
...
#else
	printf("@@@@@inside setup_ue_buffers 2\n");
    // replace RX signal buffers with mmaped HW versions
    rxdata = (int32_t**)(16+ malloc16(16+ frame_parms->nb_antennas_rx*sizeof(int32_t*) ));
    txdata = (int32_t**)(16+malloc16( 16+frame_parms->nb_antennas_tx*sizeof(int32_t*) ));
