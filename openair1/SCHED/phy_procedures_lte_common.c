...
uint8_t phich_subframe_to_harq_pid(LTE_DL_FRAME_PARMS *frame_parms,uint32_t frame,uint8_t subframe)
{
  ...
  return(subframe2harq_pid(frame_parms,
                           ...
                           phich_subframe2_pusch_subframe(frame_parms,subframe,2)));
}
...
extern void phy_dlsch_dump_ue( PHY_VARS_eNB *phy_vars_eNB);
...
LTE_eNB_UE_stats* get_eNB_UE_stats(uint8_t Mod_id, uint8_t  CC_id,uint16_t rnti)
{
  ...
  if (UE_id == -1) {
    LOG_D(PHY,"get_eNB_UE_stats: UE with rnti %x Mod_id=%d CC_id=%d not found\n",rnti, Mod_id,CC_id);
    //phy_dlsch_dump_ue(PHY_vars_eNB_g[Mod_id][CC_id]);
    return NULL;
  }
...
//xhd_oai_enb
void phy_remove_ue(uint8_t Mod_id, uint8_t  CC_id,uint16_t rnti)
{
  int8_t UE_id;
  ...
  //UE_id = find_ue(rnti, PHY_vars_eNB_g[Mod_id][CC_id]);
  UE_id = RNTI2UEID(rnti); //find_ue(rnti, PHY_vars_eNB_g[Mod_id][CC_id]);

  if (UE_id < NUMBER_OF_UE_MAX) 
  {
      // check at the phy enb_ue state for this rnti
      LOG_E(PHY,"phy_remove_ue: remove UE(rnti=%x)\n", rnti);
      remove_ue(rnti, PHY_vars_eNB_g[Mod_id][CC_id], 0);
  }

  return;
}
void phy_remain_ue(uint8_t Mod_id, uint8_t  CC_id,uint16_t rnti)
{
  int8_t UE_id;
  uint8_t i;
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
    if ((phy_vars_eNB->dlsch_eNB[i]) &&
        ...
        // check at the phy enb_ue state for this rnti
        printf("phy_remain_ue: remove UE(rnti=%x) for rnti %x\n", 
            phy_vars_eNB->dlsch_eNB[i][0]->rnti, rnti);
        remove_ue(phy_vars_eNB->dlsch_eNB[i][0]->rnti, phy_vars_eNB, 0);
        //cancel_ra_proc(0, 0, 0, phy_vars_eNB->dlsch_eNB[i][0]->rnti);
    }
  }

  //clear_ra_proc(0,0);
  cancel_ra_proc_of_others(Mod_id,CC_id,rnti);
...
void phy_dlsch_dump_ue( PHY_VARS_eNB *phy_vars_eNB)
{
  uint8_t i;
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
    ...
    {
      printf("%d(0x%x)",i,phy_vars_eNB->dlsch_eNB[i][0]->rnti);
    }
  }
  printf("\n");
}
uint8_t get_phy_ueid_by_rnti(uint16_t rnti)
{
  uint8_t i;
...
uint16_t get_rnti_by_phy_ueid(uint16_t ueid)
{
  PHY_VARS_eNB *phy_vars_eNB = PHY_vars_eNB_g[0][0];
  
  if (ueid<NUMBER_OF_UE_MAX) {
    return phy_vars_eNB->dlsch_eNB[ueid][0]->rnti;
  }
