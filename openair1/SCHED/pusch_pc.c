...
// This is the formula from Section 5.1.1.1 in 36.213 100*10*log10((2^(MPR*Ks)-1)), 
// where MPR is in the range [0,6] and Ks=1.25

int16_t hundred_times_delta_TF[100] = {
-32768,-1268,-956,-768,-631,-523,-431,-352,-282,-219,
  -161, -107, -57,  -9,  36,  79, 120, 159, 197, 234,
   269,  304, 337, 370, 402, 434, 465, 495, 525, 555,
   583,  612, 640, 668, 696, 723, 750, 777, 803, 829,
   856,  881, 907, 933, 958, 983,1008,1033,1058,1083,
  1108, 1132,1157,1181,1205,1229,1254,1278,1302,1325,
  1349, 1373,1397,1421,1444,1468,1491,1515,1538,1562,
  1585, 1609,1632,1655,1679,1702,1725,1748,1772,1795,
  1818, 1841,1864,1887,1910,1933,1956,1980,2003,2026,
  2049, 2072,2095,2118,2141,2164,2186,2209,2232,2255};

uint16_t hundred_times_log10_NPRB[100] = {
  ...
  1491, 1505,1518,1531,1544,1556,1568,1579,1591,1602,
  1612, 1623,1633,1643,1653,1662,1672,1681,1690,1698,
  1707, 1716,1724,1732,1740,1748,1755,1763,1770,1778,
  1785, 1792,1799,1806,1812,1819,1826,1832,1838,1845,
  1851, 1857,1863,1869,1875,1880,1886,1892,1897,1903,
  1908, 1913,1919,1924,1929,1934,1939,1944,1949,1954,
  1959, 1963,1968,1973,1977,1982,1986,1991,1995,2000};


int16_t get_hundred_times_delta_IF_eNB(
    PHY_VARS_eNB *phy_vars_eNB,uint8_t UE_id,uint8_t harq_pid, uint8_t bw_factor,
    uint8_t fun_id, uint32_t ret, uint32_t ulTBS)
{
  ...
  uint8_t sched_subframe = 0;
  int frame_rx=0;
  int subframe_rx=0;

  if( fun_id >= 100 )
  {
      sched_subframe = fun_id - 100;

      frame_rx = phy_vars_eNB->proc[sched_subframe].frame_rx;
      subframe_rx = phy_vars_eNB->proc[sched_subframe].subframe_rx;
      
      uint8_t harq_pid_t = subframe2harq_pid(&phy_vars_eNB->lte_frame_parms,
        frame_rx,
        subframe_rx);
      ...
      if( harq_pid_t != harq_pid )
      {
            printf("ulsch_decoding:harq_pid(%d-%d) mismatch sched_subframe=%d frame_rx=%d subframe_rx=%d\n",
                harq_pid_t,harq_pid,sched_subframe,
                frame_rx,
                subframe_rx);
      }
  }
  ...
  /*Nre为总的ULSCH子载波数
  比如12个有效符号，nb_rb=10，共有1440个可用子载波*/
...
#warning "This condition happens sometimes. Need more investigation" // navid
  ...
  if( MPR_x100 >= 600 
    || phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->TBS!= phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->TBS2)
  {
        ...
        lte_segmentation(NULL,
                     NULL,
                     phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->TBS+24,
                     &C,
                     &Cplus,
                     &Cminus,
                     &Kplus,
        ...
        printf("get_hundred_times_delta_IF_eNB:UE%d MPR_x100 more than 600:%d TBS=%d-%d-%d sumKr=%d(%d-%d %d-%d %d-%d %d-%d-%d) Nre=%d(%d*%d) harq_pid=%d round=%d fun_id=%d ret=%d sched_subframe=%d %d %d\n", 
            UE_id, MPR_x100,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->TBS,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->TBS2,ulTBS,
            sumKr,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->C,C,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->Cminus,Cminus,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->Kminus,Kminus,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->Kplus,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->Kplus2,
            Kplus,
            Nre,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->Nsymb_initial,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->nb_rb,
            harq_pid,
            phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->round,
            fun_id,ret,
            sched_subframe,frame_rx,subframe_rx
            );
        MPR_x100 = 599;
  }
...
int16_t get_hundred_times_delta_IF_mac(module_id_t module_idP, uint8_t CC_id, rnti_t rnti, uint8_t harq_pid)
{
  ...
  return get_hundred_times_delta_IF_eNB( PHY_vars_eNB_g[module_idP][CC_id], UE_id, harq_pid, 0, 9, 0,0 );
}
...
void pusch_power_cntl(PHY_VARS_UE *phy_vars_ue,uint8_t subframe,uint8_t eNB_id,uint8_t j, uint8_t abstraction_flag)
{
  ...
  {  
    ...
    print_info("[UE  %d][RAPROC] frame %d, subframe %d: Msg3 Po_PUSCH %d dBm (%d,%d,PL=%d,%d,100*f_pusch=%d)\n",
          phy_vars_ue->Mod_id,phy_vars_ue->frame_tx,subframe,
  ...
  { 
    ...
    phy_vars_ue->ulsch_ue[eNB_id]->Po_PUSCH +=  mac_xface->get_Po_NOMINAL_PUSCH(phy_vars_ue->Mod_id,0); //phy_vars_ue->lte_frame_parms.ul_power_control_config_common.p0_NominalPUSCH;
    ...
    print_info("pusch_power_cntl[UE  %d]frame %d, subframe %d harq %d: Po_PUSCH(%d dBm)=[lgNPRB(%d NPRB=%d)+delta_if(%d deltaMCS_Enabled=%d)+f_pusch(%d)*100]/100+alpha(%d)*PL(%d rxgain:%d rsrp:%d ref:%d):%d+Po_NOMINAL_PUSCH(%d/%d), PHR %d,  tx power max%d \n",
          phy_vars_ue->Mod_id,phy_vars_ue->frame_tx,subframe,harq_pid,
          phy_vars_ue->ulsch_ue[eNB_id]->Po_PUSCH,
          hundred_times_log10_NPRB[nb_rb-1],
          nb_rb,
          get_hundred_times_delta_IF(phy_vars_ue,eNB_id,harq_pid),
          ...
          phy_vars_ue->lte_frame_parms.ul_power_control_config_common.alpha,
          PL,
          phy_vars_ue->rx_total_gain_dB,
          dB_fixed_times10(phy_vars_ue->PHY_measurements.rsrp[eNB_id]),
