...
uint32_t sub_block_interleaving_turbo(uint32_t D, uint8_t *d,uint8_t *w)
{
  ...
  for (col=0; col<32; col++) {
    ...
    for (row=0; row<RTC; row++) {
      ...
      if(index3-ND3+5 == 2)
      {
        w[Kpi+1+k2] = LTE_NULL;  //xhd_oai_debug 
      }
  ...
  if (ND>0)
  {
    //w[(3*Kpi) - 1] = LTE_NULL;//xhd_oai_debug
  }
...
uint32_t generate_dummy_w_ver(uint32_t D, uint8_t *w,uint8_t F)
{
...
uint32_t generate_dummy_w(uint32_t D, uint8_t *w,uint8_t F)
{
  ...
  //#if 0
    for (col=0; col<32; col++) {
      ...
      if( index3 + 96< ND3 + 3*F)
      {
        w[k+1]            =  LTE_NULL;
      }
      if( index3 + 96*2 < ND3 + 3*F)
      {
        w[k+2]            =  LTE_NULL;
      }
      if( index3+1 < ND3 + 3*F )
      ...
      if( index3+1 + 96 < ND3 + 3*F )
      {
        w[Kpi+k2+2]       =  LTE_NULL;
      }
      if( index3+1 + 96*2 < ND3 + 3*F )
      {
        w[Kpi+k2+4]       =  LTE_NULL;
      }
      ...
      /*这个判断其实多余，已经包含在index3<ND3+3*F里边了*/
      if(index3-ND3+5 == 2)
      {
        w[Kpi+1+k2] = LTE_NULL;  //xhd_oai_debug 
      }
...
uint32_t lte_rate_matching_turbo_ver(uint32_t RTC,
                                 uint32_t G,
                                 uint8_t *w,
                                 uint8_t *e,
                                 uint8_t C,
...
uint32_t lte_rate_matching_turbo_ue(uint32_t RTC,
                                 uint32_t G,
                                 uint8_t *w,
                                 uint8_t *dummy_w,
                                 uint8_t *e,
                                 ...
                                 uint8_t harq_pid)
{
  ...
  if( harq_pid == 4 )
  {
    print_info("lte_rate_matching_turbo_ue:ind=%d Ncb=%d RTC=%d rvidx=%d E=%d G=%d Nl=%d Qm=%d C=%d Nsoft=%d Kmimo=%d Mdlharq=%d\n",
        ind,Ncb,RTC,rvidx,E,G,Nl,Qm,C,Nsoft,Kmimo,Mdlharq);
  }
  ...
  for (; (ind<Ncb)&&(k<E); ind++) 
  {
    if (w[ind] != LTE_NULL)//xhd_oai_debug
    {
        e2[k++]=w[ind];
    }
  }
...
int lte_rate_matching_turbo_rx_ver(uint32_t RTC,
                               uint32_t G,
                               int16_t *w,
                               uint8_t *dummy_w,
                               int16_t *soft_input,
...
int lte_rate_matching_turbo_rx_ue(uint32_t RTC,
                               uint32_t G,
                               int16_t *w,
                               uint8_t *dummy_w,
                               int16_t *soft_input,
                               ...
                               uint32_t *E_out)
{
  ...
  if (clear==1)  //xhd_oai_debug 最重要的修改
  {
    memset(w,0,Ncb*sizeof(int16_t));
  }
...
int lte_rate_matching_turbo_rx_enb(uint32_t RTC,
                               uint32_t G,
                               int16_t *w,
                               int8_t *wc,
                               uint8_t *dummy_w,
                               ...
                               uint8_t harq_pid)
{
  ...
  uint32_t ulNullCount1 = 0,ulNullCount2 = 0;
  uint32_t ulNotNullCount1a = 0,ulNotNullCount2a = 0;
  uint32_t ulNotNullCount1b = 0,ulNotNullCount2b = 0;
  ...
  if( harq_pid == 4 )
  {
    print_info("lte_rate_matching_turbo_rx_enb:ind=%d-%d Ncb=%d NotNullCount=%d %d %d %d NullCount=%d %d RTC=%d rvidx=%d E=%d G=%d Nl=%d Qm=%d C=%d Nsoft=%d Kmimo=%d Mdlharq=%d\n",
        ind_start,ind,Ncb,ulNotNullCount1a,ulNotNullCount1b,ulNotNullCount2a,ulNotNullCount2b,ulNullCount1,ulNullCount2,
        RTC,rvidx,E,G,Nl,Qm,C,Nsoft,Kmimo,Mdlharq);
  }
