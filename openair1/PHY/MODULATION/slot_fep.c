	     ...
	     int reset_freq_est)
{
  ...
  //unsigned char symbol = l+((7-frame_parms->Ncp)*(Ns)); ///symbol within sub-frame
  ...
  int tmp_dft_in[4096] __attribute__((aligned(16)));  // This is for misalignment issues for 6 and 15 PRBs
  ...
  for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
    ...
    rx_offset = (sample_offset + slot_offset + subframe_offset + nb_prefix_samples0) % frame_length_samples ;
    ...
    if (l==0 ) {
      ...
      rx_offset = rx_offset % frame_length_samples;
  ...
  if (phy_vars_ue->perfect_ce == 0) {
    if ((l==0) || (l==(4-frame_parms->Ncp))) {
      for (aa=0; aa<frame_parms->nb_antennas_tx_eNB; aa++) {
        ...
        //xhd_oai_ue
        //printf("Channel estimation eNB %d, aatx %d, slot %d, symbol %d/%d\n",eNB_id,aa,Ns,l,symbol);
...
int slot_fep_pbch(PHY_VARS_UE *phy_vars_ue,
             unsigned char l,
             unsigned char Ns,
             int sample_offset,
             int no_prefix,
	     int reset_freq_est)
{
  ...
  for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
    ...
    //xhd_oai_ue
    //rx_offset = sample_offset + slot_offset + nb_prefix_samples0 + subframe_offset - SOFFSET;
    rx_offset = sample_offset + slot_offset + subframe_offset + nb_prefix_samples0 ;
    // Align with 128 bit
    //xhd_oai_ue
    //rx_offset = rx_offset - rx_offset % 4;
    ...
    if (l==0 ) {
      ...
      //    frame_parms->ofdm_symbol_size, 0, "DataF of subframe%d symbol %d len=%d\n", 
      //    Ns>>1, (Ns&1)*7+l, frame_parms->ofdm_symbol_size);
      
      //print_buff(
      //  (int16_t *)&ue_common_vars->rxdata[aa][(rx_offset-40) % frame_length_samples], 
      //  40, 8,"CP of subframe%d symbol %d len=%d\n", 
      //  Ns>>1, (Ns&1)*7+l, 40);

      //print_buff(
      //  (int16_t *)&ue_common_vars->rxdata[aa][(rx_offset) % frame_length_samples], 
      //  frame_parms->ofdm_symbol_size, 0,"Data of subframe%d symbol %d len=%d\n", 
      //  Ns>>1, (Ns&1)*7+l, frame_parms->ofdm_symbol_size);
    ...
    //print_buff(
    //  (int16_t *)&ue_common_vars->rxdata[aa][(rx_offset-36) % frame_length_samples], 
    //  36, 12, "CP of subframe%d symbol %d len=%d\n", 
    //  Ns>>1, (Ns&1)*7+l, 36);
