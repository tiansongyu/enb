...
#ifndef OFDMA_ULSCH
        //xhd_oai_60m
        (int16_t *)&eNB_common_vars->rxdata_7_5kHz[eNB_id][(Ns>>1)&1][aa][slot_offset +
            nb_prefix_samples0],
#else
        (int16_t *)&eNB_common_vars->rxdata[eNB_id][aa][((frame_parms->samples_per_tti>>1)*Ns) +
            nb_prefix_samples0],
#endif
        (int16_t *)&eNB_common_vars->rxdataF[eNB_id][(Ns>>1)&1][aa][frame_parms->ofdm_symbol_size*symbol],1);
    } else {
      dft(
#ifndef OFDMA_ULSCH
        (short *)&eNB_common_vars->rxdata_7_5kHz[eNB_id][(Ns>>1)&1][aa][slot_offset +
#else
        (short *)&eNB_common_vars->rxdata[eNB_id][aa][((frame_parms->samples_per_tti>>1)*Ns) +
#endif
            (frame_parms->ofdm_symbol_size+nb_prefix_samples0+nb_prefix_samples) +
            (frame_parms->ofdm_symbol_size+nb_prefix_samples)*(l-1)],
        (short*)&eNB_common_vars->rxdataF[eNB_id][(Ns>>1)&1][aa][frame_parms->ofdm_symbol_size*symbol],1);
    }
  }
