                      ...
                      short coef)
{
  ...
  if ( diff > SYNCH_HYST )
  {
    ...
    my_printf("lte_adjust_synch rx_offset increase to %d\n", phy_vars_ue->rx_offset);
  }
  else if (diff < -SYNCH_HYST)
  {
    ...
    if ( phy_vars_ue->rx_offset < 0 )
      phy_vars_ue->rx_offset += FRAME_LENGTH_COMPLEX_SAMPLES;
    my_printf("lte_adjust_synch rx_offset decrease to %d\n", phy_vars_ue->rx_offset);
  }
...
extern int ta_detect_posi_sum[NUMBER_OF_UE_MAX];
extern int ta_detect_posi_num[NUMBER_OF_UE_MAX];
extern int ta_detect_nposi_sum[NUMBER_OF_UE_MAX];
extern int ta_detect_nposi_num[NUMBER_OF_UE_MAX];
extern int ta_detect_zero_num[NUMBER_OF_UE_MAX];
...
int lte_est_timing_advance_pusch(PHY_VARS_eNB* phy_vars_eNB,uint8_t UE_id,uint8_t sched_subframe)
{
  ...
  // filter position to reduce jitter
  if (phy_vars_eNB->first_run_timing_advance[UE_id] == 1) 
  {
    phy_vars_eNB->first_run_timing_advance[UE_id]=0;
    phy_vars_eNB->max_pos_fil2[UE_id] = max_pos;
  } 
  else
  {
    phy_vars_eNB->max_pos_fil2[UE_id] = ((phy_vars_eNB->max_pos_fil2[UE_id] * coef) + (max_pos * ncoef)) >> 15;
  }

  if( max_pos == 0)
  {
      ta_detect_zero_num[UE_id]++;
  }
  else if( max_pos > 0)
  {
      ta_detect_posi_sum[UE_id] +=  max_pos;
      ta_detect_posi_num[UE_id]++;
  }
  else
  {
      ta_detect_nposi_sum[UE_id] +=  max_pos;
      ta_detect_nposi_num[UE_id]++;
  }
  
#ifdef DEBUG_PHY
  LOG_D(PHY,"frame %d: max_pos = %d, max_pos_fil = %d, sync_pos=%d UE_id=%d\n",
     phy_vars_eNB->proc[sched_subframe].frame_rx,max_pos,phy_vars_eNB->max_pos_fil2[UE_id],sync_pos, UE_id);
#endif //DEBUG_PHY

  return(phy_vars_eNB->max_pos_fil2[UE_id]-sync_pos);
}
