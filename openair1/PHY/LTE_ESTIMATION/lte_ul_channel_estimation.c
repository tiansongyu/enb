...
int16_t tan_128[128] = {
         201, 402, 603, 804, 1006, 1207, 1408, 1610, 1811, 2013, 2215, 2417, 2619, 2822, 3024, 3227, 
         3431, 3634, 3838, 4042, 4246, 4450, 4655, 4861, 5066, 5272, 5479, 5686, 5893, 6101, 6309, 6518, 
         6727, 6937, 7147, 7358, 7570, 7782, 7995, 8208, 8422, 8637, 8852, 9068, 9285, 9503, 9721, 9940, 
         10160, 10381, 10603, 10825, 11049, 11273, 11498, 11725, 11952, 12180, 12410, 12640, 12872, 13104, 13338, 13573, 
         13809, 14046, 14285, 14525, 14766, 15009, 15253, 15498, 15745, 15993, 16243, 16494, 16747, 17001, 17257, 17515, 
         17774, 18035, 18298, 18563, 18829, 19098, 19368, 19640, 19915, 20191, 20470, 20750, 21033, 21318, 21605, 21895, 
         22187, 22481, 22778, 23078, 23380, 23685, 23992, 24302, 24615, 24931, 25250, 25572, 25897, 26226, 26557, 26892, 
         27230, 27572, 27917, 28266, 28618, 28975, 29335, 29699, 30067, 30440, 30817, 31198, 31583, 31973, 32368, 32767};



uint16_t chazhi_get_tan(int16_t *ch1, int16_t *ch2)
{
    ...
    /*tan(a-b)=(tan(a)-tan(b))/(1+tan(a)*tan(b))
      IM = x1/y1-x2/y2 ==> x1*y2-x2*y1
      RE = 1+(x1/y1)*(x2/y2) ==> y1*y2+x1*x2*/
    iv0 = (ch1[0]*ch2[1]-ch2[0]*ch1[1]);
    rv0 = (ch1[1]*ch2[1]+ch1[0]*ch2[0]);
    ...
    for (int i=0; i<6; i++) 
    {
        ...
        if( count2 < 60)
        {
            print_info("chazhi_get_tan: iv=%d tan_128[%d](=%d)*rv(%d)=%d  (((tan_128[delta_phase]*rv))>>15)=%d\n",
              iv, delta_phase, tan_128[delta_phase],
              rv, tan_128[delta_phase]*rv, (((tan_128[delta_phase]*rv))>>15));
            count2++;
        }
        if(iv<(((tan_128[delta_phase]*rv))>>15))
        {
            (delta_phase -= (32>>i));
            //print_info("new delta_phase AAA=%d\n", delta_phase);
        }
        else
        {
            (delta_phase += (32>>i));
            //print_info("new delta_phase BBB=%d\n", delta_phase);
        }
    }
    if( reverse == 1)
    {
        //print_info("new delta_phase CCC1=%d\n", delta_phase);
        delta_phase = 256-delta_phase;
        //print_info("new delta_phase CCC=%d\n", delta_phase);
    }
    if( rv_flag == -1)
    {
        delta_phase = 512-delta_phase;
        //print_info("new delta_phase DDD=%d\n", delta_phase);
    }

    static int count = 0;
    if( count < 10 /*|| rv_flag==-1 || reverse == 1 || rv_flag==-1*/ )
    {
        print_info("chazhi_get_tan:%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d delta =%d reverse=%d rv_flag=%d iv_flag=%d\n", 
            ch1[0], ch2[0], rv, rv0,
            ch1[1], ch2[1], iv, iv0,
            delta_phase, reverse, rv_flag, iv_flag
            );
        count++;
    }
    return delta_phase *iv_flag;    

}
void chazhi_base(int16_t *ch1, int16_t *ch2, int16_t delta_phase, int sum, int pos, int16_t *ch)
{

    int32_t rot[2][2];
    int16_t current_phase[2];
    ...
    current_phase[0] = ((pos)*delta_phase/sum)>>1;
    current_phase[1] = ((pos-sum)*delta_phase/sum)>>1;
    ...
    for( int i = 0; i<2; i++)
    {
        if(current_phase[i] > 128) 
        {
            current_phase[i] = 256 - current_phase[i];
            rot[i][0] = ((-(rotate_128[2*current_phase[i]]*rv_flag)*alpha[i])>>3);
            rot[i][1] = (((rotate_128[2*current_phase[i]+1]*iv_flag)*alpha[i])>>3);
        }
        else if(current_phase[i] >= 0) 
        {
            rot[i][0] = (((rotate_128[2*current_phase[i]]*rv_flag)*alpha[i])>>3);
            rot[i][1] = (((rotate_128[2*current_phase[i]+1]*iv_flag)*alpha[i])>>3);
        }
    ...
    ch[0] = (((ch1[0]*rot[0][0]-ch1[1]*rot[0][1]+ch2[0]*rot[1][0]-ch2[1]*rot[1][1])/sum)>>12);
    ch[1] = (((ch1[0]*rot[0][1]+ch1[1]*rot[0][0]+ch2[0]*rot[1][1]+ch2[1]*rot[1][0])/sum)>>12);

    static int count = 0;
    if( count < 10 )
    {
        print_info("chazhi_base[%d]:\t%d\t%d\t%d\t%d\t%d\t%d\t delta =%d sum=%d pos=%d phase0=%d phase1=%d rot0[%d,%d] rot1[%d,%d] rv_flag=%d iv_flag=%d\n", 
            count,
            ch1[0], ch2[0], ch[0], ch1[1],ch2[1],ch[1],
            delta_phase, sum, pos, 
            current_phase[0],
            current_phase[1],
            rot[0][0],rot[0][1],
...
#define chazhi1(ch, type){\
    if(type != 0){\
        ch[2 ]=((ch[0]*28 +ch[12]*5 )>>5);ch[3 ]=((ch[1]*28+ch[13]*5 )>>5);\
        ch[4 ]=((ch[0]*23 +ch[12]*11)>>5);ch[5 ]=((ch[1]*23+ch[13]*11)>>5);\
        ch[6 ]=((ch[0]*18 +ch[12]*18)>>5);ch[7 ]=((ch[1]*18+ch[13]*18)>>5);\
        ch[8 ]=((ch[0]*11 +ch[12]*23)>>5);ch[9 ]=((ch[1]*11+ch[13]*23)>>5);\
        ch[10]=((ch[0]*5  +ch[12]*28)>>5);ch[11]=((ch[1]*5 +ch[13]*28)>>5);\
    }\
    if(type == 2 || type == 4){\
        ch[14]=(-ch[0]*5  +ch[12]*35 )>>5; ch[15]=(-ch[1]*5  +ch[13]*35 )>>5;\
        ch[16]=(-ch[0]*10 +ch[12]*40 )>>5; ch[17]=(-ch[1]*10 +ch[13]*40 )>>5;\
    }\
    if( type == 2){\
        ch[18]=((-ch[0]*14+ch[12]*42)>>5); ch[19]=((-ch[1]*14+ch[13]*42)>>5);\
        ch[20]=((-ch[0]*18+ch[12]*45)>>5); ch[21]=((-ch[1]*18+ch[13]*45)>>5);\
        ch[22]=((-ch[0]*20+ch[12]*44)>>5); ch[23]=((-ch[1]*20+ch[13]*44)>>5);\
    }\
    if(type == 3){\
        ch -= 6;\
        ch[0]=((ch[6]*42  -ch[18]*14)>>5); ch[1]=((ch[7]*42  -ch[19]*14)>>5);\
        ch[2]=((ch[6]*40  -ch[18]*10)>>5); ch[3]=((ch[7]*40 -ch[19]*10)>>5);\
        ch[4]=((ch[6]*35  -ch[18]*5)>>5); ch[5]=((ch[7]*35  -ch[19]*5)>>5);\
        ch += 6;\
    }\
}

#define chazhi0(ch, type){\
    if(type != 0){\
        ch[2 ]=(ch[0]*5+ch[12]  )/6;ch[3 ]=(ch[1]*5+ch[13]  )/6;\
        ch[4 ]=(ch[0]*2+ch[12]  )/3;ch[5 ]=(ch[1]*2+ch[13]  )/3;\
        ch[6 ]=(ch[0]  +ch[12]  )/2;ch[7 ]=(ch[1]  +ch[13]  )/2;\
        ch[8 ]=(ch[0]  +ch[12]*2)/3;ch[9 ]=(ch[1]  +ch[13]*2)/3;\
        ch[10]=(ch[0]  +ch[12]*5)/6;ch[11]=(ch[1]  +ch[13]*5)/6;\
    }\
    if(type == 2 || type == 4){\
        ch[14]=(-ch[0]  +ch[12]*7 )/6; ch[15]=(-ch[1]  +ch[13]*7 )/6;\
        ch[16]=(-ch[0]  +ch[12]*4 )/3; ch[17]=(-ch[1]  +ch[13]*4 )/3;\
    }\
    if( type == 2){\
        ch[18]=(-ch[0]  +ch[12]*3 )/2; ch[19]=(-ch[1]  +ch[13]*3 )/2;\
        ch[20]=(-ch[0]*2+ch[12]*5 )/3; ch[21]=(-ch[1]*2+ch[13]*5 )/3;\
        ch[22]=(-ch[0]*5+ch[12]*11)/6; ch[23]=(-ch[1]*5+ch[13]*11)/6;\
    }\
    if(type == 3){\
        ch -= 6;\
        ch[0]=(ch[6]*9  -ch[18]*3)/6; ch[1]=(ch[7]*9  -ch[19]*3)/6;\
        ch[2]=(ch[6]*8  -ch[18]*2)/6; ch[3]=(ch[7]*8  -ch[19]*2)/6;\
        ch[4]=(ch[6]*7  -ch[18]  )/6; ch[5]=(ch[7]*7  -ch[19]  )/6;\
        ch += 6;\
    }\
}
...
#define chazhi2(ch, type, delta){\
    if(type != 0){\
        chazhi_base(ch, ch+12,delta,6,1,ch+2);\
        chazhi_base(ch, ch+12,delta,6,2,ch+4);\
        chazhi_base(ch, ch+12,delta,6,3,ch+6);\
        chazhi_base(ch, ch+12,delta,6,4,ch+8);\
        chazhi_base(ch, ch+12,delta,6,5,ch+10);\
    }\
    if(type == 2 || type == 4){\
        chazhi_base(ch, ch+12,delta,6,7,ch+14);\
        chazhi_base(ch, ch+12,delta,6,8,ch+16);\
    }\
    if( type == 2){\
        chazhi_base(ch, ch+12,delta,6,9,ch+18);\
        chazhi_base(ch, ch+12,delta,6,10,ch+20);\
        chazhi_base(ch, ch+12,delta,6,11,ch+22);\
    }\
    if(type == 3){\
        ch -= 6;\
        chazhi_base(ch+6, ch+18,delta,6,-3,ch);\
        chazhi_base(ch+6, ch+18,delta,6,-2,ch+2);\
        chazhi_base(ch+6, ch+18,delta,6,-1,ch+4);\
        ch += 6;\
    }\
}

#define chazhi_symbol_a2(p1, p2, ch1, ch2, delta){\
    chazhi_base(p1, p2,delta,3,1,ch1);\
    chazhi_base(p1, p2,delta,3,2,ch2);\
}
#define chazhi_symbol_b2(p1, p2, ch1, ch2, ch3, delta){\
    chazhi_base(p1, p2,delta,4,1,ch1);\
    chazhi_base(p1, p2,delta,4,2,ch2);\
    chazhi_base(p1, p2,delta,4,3,ch3);\
}
#define chazhi_symbol_c2(p1, p2, offset,delta){\
    chazhi_base(p1, p2,delta,7,-3,p1-3*offset);\
    chazhi_base(p1, p2,delta,7,-2,p1-2*offset);\
    chazhi_base(p1, p2,delta,7,-1,p1-offset);\
    chazhi_base(p1, p2,delta,7,1,p1+offset);\
    chazhi_base(p1, p2,delta,7,2,p1+2*offset);\
    chazhi_base(p1, p2,delta,7,3,p1+3*offset);\
    chazhi_base(p1, p2,delta,7,4,p1+4*offset);\
    chazhi_base(p1, p2,delta,7,5,p1+5*offset);\
    chazhi_base(p1, p2,delta,7,6,p1+6*offset);\
    chazhi_base(p1, p2,delta,7,8,p2+offset);\
    chazhi_base(p1, p2,delta,7,9,p2+2*offset);\
    chazhi_base(p1, p2,delta,7,10,p2+3*offset);\
}

#define chazhi_symbol_a0(p1, p2, ch1, ch2){\
    ch1[0] = ((p1[0]*2+p2[0])/3);ch1[1] = ((p1[1]*2+p2[1])/3);\
    ch2[0] = ((p1[0]+p2[0]*2)/3);ch2[1] = ((p1[1]+p2[1]*2)/3);\
}
#define chazhi_symbol_b0(p1, p2, ch1, ch2, ch3){\
    ch1[0] = ((p1[0]*3+p2[0])>>2);ch1[1] = ((p1[1]*3+p2[1])>>2);\
    ch2[0] = ((p1[0]+p2[0])>>1);ch2[1] = ((p1[1]+p2[1])>>1);\
    ch3[0] = ((p1[0]+p2[0]*3)>>2);ch3[1] = ((p1[1]+p2[1]*3)>>2);\
}
#define chazhi_symbol_c0(p1, p2, offset){\
    *(p1-3*offset) = ((p1[0]*10-p2[0]*3)/7); *(p1-3*offset+1) = ((p1[1]*10-p2[1]*3)/7);\
    *(p1-2*offset) = ((p1[0]*9 -p2[0]*2)/7); *(p1-2*offset+1) = ((p1[1]*9 -p2[1]*2)/7);\
    *(p1-  offset) = ((p1[0]*8 -p2[0]  )/7); *(p1-  offset+1) = ((p1[1]*8 -p2[1]  )/7);\
    *(p1+  offset) = ((p1[0]*6 +p2[0]  )/7); *(p1+  offset+1) = ((p1[1]*6 +p2[1]  )/7);\
    *(p1+2*offset) = ((p1[0]*5 +p2[0]*2)/7); *(p1+2*offset+1) = ((p1[1]*5 +p2[1]*2)/7);\
    *(p1+3*offset) = ((p1[0]*4 +p2[0]*3)/7); *(p1+3*offset+1) = ((p1[1]*4 +p2[1]*3)/7);\
    *(p1+4*offset) = ((p1[0]*3 +p2[0]*4)/7); *(p1+4*offset+1) = ((p1[1]*3 +p2[1]*4)/7);\
    *(p1+5*offset) = ((p1[0]*2 +p2[0]*5)/7); *(p1+5*offset+1) = ((p1[1]*2 +p2[1]*5)/7);\
    *(p1+6*offset) = ((p1[0]   +p2[0]*6)/7); *(p1+6*offset+1) = ((p1[1]   +p2[1]*6)/7);\
    *(p2+  offset) = ((-p1[0]  +p2[0]*8)/7); *(p2+  offset+1) = ((-p1[1]  +p2[1]*8)/7);\
    *(p2+2*offset) = ((-p1[0]*2+p2[0]*9)/7); *(p2+2*offset+1) = ((-p1[1]*2+p2[1]*9)/7);\
    *(p2+3*offset) = ((-p1[0]*3+p2[0]*10)/7); *(p2+3*offset+1) = ((-p1[1]*3+p2[1]*10)/7);\
}



#define chazhi(ch, type){\
    int16_t delta = chazhi_get_tan(ch, ch+12);\
    if( delta < 24 && delta > -24 ){chazhi0(ch, type);}else{chazhi2(ch, type, delta);}\
}
#define get_ch(ch, pil, rxF, type){\
...
#define chazhi_symbol_a(p1, p2, ch1, ch2){\
    int16_t delta = chazhi_get_tan(p1, p2);\
    if( delta < 24 && delta > -24 ){chazhi_symbol_a0(p1, p2, ch1, ch2);}else{chazhi_symbol_a2(p1, p2, ch1, ch2, delta);}\
}
#define chazhi_symbol_b(p1, p2, ch1, ch2, ch3){\
    int16_t delta = chazhi_get_tan(p1, p2);\
    if( delta < 24 && delta > -24 ){chazhi_symbol_b0(p1, p2, ch1, ch2, ch3);}else{chazhi_symbol_b2(p1, p2, ch1, ch2, ch3, delta);}\
}
#define chazhi_symbol_c(p1, p2, offset){\
    int16_t delta = chazhi_get_tan(p1, p2);\
    if( delta < 24 && delta > -24 ){chazhi_symbol_c0(p1, p2, offset);}else{chazhi_symbol_c2(p1, p2, offset, delta);}\
}
                                  ...
                                  uint8_t cooperation_flag)
{
  ...
  int16_t alpha, beta;
  int16_t *ul_ch1=NULL, *ul_ch2=NULL;
  ...
  /*
  print_info("lte_ul_channel_estimation: cyclic_shift=%d cyclicShift=%d n_DMRS2[harq%d]=%d nPRS[subframe%d Ns%d]=%d\n",
       cyclic_shift,
       frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.cyclicShift,
       harq_pid, phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->n_DMRS2,
       subframe, Ns,
       frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.nPRS[(subframe<<1)+Ns]);*/
  ...
  if (l == (3 - frame_parms->Ncp)) 
  {
    ...
    for (aa=0; aa<nb_antennas_rx; aa++) {
      ...
      if (Ns&1) 
      {
        ...
        int offset = frame_parms->N_RB_UL*12;
        ul_ch1 = (int16_t *)&ul_ch_estimates[aa][offset*pilot_pos1];
        ul_ch2 = (int16_t *)&ul_ch_estimates[aa][offset*pilot_pos2];

        offset*=2;
        
        for(int i = 0; i< Msc_RS; i++)
        {
            chazhi_symbol_c(ul_ch1, ul_ch2, offset);  
            ul_ch1+=2;
            ul_ch2+=2;
        }
        if( (subframe == 4  || subframe == 8)&& (frame&0xf)== 0 )
        {
            for( int i = 0; i<14; i++)
            {
                print_buff_excel(0,&ul_ch_estimates[aa][frame_parms->N_RB_UL*12*i],Msc_RS,
                 "lte_ul_channel_estimation: subframe=%d Ns=%d symbol=%d  N_rb_alloc=%d UE_id=%d cyclic_shift=%d(%d %d %d) \n",
                      subframe,Ns,i,N_rb_alloc,UE_id,
                      cyclic_shift,
                      frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.cyclicShift,
                      phy_vars_eNB->ulsch_eNB[UE_id]->harq_processes[harq_pid]->n_DMRS2,
                      frame_parms->pusch_config_common.ul_ReferenceSignalsPUSCH.nPRS[(subframe<<1)+Ns]
...
int32_t lte_ul_channel_estimation_ver(PHY_VARS_eNB *phy_vars_eNB,
                                  uint8_t eNB_id,
                                  uint8_t UE_id,
                                  uint8_t sched_subframe,
                                  unsigned char l,
  ...
  if ((1<<(sub_frame_number%T_SFC))&transmission_offset_tdd[Ssrs]) {
    ...
    for (aa=0; aa<nb_antennas_rx; aa++) {
      ...
      msg("SRS channel estimation eNb %d, subframs %d, aarx %d, %p, %p, %p\n",eNb_id,sub_frame_number,aa,
          &eNb_common_vars->rxdataF[eNb_id][sub_frame_number&1][aa][2*frame_parms->ofdm_symbol_size*symbol],
          eNb_srs_vars->srs,
      ...
      mult_cpx_conj_vector((int16_t*) &eNb_common_vars->rxdataF[eNb_id][sub_frame_number&1][aa][2*frame_parms->ofdm_symbol_size*symbol],
                                      ...
                                      uint16_t nb_rb)
{
  ...
  avg[0] = (((unsigned int*)&avg128U1)[0] +
            ((unsigned int*)&avg128U1)[1] +
            ((unsigned int*)&avg128U1)[2] +
            ((unsigned int*)&avg128U1)[3])/(nb_rb*12);

  avg[1] = (((unsigned int*)&avg128U2)[0] +
            ((unsigned int*)&avg128U2)[1] +
            ((unsigned int*)&avg128U2)[2] +
            ((unsigned int*)&avg128U2)[3])/(nb_rb*12);
  ...
  output_shift = gTableWidth[avg[1]];
  print_info("lte_ul_freq_offset_estimation:avg[1]=%d,output_shift=%d\n",avg[1], output_shift);
  ...
  for (rb=0; rb<nb_rb; rb++) {
    ...
    R[0] = _mm_add_epi16(R[0],_mm_srai_epi16(R[2],1));
