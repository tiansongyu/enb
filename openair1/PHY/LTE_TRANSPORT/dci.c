                  ...
                  uint16_t rnti)
{
  ...
  uint8_t d[3*(MAX_DCI_SIZE_BITS + 16) + 96]={0};
  uint8_t w[3*3*(MAX_DCI_SIZE_BITS+16)]={0};
                 ...
                 uint8_t is_secondary_ue)
{
  ...
  //log2_maxh = (ucWidth/2) + 6 + frame_parms->nb_antennas_rx - 1;
  
  print_info("rx_pdcch: ucWidth=%d log2_maxh=%d\n",ucWidth,log2_maxh);
                              ...
                              uint8_t subframe)
{
  ...
  { 
    ...
    {   
          // 10ms switching periodicity is always 3 symbols, any DL-only subframe is 3 symbols
          nCCEmin = 3;
    }
  }
                         ...
                         uint32_t subframe)
{
  ...
  int16_t re_offset;
  uint8_t mi = get_mi(frame_parms,subframe);
  uint8_t e[DCI_BITS_MAX];  //xhd_oai_debug È¥µôstatic
...
//#ifdef DEBUG_DCI_ENCODING
        LOG_D(PHY,"Generating common DCI %d/%d (nCCE %d) of length %d, aggregation %d (%x)\n",
            i,num_common_dci,dci_alloc[i].nCCE,dci_alloc[i].dci_length,1<<dci_alloc[i].L,
              *(unsigned int*)dci_alloc[i].dci_pdu);
        dump_dci(frame_parms,&dci_alloc[i]);
//#endif

        if (dci_alloc[i].nCCE>=88)
        {
            LOG_E(PHY,"generate_dci_top:AAA nCCE=%d i=%d num_common_dci=%d\n", dci_alloc[i].nCCE, i,num_common_dci);
        }
        else if (dci_alloc[i].nCCE>=0) {
...
//#ifdef DEBUG_DCI_ENCODING
        LOG_D(PHY," Generating UE (rnti %x) specific DCI %d of length %d, aggregation %d, format %d (%x)\n",dci_alloc[i].rnti,i,dci_alloc[i].dci_length,1<<dci_alloc[i].L,dci_alloc[i].format,
              dci_alloc[i].dci_pdu);
        dump_dci(frame_parms,&dci_alloc[i]);
//#endif
        ...
        if (dci_alloc[i].nCCE>=88)
        {
            LOG_E(PHY,"generate_dci_top:BBB nCCE=%d i=%d num_common_dci=%d num_ue_spec_dci=%d\n", 
                dci_alloc[i].nCCE, i, num_common_dci, num_ue_spec_dci);
        }
        else if (dci_alloc[i].nCCE >= 0) {
          e_ptr = generate_dci0(dci_alloc[i].dci_pdu,
                                e+(72*dci_alloc[i].nCCE),
  ...
  for (kprime=0; kprime<frame_parms->N_RB_DL*12; kprime++) {
    for (lprime=0; lprime<num_pdcch_symbols; lprime++) {
      ...
      if (check_phich_reg(frame_parms,kprime,lprime,mi) == 1) {
#ifdef DEBUG_DCI_ENCODING
        //print_info("generate_dci: skipping REG %d (kprime %d, lprime %d)\n",(lprime==0)?(kprime/6) : (kprime>>2),kprime,lprime);
#endif
      } else {
                  ...
                  uint8_t *decoded_output)
{
  ...
  uint8_t dummy_w_rx[3*(MAX_DCI_SIZE_BITS+16+64)]={0};
  int8_t w_rx[3*(MAX_DCI_SIZE_BITS+16+32)]={0},d_rx[96+(3*(MAX_DCI_SIZE_BITS+16))]={0};
...
#endif
  PERF_TIME_LINE(PERF_UE_dci_decoding_procedure, PERF_LINE);
...
uint16_t get_nCCE(uint8_t num_pdcch_symbols,LTE_DL_FRAME_PARMS *frame_parms,uint8_t mi)
{
    ...
    usNcce = get_nquad(2/*num_pdcch_symbols*/,frame_parms,mi)/9;

    //xhd_oai_64m
    #if 0
    if(usCount++==5000)
    {
        printf("###get_nCCE:usNcce=%d num_pdcch_symbols=%d mi=%d phich_resource=%d\n", 
            usNcce, num_pdcch_symbols,mi,
...
void dci_decoding_procedure1(LTE_UE_PDCCH **lte_ue_pdcch_vars,
			     int do_common,
			     int frame, uint8_t subframe,
                             DCI_ALLOC_t *dci_alloc,
                             int16_t eNB_id,
                             LTE_DL_FRAME_PARMS *frame_parms,
                             uint8_t mi,
                             ...
                             uint32_t *CCEmap2)
{
  ...
  uint8_t aucCceIdx[3] = {0};
  uint16_t ausCrc[3] = {0};
  
  //xhd_oai_ue
  uint8_t dci_decoded_output[(MAX_DCI_SIZE_BITS+64)/8]={0};

  nCCE = get_nCCE(lte_ue_pdcch_vars[eNB_id]->num_pdcch_symbols,frame_parms,mi);

  if (nCCE > get_nCCE(3,frame_parms,1))
  {
    printf("dci_decoding_procedure1:return nCCE(%d)>max(%d)\n",nCCE,get_nCCE(3,frame_parms,1));
    return;
  }
  
  if (nCCE<L2)
  {
    printf("dci_decoding_procedure1:return nCCE(%d)<L2(%d)\n",nCCE,L2);
    return;
  }
  else
  {
  ...
  for (m=0; m<nb_candidates; m++) {
    ...
    if (CCEmap_cand == 0) {
      ...
      if (do_common == 1)
        LOG_I(PHY,"[DCI search - common] Attempting candidate %d Aggregation Level %d DCI length %d at CCE %d/%d (CCEmap %x,CCEmap_mask %x)\n",
            m,L2,sizeof_bits,CCEind,nCCE,*CCEmap,CCEmap_mask);
      else if(L2==1)
        LOG_I(PHY,"[DCI search - ue spec] Attempting candidate %d Aggregation Level %d DCI length %d at CCE %d/%d (CCEmap %x,CCEmap_mask %x)\n",
            m,L2,sizeof_bits,CCEind,nCCE,*CCEmap,CCEmap_mask);
      ...
      */
      ...
      //if(count >= 1000)
      {
          printf("dci_decoding_procedure0: L=%d crc =>%x crnti=%x si_rnti=%x ra_rnti=%x\n",L, 
            crc,lte_ue_pdcch_vars[eNB_id]->crnti,si_rnti,ra_rnti);
          count = 0;
      }
#endif
      if( L2 == 11)
      {
          LOG_I(PHY,"[DCI search - ue spec] Attempting candidate %d Aggregation Level %d DCI length %d at CCE %d/%d (CCEmap %x,CCEmap_mask %x) crc =>%x crnti=%x si_rnti=%x ra_rnti=%x\n",
                  m,L2,sizeof_bits,CCEind,nCCE,*CCEmap,CCEmap_mask,
                  crc,
                  lte_ue_pdcch_vars[eNB_id]->crnti,si_rnti,ra_rnti);
      }
      PERF_TIME_LINE(PERF_UE_dci_decoding_procedure, PERF_LINE);
      ...
      if (((do_common == 1) /*&&(L>1)*/ &&((crc == si_rnti)||
                     (crc == ra_rnti)))||
          (crc == lte_ue_pdcch_vars[eNB_id]->crnti))   
      {
        ...
        else if (crc==lte_ue_pdcch_vars[eNB_id]->crnti) 
        {
          ...
          if ((format_c == format0)&&((dci_decoded_output[0]&0x80)==0)) 
          {
              ...
              *dci_cnt = *dci_cnt+1;
              lte_ue_pdcch_vars[eNB_id]->nCCE[subframe]=CCEind;
                ...
                if( (((frame<<1)+subframe)&7) == 0)//xhd_oai_debug
                {
                    if(L == 0)
                    {
                        print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 0 in %d/%d succ! L=%d\n",
                          frame, subframe,CCEind,nCCE,L);
                    }
                    else
                    {
                        print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 0 in %d/%d so strange! L=%d crc=%x/%x\n",
                          frame, subframe,CCEind,nCCE,L,crc,lte_ue_pdcch_vars[eNB_id]->crnti);
                    }
                }
            }
            else
            ...
            if (*format1a_found == 0) 
            {
                if(do_common == 1)
                {
                    ...
                    *format1a_found = 1;
                    ...
                    //print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 1A in %d/%d succ! L=%d\n",
                    //  frame, subframe,CCEind,nCCE,L);
                ...
                else
                {
                    if( (((frame<<1)+subframe)&7) == 0)//xhd_oai_debug
                    {
                        print_info("dci_decoding_procedure1:so strange of format1A for UE specific: frame=%d subframe=%d crc=%x rntl=%x L=%d CCEind=%d nCCE=%d\n",
                            frame, subframe, crc,lte_ue_pdcch_vars[eNB_id]->crnti,L,CCEind,nCCE);
                    }
                    continue;
                }
            }
          ...
          else 
          {
            ...
            if (*format_c_found == 0) 
            {
              ...
              *format_c_found = 1;
              ...
              if(L==0 )
              {
                print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 1 in %d/%d success! L=%d sizeof_bits=%d\n",
                    frame, subframe,CCEind,nCCE,L,sizeof_bits);
              }
              else
              {
                  print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 1 in %d/%d so strange! L=%d sizeof_bits=%d crc=%x/%x\n",
                      frame, subframe,CCEind,nCCE,L,sizeof_bits,crc,lte_ue_pdcch_vars[eNB_id]->crnti);
              }
            }
            else
            {
                //if(L==0 && (frame&0xf)==0 && subframe == 0)
                {
                  //print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 1 in %d/%d fail! L=%d\n",
                  //  frame, subframe,CCEind,nCCE,L);
                }
                continue;
            }
...
//#endif
  ...
  if( (((frame<<1)+subframe)&7) == 0) //xhd_oai_debug
  {
      if(L == 0 && (format_c == format0) )
      {
        if( *format0_found == 0 )
        {
            print_info("===>dci_decoding_procedure1:frame=%d subframe=%d found dci format 0 in  %d-%d-%d/%d fail! crc=[%x %x %x] si_rnti=%x ra_rnti=%x c_rnti=%x dci_cnt=%d [%d %d %d %x][%d %d %d %x][%d %d %d %x]\n",
              frame, subframe,
              aucCceIdx[0],aucCceIdx[1],aucCceIdx[2],nCCE,
              ausCrc[0],ausCrc[1],ausCrc[2],
              si_rnti,ra_rnti,lte_ue_pdcch_vars[eNB_id]->crnti,
              *dci_cnt,
              dci_alloc[0].format,
              dci_alloc[0].nCCE,
              dci_alloc[0].L,
...
uint16_t dci_decoding_procedure_ver(PHY_VARS_UE *phy_vars_ue,
                                DCI_ALLOC_t *dci_alloc,
                                int do_common,
                                int16_t eNB_id,
                                uint8_t subframe)
{
  ...
  PERF_TIME(PERF_UE_dci_decoding_procedure, PERF_START);
                                ...
                                uint8_t subframe)
{
  ...
  uint8_t mi = get_mi(&phy_vars_ue->lte_frame_parms,subframe);
  uint16_t ra_rnti=99;
  uint8_t format0_found=0,format1a_found=0,format_c_found=0;
  uint8_t tmode = phy_vars_ue->transmission_mode[eNB_id];
  uint8_t frame_type = frame_parms->frame_type;
  uint8_t format1A_size_bits=0,format1A_size_bytes=0;
  uint8_t format1C_size_bits=0,format1C_size_bytes=0;
  ...
  if (do_common == 1) 
  {
    ...
    for( uint8_t ucL = 2; ucL <= 3; ucL++)
    {
        // First check common search spaces at aggregation 4 (SI_RNTI and RA_RNTI format 0/1A),
        // and UE_SPEC format0 (PUSCH) too while we're at it
        dci_decoding_procedure1(lte_ue_pdcch_vars,1,frame,subframe,
                                dci_alloc,
                                eNB_id,
                                frame_parms,
                                mi,
  ...
  for( uint8_t ucL = 0; ucL <= 3; ucL++)
  {

      // Now check UE_SPEC format0/1A ue_spec search spaces at aggregation 8
      //  printf("[DCI search] Format 0/1A aggregation 8\n");
      dci_decoding_procedure1(lte_ue_pdcch_vars,do_common,
                              frame,subframe,
                              dci_alloc,
                              eNB_id,
                              frame_parms,
  ...
  for( uint8_t ucL = 0; ucL <= 3; ucL++)
  {
    // Now check UE_SPEC format 1 search spaces at aggregation 1
    old_dci_cnt=dci_cnt;
    dci_decoding_procedure1(lte_ue_pdcch_vars,0,frame,subframe,
                            dci_alloc,
                            eNB_id,
                            frame_parms,
                            mi,
