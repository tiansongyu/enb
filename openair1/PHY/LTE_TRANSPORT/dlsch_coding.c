                   ...
                   time_stats_t *i_stats)
{
  ...
  unsigned char harq_pid = dlsch->current_harq_pid[subframe];
  ...
  if (dlsch->harq_processes[harq_pid]->round == 0 && a != NULL) 
  {  
  ...
  else if((dlsch->harq_processes[harq_pid]->round == 0 && a == NULL))
  {
    printf("dlsch_encoding: ERROR! harq_pid=%d subframe=%d\n",harq_pid,subframe);
  }
