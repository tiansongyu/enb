...
LTE_UE_ULSCH_t *new_ue_ulsch(unsigned char Mdlharq,unsigned char N_RB_UL, uint8_t abstraction_flag)
{
  ...
  //printf("new_ue_ulsch: N_RB_UL=%d abstraction_flag=%d Mdlharq=%d\n",N_RB_UL,abstraction_flag,Mdlharq);
    ...
    for (i=0; i<Mdlharq; i++) {
      ...
      if (ulsch->harq_processes[i]) {
        ...
        if (abstraction_flag==0) {
          ...
          #if 0
          printf("new_ue_ulsch: N_RB_UL=%d abstraction_flag=%d ulsch=%llx harq[%d]=%llx c=%llx c0=%llx c1=%llx c2=%llx\n",
            N_RB_UL,abstraction_flag,
            ulsch,
            i, ulsch->harq_processes[i],
            ulsch->harq_processes[i]->c,
            ulsch->harq_processes[i]->c[0],
            ulsch->harq_processes[i]->c[1],
            ulsch->harq_processes[i]->c[2]
            );
          #endif
        }
        ...
        ulsch->harq_processes[i]->DciNew = 0;
        ulsch->harq_processes[i]->HichNew = 0;
      } else {
                        ...
                        uint8_t Nbundled)
{
  ...
  uint16_t ausRiIdx[120]={0};
  
  LTE_UL_UE_HARQ_t *pHarq=NULL;
  ...
  pHarq = ulsch->harq_processes[harq_pid];
  ...
  if (pHarq->O>0) {
    ...
    LOG_D(PHY,"UE CQI\n");
    print_CQI(pHarq->o,pHarq->uci_format,0,phy_vars_ue->lte_frame_parms.N_RB_DL);

    // save PUSCH pmi for later (transmission modes 4,5,6)
    if (dlsch[0]) {
      //LOG_I(PHY,"XXX saving pmi for DL %x\n",pmi2hex_2Ar1(((wideband_cqi_rank1_2A_5MHz *)ulsch->o)->pmi));
      dlsch[0]->pmi_alloc = ((wideband_cqi_rank1_2A_5MHz *)pHarq->o)->pmi;
    }
  }
  ...
  if (control_only_flag == 0) 
  {
    ...
    for (i=0; i<pHarq->O_RI; i++)
      msg("ulsch_coding: o_RI[%d] %d\n",i,pHarq->o_RI[i]);

    msg("ulsch_coding: O=%d\n",pHarq->O);

    for (i=0; i<1+((8+pHarq->O)/8); i++) {
      //    ulsch->o[i] = i;
      msg("ulsch_coding: O[%d] %d\n",i,pHarq->o[i]);
    }

    if ((tmode != 4))
      print_CQI(pHarq->o,wideband_cqi_rank1_2A,0,phy_vars_ue->lte_frame_parms.N_RB_DL);
    else
      print_CQI(pHarq->o,HLC_subband_cqi_rank1_2A,0,phy_vars_ue->lte_frame_parms.N_RB_DL);
    ...
    {  
      ...
      //printf("ulsch_encoding: phy_vars_ue(%llx)->ulsch_ue[eNB_id=%d]=%llx ulsch->harq_processes[%d]=%llx c=%llx\n",
      //  phy_vars_ue,eNB_id, phy_vars_ue->ulsch_ue[eNB_id],
      //  harq_pid, ulsch->harq_processes[harq_pid],ulsch->harq_processes[harq_pid]->c);
    ...
    if (ulsch->harq_processes[harq_pid]->C == 0) {
      LOG_E(PHY,"null segment harq_pid=%d\n",harq_pid);
      VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_UE_ULSCH_ENCODING, VCD_FUNCTION_OUT);
      return(-1);
    }
    ...
    for (r=0; r<ulsch->harq_processes[harq_pid]->C; r++) 
    {
      ...
      //printf("ulsch_encoding:%d harq_pid=%d r=%d Kr>>3=%d\n", __LINE__, harq_pid, r, Kr>>3);

      if( ulsch->harq_processes[harq_pid]->printf_flag && 0) //xhd_oai_debug UE ulSch enCoding
      {
          print_buff_excel(2,&ulsch->harq_processes[harq_pid]->c[r],Kr>>3,
            "ulsch_encoding: before threegpplte_turbo_encoder r=%d Kr=%d(%d bytes) harq_pid=%d round=%d\n",
            r, Kr,Kr>>3, harq_pid,ulsch->harq_processes[harq_pid]->round);
          print_buff_excel(2,&ulsch->harq_processes[harq_pid]->d[r][96],Kr,
            "ulsch_encoding: after threegpplte_turbo_encoder r=%d Kr=%d(%d bytes) harq_pid=%d round=%d\n",
            r, Kr,Kr>>3, harq_pid,ulsch->harq_processes[harq_pid]->round);
      }
  ...
  { 
    // This is a control-only PUSCH, set sumKr to O_CQI-MIN
  ...
  Qprime = pHarq->O_RI*ulsch->harq_processes[harq_pid]->Msc_initial*ulsch->harq_processes[harq_pid]->Nsymb_initial * ulsch->beta_offset_ri_times8;
  ...
  {
      //printf("ulsch_encoding: Qprime=%d Qprime_RI=%d Q_RI=%d Q_m=%d ulsch->O_RI=%d\n",
      //    Qprime,Qprime_RI,Q_RI,Q_m,pHarq->O_RI);
      //Qprime_RI=0;
      //Q_RI = 0;
  }
  ...
  if (control_only_flag == 0) 
  {
    ...
    if (pHarq->O > 0)
      Qprime = (pHarq->O + L) * ulsch->harq_processes[harq_pid]->Msc_initial*ulsch->harq_processes[harq_pid]->Nsymb_initial * ulsch->beta_offset_cqi_times8;
    else
      Qprime = 0;
    ...
    for (r=0; r<ulsch->harq_processes[harq_pid]->C; r++) {
      ...
      (void)generate_dummy_w(4+Kr,
                                              (uint8_t*) &dummy_w[r][0],
                                              (r==0) ? ulsch->harq_processes[harq_pid]->F : 0);

      start_meas(rm_stats);
      r_offset += lte_rate_matching_turbo_ue(ulsch->harq_processes[harq_pid]->RTC[r],
                                          G,
                                          ulsch->harq_processes[harq_pid]->w[r],
                                          (uint8_t*) &dummy_w[r][0],
                                          ulsch->e+r_offset,
  ...
  else 
  {
    ...
    o_RCC = sub_block_interleaving_cc(8+pHarq->O,
                                      &ulsch->o_d[96],
                                      ulsch->o_w);
  ...
  if (ulsch->harq_processes[harq_pid]->O_ACK == 1) {
    switch (Q_m) {
    case 2:
      ulsch->q_ACK[0] = (pHarq->o_ACK[0]+wACK[wACK_idx][0])&1;
      ulsch->q_ACK[1] = (ulsch->bundling==0)? PUSCH_y : ((pHarq->o_ACK[0]+wACK[wACK_idx][1])&1);//ulsch->o_ACK[0];
      len_ACK = 2;
      break;

    case 4:
      ulsch->q_ACK[0] = (pHarq->o_ACK[0]+wACK[wACK_idx][0])&1;
      ulsch->q_ACK[1] = (ulsch->bundling==0)? PUSCH_y : ((pHarq->o_ACK[0]+wACK[wACK_idx][1])&1);
  ...
  // two-bit ACK/NAK
  if (ulsch->harq_processes[harq_pid]->O_ACK == 2) {
    ack_parity = (pHarq->o_ACK[0]+pHarq->o_ACK[1])&1;

    switch (Q_m) {
    case 2:
      ulsch->q_ACK[0] = (pHarq->o_ACK[0]+wACK[wACK_idx][0])&1;
      ulsch->q_ACK[1] = (pHarq->o_ACK[1]+wACK[wACK_idx][0])&1;
      ulsch->q_ACK[2] = (ack_parity+wACK[wACK_idx][0])&1;
      ulsch->q_ACK[3] = (pHarq->o_ACK[0]+wACK[wACK_idx][1])&1;
      ulsch->q_ACK[4] = (pHarq->o_ACK[1]+wACK[wACK_idx][1])&1;
    ...
    case 4:
      ...
      ulsch->q_ACK[5]  = (pHarq->o_ACK[0]+wACK[wACK_idx][1])&1;
      ulsch->q_ACK[6]  = PUSCH_x;
      ulsch->q_ACK[7]  = PUSCH_x;//1;
      ulsch->q_ACK[8]  = (pHarq->o_ACK[1]+wACK[wACK_idx][1])&1;
    ...
    case 6:
      ...
      ulsch->q_ACK[7] = (pHarq->o_ACK[0]+wACK[wACK_idx][1])&1;
      ulsch->q_ACK[8] = PUSCH_x;
      ulsch->q_ACK[9] = PUSCH_x;
      ulsch->q_ACK[10] = PUSCH_x;
      ulsch->q_ACK[11] = PUSCH_x;

      ulsch->q_ACK[12] = (pHarq->o_ACK[1]+wACK[wACK_idx][1])&1;
  ...
  if( ulsch->harq_processes[harq_pid]->O_ACK == 1)
  {
      print_info("ACK_HANDLE:  ulsch_encoding: O_ACK=%d ACK=%d %d q_ACK[0]=%d %d ulsch->bundling=%d wACK_idx=%d Nbundled=%d\n",
        ulsch->harq_processes[harq_pid]->O_ACK,
        pHarq->o_ACK[0],pHarq->o_ACK[1],
        ulsch->q_ACK[0],ulsch->q_ACK[1],
        ulsch->bundling,wACK_idx,Nbundled);
  }
  ...
  start_meas(m_stats);
  Hpp = Hprime + Qprime_RI; //Q_RI; //xhd_oai_debug
  ...
  //  printf("Qprime_CQI = %d\n",Qprime_CQI);
  // RI BITS
  if( Qprime_RI>0 &&(len_RI == 0 || len_RI > MAX_RI_PAYLOAD) )
  {
      printf("ulsch_encoding: FATAL error AAAA len_RI=%d Qprime_RI=%d Q_RI=%d Q_m=%d ulsch->O_RI=%d Q_m(%d)*Hpp(%d)=%d\n",
        len_RI,Qprime_RI,Q_RI,Q_m,pHarq->O_RI,Q_m,Hpp,Q_m*Hpp);
      Qprime_RI=0;
  }
  ...
  if( Qprime_RI>0 &&(len_RI == 0 || len_RI > MAX_RI_PAYLOAD) )
  {
      printf("ulsch_encoding: FATAL error BBBB len_RI=%d Qprime_RI=%d Q_RI=%d Q_m=%d ulsch->O_RI=%d Q_m(%d)*Hpp(%d)=%d\n",
        len_RI,Qprime_RI,Q_RI,Q_m,pHarq->O_RI,Q_m,Hpp,Q_m*Hpp);
      Qprime_RI=0;
  }
  ...
  for (i=0; i<Qprime_RI; i++) 
  {
    ...
    for (q=0; q<Q_m; q++)  
    {
      int iIdx = q+(Q_m*((r*Cmux) + columnset[j]));
      
      if( iIdx> 6*14*1200 )
      {
          printf("ulsch_encoding: FATAL error len_RI=%d\n",len_RI);
          return(-1);
      }

      y[iIdx]  = ulsch->q_RI[(q+(Q_m*i))%len_RI];
      ausRiIdx[(q+(Q_m*i))]=iIdx; //xhd_oai_debug
  ...
  if( ulsch->harq_processes[harq_pid]->printf_flag ) //xhd_oai_debug UE ulSch encoding RI
  {
        print_info("ulsch_encoding: q_RI:len=%d[%d %d]at[%d %d %d %d] Rmux_prime=%d Cmux=%d Q_m=%d len_RI=%d Hpp=%d Hprime=%d Q_RI=%d O_RI=%d o_RI[0]=%d\n",
            Qprime_RI,ulsch->q_RI[0],ulsch->q_RI[1],ausRiIdx[0],ausRiIdx[1],ausRiIdx[2],ausRiIdx[3],
            Rmux_prime,Cmux,Q_m,len_RI,Hpp,Hprime,Q_RI,
            pHarq->O_RI,pHarq->o_RI[0]);
  }
  ...
  */
  ...
  if( ulsch->harq_processes[harq_pid]->O_ACK && 0) //xhd_oai_debug UE ulSch enCoding CQI
  {
        print_buff(ulsch->q,Qprime_CQI*Q_m,0,"ulsch_encoding: q:len=%d Qprime_CQI=%d Q_m=%d j=%d pHarq->O=%d o=%02x%02x%02x%02x %02x%02x%02x%02x\n",
            MAX_CQI_PAYLOAD,Qprime_CQI,Q_m,j, pHarq->O,
            pHarq->o[0],
            pHarq->o[1],
            pHarq->o[2],
            pHarq->o[3],
  ...
  if( ulsch->harq_processes[harq_pid]->printf_flag && 0) //xhd_oai_debug UE ulSch enCoding ACK
  {
        print_buff(ulsch->q_ACK,MAX_ACK_PAYLOAD,0,"ulsch_encoding: q_Ack:len=%d Qprime_ACK=%d Q_m=%d O_ACK=%d o_ACK[0]=%d \n",
            MAX_ACK_PAYLOAD,Qprime_ACK,Q_m,
            pHarq->O_ACK,pHarq->o_ACK[0]);
        print_buff(y,Cmux*Rmux_prime*Q_m,0,"ulsch_encoding: Y before Add Ack:Cmux=%d Rmux_prime=%d Q_m=%d\n",Cmux,Rmux_prime,Q_m);
  }

  j=0;

  for (i=0; i<Qprime_ACK && len_ACK > 0; i++) //xhd_oai_debug
  {
    r = Rmux_prime - 1 - (i>>2);
...
#endif
         print_info("ACK %d => %d (%d,%d,%d)i=%d/%d Q_m=%d Cmux=%d Rmux_prime=%d len_ACK=%d\n",
                (q+(Q_m*i))%len_ACK,
                ulsch->q_ACK[(q+(Q_m*i))%len_ACK],
                q+(Q_m*((r*Cmux) + columnset[j])),
                r,columnset[j],
                i,Qprime_ACK,
                Q_m,Cmux,Rmux_prime,len_ACK);
  ...
  if( ulsch->harq_processes[harq_pid]->O_ACK ) //xhd_oai_debug UE ulSch after encoding ACK
  {
        print_buff_excel(5,y,Cmux*Rmux_prime*Q_m,"ulsch_encoding: Y after Add Ack:Cmux=%d Rmux_prime=%d Q_m=%d \n",
            Cmux,Rmux_prime,Q_m);
  }
