...
void generate_RIV_tables()
{
  ...
  printf("==================generate_RIV_tables: RIV_max50=%d====================\n",RIV_max50);
...
uint32_t get_rballoc50(vrb_t vrb_type,uint16_t rb_alloc_dci)
{

  return(localRIV2alloc_LUT50_0[rb_alloc_dci]);
...
uint8_t get_transmission_mode(module_id_t Mod_id, uint8_t CC_id, rnti_t rnti)
{
  ...
  if( UE_id < 0 )
  {
      printf("get_transmission_mode, rnti not exist:%x\n",rnti);
      return 255;
  }
                                       ...
                                       int frame0)
{
  ...
  case format1A:  // This is DLSCH allocation for control traffic
    ...
    case 25:
      ...
      //xhd_oai_ue FORMAT 1A一般用于SI，可以不用打印
      /*
        print_info("GEN DL Format 1A:  Frame %d subframe %d   vrb_type %d  mcs=%d rb_alloc=%d  rv=%d ndi=%d TPC %d harq_pid=%d RIV_max=%d\n",
              frame0,subframe,vrb_type,mcs,rballoc,rv,ndi,TPC,harq_pid,RIV_max);
       */
      break;
    ...
    else 
    {
      ...
      if (rballoc>RIV_max) {
        LOG_E(PHY,"ERROR: Format 1A: vrb_type=%d harq_pid=%d mcs=%d rv=%d rb_alloc (%x) > RIV_max (%x)\n",
            vrb_type, harq_pid, mcs, rv, rballoc,RIV_max);
        return(-1);
      }
    ...
    if (NPRB==0)
    {
        LOG_E(PHY,"ERROR: Format 1A: NPRB==0\n");
        return(-1);
    }
    ...
    */
    ...
    dlsch[0]->current_harq_pid[subframe]   = harq_pid;
    dlsch[0]->harq_ids[subframe] = harq_pid;

    dlsch[0]->active[subframe] = 1;
    dlsch0 = dlsch[0];
  ...
  case format1:
    ...
    case 25:
      ...
      //if( (subframe == 0) && (frame&0xf)== 8) //xhd_oai_debug eNB gen DL DCI
      {
          print_info("GEN DL Format 1:  Frame %d subframe %d   rah %d  mcs=%d rb_alloc=%d  rv=%d ndi=%d TPC %d harq_pid=%d RIV_max=%d\n",
                  frame0,subframe,rah,mcs,rballoc,rv,ndi,TPC,harq_pid,RIV_max);
      }
      break;
    ...
    if (NPRB==0)
    {
        LOG_E(PHY, "ERROR: Format 1: NPRB==0 frame=%d subframe=%d rballoc=%d\n ", 
                    frame,subframe,rballoc);
        return(-1);
    }
    ...
    */
    ...
    print_info("GEN DL Format 1:  Frame %d subframe %d harq %d round %d rv=%d ndi=%d/%d rah %d  mcs=%d rb_alloc=%d   TPC %d  RIV_max=%d\n",
            frame0,subframe,harq_pid,dlsch0_harq->round,rv,ndi,dlsch0_harq->ndi,rah,mcs,rballoc,TPC,RIV_max);
  ...
  case format2:
    ...
    dlsch0->current_harq_pid[subframe] = harq_pid;
    dlsch1->current_harq_pid[subframe] = harq_pid;
    ...
    {
        LOG_E(PHY,"ERROR: Format 2: NPRB==0\n");
      return(-1);
    }
    ...
    if (frame_parms->nb_antennas_tx == 2) {
      if (dlsch1->active[subframe] == 1) { // both TBs are active
  ...
  case format2A:
    ...
    if (dlsch0_harq->nb_rb == 0)
    {
        LOG_E(PHY,"ERROR: Format 2A: NPRB==0\n");
      return(-1);
    }
  ...
  case format2C:
    ...
    if (dlsch0_harq->nb_rb == 0)
    {
        LOG_E(PHY,"ERROR: Format 2C: NPRB==0\n");
      return(-1);
    }
    ...
    if ((dlsch0_harq->round == 0) && (dlsch0->active[subframe] == 1) ) {
      dlsch0_harq->status      = ACTIVE;
      dlsch0_harq->mcs         = mcs1;
    }

    if ((dlsch1_harq->round == 0) && (dlsch1->active[subframe] == 1) ) {
      dlsch1_harq->status      = ACTIVE;
      dlsch1_harq->mcs         = mcs2;
    }
                                      ...
                                      uint16_t p_rnti)
{
  ...
  case format1A:
    ...
    case 6:
      ...
      if ((rnti==si_rnti) || (rnti==p_rnti) || (rnti==ra_rnti)) { //
        ...
        if (harq_pid>1) {
          LOG_E(PHY,"Format 1A: harq_pid(%d) > 1  mcs=%d rballoc=%d rv=%d ndi=%d\n",harq_pid,mcs,rballoc,rv,ndi);
          return(-1);
        }
    ...
    case 25:
      ...
      if ((rnti==si_rnti) || (rnti==p_rnti) || (rnti==ra_rnti)) { //
        ...
        if( frame & 0xff == 0 )
        {
            printf("==>generate_ue_dlsch_params_from_dci: frame=%d subframe=%d mcs=%d TPC=%d rballoc=%d NPRB=%d/%d %s\n",
                frame,subframe,mcs,TPC,rballoc,NPRB,RIV2nb_rb_LUT25[rballoc],
                ((rnti==si_rnti)?"SI":"RA"));
        }
        ...
        //xhd_oai_ue
        //if (harq_pid>1) {
        if (((harq_pid>7)&&(frame_type == TDD))||((harq_pid>7)&&(frame_type == FDD))) {
            LOG_E(PHY,"Frame %d subframe %d Format 1A: harq_pid(%d) > 1  mcs=%d rballoc=%d rv=%d ndi=%d\n",
                frame,subframe,harq_pid,mcs,rballoc,rv,ndi);
          return(-1);
        }
        ...
        dlsch0_harq->flag = 98;
        printf("==>generate_ue_dlsch_params_from_dci: frame=%d subframe=%d TPC=%d NPRB=%d/%d rballoc=%d vrb_type=%d NORMAL\n",
            frame,subframe,TPC,NPRB,RIV2nb_rb_LUT25[rballoc],rballoc,vrb_type);

      }
      dlsch0_harq->NPRB = NPRB;
      dlsch0_harq->TPC = TPC;
      dlsch0_harq->rballoc = rballoc;
      dlsch0_harq->vrb_type2    = vrb_type;
    ...
    if (rballoc>RIV_max) 
    {
      ...
      LOG_E(PHY,"Frame %d subframe %d Format 1A: rb_alloc(%d) > RIV_max(%d) harq_pid(%d)  mcs=%d rv=%d ndi=%d\n",
        frame,subframe,rballoc,RIV_max,harq_pid,mcs,rv,ndi);
      ...
      if( frame_parms->N_RB_DL == 25)
      {
          ...
          LOG_E(PHY,"Frame %d subframe %d Format 1A:  vrb_type %d  mcs=%d rb_alloc=%d  rv=%d ndi=%d TPC %d harq_pid=%d \n",
              frame,subframe,vrb_type,mcs,rballoc,rv,ndi,TPC,harq_pid);
    ...
    LOG_D(PHY,"====>UE DCI Format1A (rnti %x/harq %d): Frame%d Subframe %d : ndi %d, old_ndi %d (first tx %d) harq_status %d, round %d nb_rb %d %d%d%d%d-%d%d%d%d mcs%d\n",
	  dlsch[0]->rnti,
	  harq_pid,
	  frame,subframe,
	  ndi,
	  ...
	  dlsch0_harq->rb_alloc_even[0],
        ...
        dlsch0_harq->rb_alloc_odd[0],
        dlsch0_harq->rb_alloc_odd[1],
        dlsch0_harq->rb_alloc_odd[2],
        dlsch0_harq->rb_alloc_odd[3],
        mcs
	  );
    ...
    dlsch0_harq->TBS2 = dlsch0_harq->TBS;
  ...
  case format1:
    ...
    if (harq_pid>=8) {
      LOG_E(PHY,"Frame %d subframe %d Format 1: harq_pid(%d) >= 8\n",frame,subframe,harq_pid);
      if( frame_parms->N_RB_DL == 25)
      {
          ...
          LOG_E(PHY,"Frame %d subframe %d Format 1: mcs=%d  rb_alloc=%d  rah %drv=%d ndi=%d TPC %d harq_pid=%d \n",
              frame,subframe,mcs,rballoc,rah,rv,ndi,TPC,harq_pid);
    ...
    //    printf("NPRB %d\n",NPRB);
    dlsch0_harq->delta_PUCCH     = delta_PUCCH_lut[TPC&3];
    if( delta_PUCCH_lut[TPC&3] < 0 )
    {
        dlsch[0]->g_pucch += delta_PUCCH_lut[TPC&3];
        if( dlsch[0]->g_pucch > 0)
        {
            dlsch[0]->g_pucch = -127;
        }
    }
    else if( delta_PUCCH_lut[TPC&3] > 0 )
    {
        dlsch[0]->g_pucch += delta_PUCCH_lut[TPC&3];
        if( dlsch[0]->g_pucch < 0)
        {
    ...
    if (TPC!=1)
    {
      print_info("format1 TPC %d, dlsch0_harq->delta_PUCCH %d \n",TPC,dlsch0_harq->delta_PUCCH);
    }

    dlsch0_harq->rvidx     = rv;
    dlsch0_harq->rballoc = rballoc; //xhd_oai_debug
    ...
    //    dlsch0_harq->first_tx);
    if( dlsch[0]->active[subframe])
    {
        print_info("@@@@@@generate_ue_dlsch_params_from_dci: format1, dlsch of subframe %d is already active!\n",subframe);
    }
    print_info("GEN DL Format 1:  Frame %d subframe %d harq %d round %d rv=%d ndi=%d/%d rah %d  mcs=%d rb_alloc=%d   TPC %d  RIV_max=%d status:%d\n",
            frame,subframe,harq_pid,dlsch0_harq->round,rv,ndi,dlsch0_harq->DCINdi,
            rah,mcs,rballoc,TPC,RIV_max,dlsch0_harq->status);
    ...
    else if (dlsch0_harq->status == SCH_IDLE) 
    { 
      ...
      if( dlsch[0]->active[subframe])
      {
          //print_info("@@@@@@generate_ue_dlsch_params_from_dci: format1, dlsch of subframe %d is cleared! AAA\n",subframe);
      }
      print_info("@@@@@@generate_ue_dlsch_params_from_dci: format1, dlsch of frame %d subframe %d is retrans\n",
        frame,subframe);
    ...
    if( dlsch0_harq->ulLastFrame + 1 < frame ) //xhd_oai_debug
    {
        dlsch0_harq->round=0;
    }
    dlsch0_harq->ulLastFrame = frame;
    ...
    print_info("GEN DL Format 1:  Frame %d subframe %d   rah %d  mcs=%d rb_alloc=%d  rv=%d ndi=%d TPC %d harq_pid=%d RIV_max=%d NPRB=%d TBS=%d/%d\n",
            frame,subframe,rah,mcs,rballoc,rv,ndi,TPC,harq_pid,RIV_max,NPRB,dlsch0_harq->TBS,dlsch0_harq->TBS/8);
  ...
  case format2A:
    ...
    dlsch0->active[subframe] = dlsch1->active[subframe] = 1;
...
void fill_CQI(LTE_UE_ULSCH_t *ulsch,PHY_MEASUREMENTS *meas,uint8_t eNB_id,uint8_t harq_pid,int N_RB_DL,uint16_t rnti, uint8_t trans_mode, double sinr_eff)
{
  ...
  double sinr_tmp;
  uint8_t *o = ulsch->harq_processes[harq_pid]->o; //ulsch->o;
  UCI_format_t uci_format = ulsch->harq_processes[harq_pid]->uci_format; //ulsch->uci_format;

  if(flag_LA==1)
  {
    sinr_tmp = sinr_eff;
    my_printf("fill_CQI:AAA sinr_tmp=%d uci_format=%d cqi=%d-%d\n",
        sinr_tmp,uci_format,
        sinr2cqi(sinr_tmp,trans_mode),
        fill_subband_cqi(meas,eNB_id,trans_mode,6));
  }
  else
  {
    sinr_tmp = (double) meas->wideband_cqi_avg[eNB_id];
    my_printf("fill_CQI:BBB sinr_tmp=%f uci_format=%d cqi=%d-%d trans_mode=%d\n",
        sinr_tmp,uci_format,
        sinr2cqi(sinr_tmp,trans_mode),
        fill_subband_cqi(meas,eNB_id,trans_mode,6),
        trans_mode);
...
void setUlAckInfo(int frame_rx,
                      uint8_t subframe,
                      PHY_VARS_UE *phy_vars_ue)
{
    ...
    int ULFrame = pdcch_alloc2ul_frame(frame_parms,frame_rx,subframe);
    uint8_t ULSubframe =  pdcch_alloc2ul_subframe(frame_parms,subframe);
    ...
    pHarq = ulsch->harq_processes[harq_pid];
        ...
        pHarq->O_ACK = 1;
        print_info("@@@ Need to send ack info for DL frame %d subframe %d at UL frame %d subframe %d harq %d status=%d\n", 
            frame_rx, subframe,
            ULFrame,ULSubframe,harq_pid,pHarq->status);
    } 

}
void ClearUlAckInfo(int frame_rx,
                      uint8_t subframe,
                      PHY_VARS_UE *phy_vars_ue)
{
    LTE_DL_FRAME_PARMS *frame_parms = &phy_vars_ue->lte_frame_parms;
    ...
    harq_pid = subframe2harq_pid(frame_parms,
                                 pdcch_alloc2ul_frame(frame_parms,frame_rx,subframe),
    ...
    pHarq = ulsch->harq_processes[harq_pid];
        ...
        // we have downlink transmission
        pHarq->O_ACK = 0;
        //print_info("@@@ No need to send ack info for DL frame %d subframe %d\n", frame_rx,subframe);
    } 
                                      ...
                                      uint8_t use_srs)
{
    ...
    uint16_t nb_rb;
    LTE_UL_UE_HARQ_t *pHarq=NULL;
    uint8_t subframe_ul = pdcch_alloc2ul_subframe(frame_parms,subframe);//xhd_oai_debug
    int frame_ul = pdcch_alloc2ul_frame(frame_parms,frame_rx,subframe);

    PERF_TIME(PERF_UE_generate_ue_ulsch_params_from_dci, PERF_START);
    ...
    case 6:
      ...
      RIV_max = RIV_max6;
      first_rb                              = RIV2first_rb_LUT6[rballoc];
      nb_rb                                 = RIV2nb_rb_LUT6[rballoc];
    ...
    case 25:
      ...
      RIV_max = RIV_max25;
      first_rb                              = RIV2first_rb_LUT25[rballoc];
      nb_rb                                 = RIV2nb_rb_LUT25[rballoc];
    ...
    case 50:
      ...
      RIV_max = RIV_max50;
      first_rb = RIV2first_rb_LUT50[rballoc];
      nb_rb    = RIV2nb_rb_LUT50[rballoc];
    ...
    case 100:
      ...
      RIV_max = RIV_max100;
      first_rb                              = RIV2first_rb_LUT100[rballoc];
      nb_rb                                 = RIV2nb_rb_LUT100[rballoc];
    ...
    if (rballoc > RIV_max) {
      LOG_E(PHY,"frame %d, subframe %d, rnti %x, format %d: FATAL ERROR: rb_alloc(%d) > RIV_max(%d) N_RB_DL=%d\n",
            phy_vars_ue->frame_rx, subframe, rnti, dci_format,
            rballoc,RIV_max,
            frame_parms->N_RB_DL);
      return(-1);
    }
    PERF_TIME_LINE(PERF_UE_generate_ue_ulsch_params_from_dci, PERF_LINE);
    PERF_TIME_LINE(PERF_UE_phy_procedures_UE_RX, PERF_LINE);


    // indicate that this process is to be serviced in subframe n+4
    if ((rnti >= cba_rnti) && (rnti < p_rnti))
      pHarq->subframe_cba_scheduling_flag = 1; //+=1 this indicates the number of dci / cba group: not supported in the data struct
    else
    {
        ;
    ...
    if (phy_vars_ue->ul_power_control_dedicated[eNB_id].accumulationEnabled == 1) {
      LOG_D(PHY,"[UE %d][PUSCH %d] Frame %d subframe %d: f_pusch (ACC) %d, adjusting by %d (TPC %d)\n",
            ...
            delta_PUSCH_acc[pHarq->TPC],
            pHarq->TPC);
      ulsch->f_pusch += delta_PUSCH_acc[pHarq->TPC];
    } else {
      LOG_D(PHY,"[UE %d][PUSCH %d] Frame %d subframe %d: f_pusch (ABS) %d, adjusting to %d (TPC %d)\n",
            phy_vars_ue->Mod_id,harq_pid,phy_vars_ue->frame_rx,subframe,ulsch->f_pusch,
            delta_PUSCH_abs[pHarq->TPC],
            pHarq->TPC);
      ulsch->f_pusch = delta_PUSCH_abs[pHarq->TPC];
    }
    ...
    if (pHarq->first_tx==1) {
      ...
      pHarq->first_tx=0;
      pHarq->round = 0;
      //ulsch->harq_processes[harq_pid]->roundDci = 0;
      ...
      //if( harq_pid == 4 ) //(subframe == 0) && (frame_rx&0xf)== 0) //xhd_oai_debug UE get UL DCI
      {
          print_info("===> generate_ue_ulsch_params_from_dci: rnti=%x frame=%d subframe=%d first tx harq_pid=%d first_rb=%d-%d nb_rb=%d-%d mcs=%d-%d TBS=%d-%d ndi=%d\n",
            ulsch->rnti,
            frame_rx,subframe,harq_pid,
            pHarq->first_rb, first_rb,
            pHarq->nb_rb, nb_rb,
            pHarq->mcs,mcs,
            pHarq->TBS>>3,TBS>>3,ndi
            );
      }
      ulsch->harq_processes[harq_pid]->rvidx = 0;//rv_table[0];

    }
    else if (pHarq->DCINdi!=ndi) 
    { 
        // new SDU opportunity
        ...
        //if( harq_pid == 4 ) //( (subframe == 0) && (frame_rx&0xf)== 0) //xhd_oai_debug UE get UL DCI
        {
          print_info("===> generate_ue_ulsch_params_from_dci: rnti=%x frame=%d subframe=%d new SDU opportunity harq_pid=%d first_rb=%d-%d nb_rb=%d-%d mcs=%d-%d TBS=%d-%d cqi_req=%d ndi=%d\n",
            ulsch->rnti,
            frame_rx,subframe,harq_pid,
            pHarq->first_rb, first_rb,
            pHarq->nb_rb, nb_rb,
    ...
    else 
    {
        ...
        //if( harq_pid == 4 ) //( (subframe == 0) && (frame_rx&0xf)== 0) //xhd_oai_debug UE get UL DCI
        {
            print_info("===> generate_ue_ulsch_params_from_dci: rnti=%x frame=%d subframe=%d old SDU opportunity harq_pid=%d round=%d first_rb=%d-%d nb_rb=%d-%d mcs=%d-%d TBS=%d-%d cqi_req=%d ndi=%d\n",
                ulsch->rnti,
                frame_rx,subframe,harq_pid,pHarq->round,
                pHarq->first_rb, first_rb,
                pHarq->nb_rb, nb_rb,
                pHarq->mcs,mcs,
                pHarq->TBS>>3,TBS>>3,
    ...
    //xhd_oai_ue
    //UE_UlDci(subframe, ulsch->harq_processes[harq_pid]->roundDci);
    ...
    */
    ...
    if ((pHarq->nb_rb>0) && (pHarq->nb_rb < 25))
      ulsch->power_offset = ue_power_offsets[pHarq->nb_rb-1];
    ...
    if (cqi_req == 1) 
    {
      pHarq->O_RI = 1; //we only support 2 antenna ports, so this is always 1 according to 3GPP 36.213 Table
      ...
      case 1:
        if ((rnti >= cba_rnti) && (rnti < p_rnti)) {
          ...
          case 6:
            pHarq->O                                   = sizeof_HLC_subband_cqi_mcs_CBA_1_5MHz;
            break;

          case 25:
            pHarq->O                                   = sizeof_HLC_subband_cqi_mcs_CBA_5MHz;
            break;

          case 50:
            pHarq->O                                   = sizeof_HLC_subband_cqi_mcs_CBA_10MHz;
            break;

          case 100:
            pHarq->O                                   = sizeof_HLC_subband_cqi_mcs_CBA_20MHz;
            break;
          }

          pHarq->uci_format                          = HLC_subband_cqi_mcs_CBA;
          pHarq->o_RI[0]                             = 0;
        } else  if(meas->rank[eNB_id] == 0) {
          switch (phy_vars_ue->lte_frame_parms.N_RB_DL) {
          case 6:
            pHarq->O                                   = sizeof_HLC_subband_cqi_nopmi_1_5MHz;
            break;

          case 25:
            pHarq->O                                   = sizeof_HLC_subband_cqi_nopmi_5MHz;
            break;

          case 50:
            pHarq->O                                   = sizeof_HLC_subband_cqi_nopmi_10MHz;
            break;

          case 100:
            pHarq->O                                   = sizeof_HLC_subband_cqi_nopmi_20MHz;
            break;
          }

          pHarq->uci_format                          = HLC_subband_cqi_nopmi;
          pHarq->o_RI[0]                             = 0;
        } else {
          switch (phy_vars_ue->lte_frame_parms.N_RB_DL) {
          case 6:
      ...
      case 4:
        if ((rnti >= cba_rnti) && (rnti < p_rnti)) {
          ...
          switch (phy_vars_ue->lte_frame_parms.N_RB_DL) {
          case 6:
            pHarq->O                                   = sizeof_wideband_cqi_rank1_2A_1_5MHz;
            break;

          case 25:
            pHarq->O                                   = sizeof_wideband_cqi_rank1_2A_5MHz;
            break;

          case 50:
            pHarq->O                                   = sizeof_wideband_cqi_rank1_2A_10MHz;
            break;

          case 100:
            pHarq->O                                   = sizeof_wideband_cqi_rank1_2A_20MHz;
            break;
          }

          pHarq->uci_format                          = wideband_cqi_rank1_2A;
          pHarq->o_RI[0]                             = 0;
        } else {
          switch (phy_vars_ue->lte_frame_parms.N_RB_DL) {
          case 6:
            pHarq->O                                   = sizeof_wideband_cqi_rank2_2A_1_5MHz;
            break;

          case 25:
            pHarq->O                                   = sizeof_wideband_cqi_rank2_2A_5MHz;
            break;

          case 50:
            pHarq->O                                   = sizeof_wideband_cqi_rank2_2A_10MHz;
            break;

          case 100:
            pHarq->O                                   = sizeof_wideband_cqi_rank2_2A_20MHz;
            break;
          }

          pHarq->uci_format                          = wideband_cqi_rank2_2A;
          pHarq->o_RI[0]                             = 1;
        }
    ...
    print_CQI(pHarq->o,pHarq->uci_format,eNB_id,phy_vars_ue->lte_frame_parms.N_RB_DL);
    ...
    */
    ...
    if (frame_parms->frame_type == FDD) {
      ...
      int dl_subframe = (subframe_ul<4) ? (subframe_ul+6) : (subframe_ul-4); //xhd_oai_debug

      if (phy_vars_ue->dlsch_ue[eNB_id][0]->harq_ack[dl_subframe].send_harq_status>0) 
      { 
        // we have downlink transmission
        pHarq->O_ACK = 1;
        //print_info("@@@ AAA Need to send ack info for DL frame %d subframe %d at UL frame %d subframe %d harq %d status=%d\n", 
        //    frame_rx,dl_subframe,
        //    frame_ul,subframe_ul, harq_pid, pHarq->status);
      } 
      else 
      {
        pHarq->O_ACK = 0;
        //print_info("@@@ AAA No need to send ack info for DL frame %d subframe %d at UL frame %d subframe %d harq %d\n", 
        //    frame_rx,dl_subframe,
        //    frame_ul,subframe_ul, harq_pid);
      }
    } else {
      if (ulsch->bundling)
        pHarq->O_ACK = (dai == 3)? 0 : 1;
      else
        pHarq->O_ACK = (dai+1)&3;

      //      ulsch->harq_processes[harq_pid]->V_UL_DAI = dai+1;
    }
    //pHarq->O_ACK = 0;//xhd_oai_debug
    //pHarq->dai = dai;
    ...
    if (pHarq->round == 0) {
      ...
      if (pHarq->mcs < 29)
        pHarq->TBS         = TBStable[get_I_TBS_UL(pHarq->mcs)][pHarq->nb_rb-1];
      else
      ...
      pHarq->Msc_initial   = 12*pHarq->nb_rb;
      pHarq->Nsymb_initial = ulsch->Nsymb_pusch;
      pHarq->round = 0;
    ...
    LOG_D(PHY,"[UE %d][PUSCH %d] Frame %d, subframe %d : Programming PUSCH with n_DMRS2 %d (cshift %d), nb_rb %d, first_rb %d, mcs %d, round %d, rv %d\n",
          phy_vars_ue->Mod_id,harq_pid,phy_vars_ue->frame_rx,subframe,pHarq->n_DMRS2,cshift,pHarq->nb_rb,pHarq->first_rb,
          pHarq->mcs,pHarq->round,pHarq->rvidx);
    ...
    UNUSED_VARIABLE(dai);
#endif
    PERF_TIME(PERF_UE_generate_ue_ulsch_params_from_dci, PERF_ONCE);
    PERF_TIME_LINE(PERF_UE_phy_procedures_UE_RX, PERF_LINE);
                                       ...
                                       uint8_t *pucRb_flag)
{
  ...
  if (dci_format == format0) {
    ...
    if (rb_alloc>RIV_max) {
      LOG_E(PHY,"Format 0: UE_id:%x rnti=%x rb_alloc(%d-%d) > RIV_max(%d-%d) first_rb:%d-%d nb_rb:%d-%d round=%d harq_pid=%d\n", 
        UE_id, rnti, rb_alloc, 
        ulsch->harq_processes[harq_pid]->rballoc_pre,
        mac_xface->computeRIV(frame_parms->N_RB_UL,
                              ulsch->harq_processes[harq_pid]->first_rb_pre,
                              ulsch->harq_processes[harq_pid]->nb_rb_pre),
        RIV_max,

        ulsch->harq_processes[harq_pid]->first_rb,ulsch->harq_processes[harq_pid]->first_rb_pre,
        ulsch->harq_processes[harq_pid]->nb_rb, ulsch->harq_processes[harq_pid]->nb_rb_pre,
        ulsch->harq_processes[harq_pid]->roundx,
        harq_pid);
    ...
    if(ulsch->harq_processes[harq_pid]->first_rb !=  ulsch->harq_processes[harq_pid]->first_rb_pre
        || ulsch->harq_processes[harq_pid]->nb_rb != ulsch->harq_processes[harq_pid]->nb_rb_pre
        || rballoc != ulsch->harq_processes[harq_pid]->rballoc_pre)
    {
        printf("generate_eNB_ulsch_params_from_dci: FATAL ERROR, RB MISMATCH: UE_id=%d rnti%x harq_pid=%d rballoc%d-%d first_rb%d-%d nb_rb%d-%d round=%d!\n", 
            UE_id,rnti,harq_pid,
            rballoc,ulsch->harq_processes[harq_pid]->rballoc_pre,
            ulsch->harq_processes[harq_pid]->first_rb, ulsch->harq_processes[harq_pid]->first_rb_pre,
            ulsch->harq_processes[harq_pid]->nb_rb, ulsch->harq_processes[harq_pid]->nb_rb_pre,
    ...
    //xhd_oai_rb
    //if( harq_pid == 4) //(subframe == 0) && (frame&0xf)== 0) //xhd_oai_debug eNB gen UL DCI
    {
        print_info("====>[eNB][UE %2d RNTI=%x]GEN UL DCI :frame=%3d    subframe=%d   harq_pid=%d   round=%d   first_rb=%2d nb_rb=%2d mcs=%2d ndi=%d generate_eNB_ulsch_params_from_dci:first_rb=%d nb_rb=%d mcs=%d cqi_req=%d\n",
            UE_id,ulsch->rnti,
    ...
    int kk = ulsch->harq_processes[harq_pid]->first_rb;
    int jj;
    for(jj=0; jj < ulsch->harq_processes[harq_pid]->nb_rb; jj++)
    {
        if(kk+jj>100)break;
        if(pucRb_flag[kk+jj] == 1)
        {
            printf("generate_eNB_ulsch_params_from_dci:fatal error UE_id=%d harq_pid=%d ",UE_id,harq_pid);
            for(uint8_t kk = 0; kk < NUMBER_OF_UE_MAX; kk++)
            {
                if ((phy_vars_eNB->ulsch_eNB[kk]==0)||
                    (phy_vars_eNB->ulsch_eNB[kk]->rnti==0))
                {
                    continue;
                }

                LTE_UL_eNB_HARQ_t *pHarq = phy_vars_eNB->ulsch_eNB[kk]->harq_processes[harq_pid];
                
                printf("[%d,%d",kk,
                    pHarq->first_rb);

                if( pHarq->first_rb != pHarq->first_rb_pre )
                {
                    printf("-%d",pHarq->first_rb_pre);
                }
    ...
    else 
    {
      if (mcs>28)
        ...
        //xhd_oai_ue
        ulsch->harq_processes[harq_pid]->rvidx = rv_table[ulsch->harq_processes[harq_pid]->round&3];//0;
        ulsch->harq_processes[harq_pid]->mcs = mcs;
      }
    ...
    #if 0
    if(ulsch->harq_processes[harq_pid]->mcs != 10)
    {
        ulsch->harq_processes[harq_pid]->mcs = 10;
        
        printf("generate_eNB_ulsch_params_from_dci:frame=%d subframe=%d error mcs=%d-%d round=%d harq_pid=%d rnti=%x ueid=%d\n",
            frame,subframe,
            ulsch->harq_processes[harq_pid]->mcs, mcs, ulsch->harq_processes[harq_pid]->round,harq_pid,
            rnti, UE_id);
