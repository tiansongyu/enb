...
typedef struct {
  ...
  //xhd_oai_multuser
  /// First Allocated RB
  uint16_t first_rb_pre;
  /// Current Number of RBs
  uint16_t nb_rb_pre;
  uint32_t rballoc_pre;
  frame_t       frameP;
  ...
  int8_t wc[MAX_NUM_ULSCH_SEGMENTS][3*(6144+64)];
  ...
  uint8_t log2_maxh; //xhd_oai_debug
  uint8_t ucWidth; //xhd_oai_debug
} LTE_UL_eNB_HARQ_t;
...
typedef struct {
  ...
  //xhd_oai_enb
  int32_t timing_advance_count;
  int32_t timing_advance_active;
