                                       ...
                                       LTE_DL_FRAME_PARMS *frame_parms)
{
  ...
  if (rballoc>RIV_max) {
    LOG_E(PHY,"[eNB]dci_tools.c: ERROR: rb_alloc (%x)> RIV_max(%d %d) N_RB_DL=%d\n",
        rballoc,RIV_max,RIV_max50,frame_parms->N_RB_DL);
    return(-1);
  }
                                      ...
                                      unsigned char eNB_id )
{
  ...
  if (cqireq==1) {
    ...
    if (meas->rank[eNB_id] == 1) {
      ulsch->harq_processes[harq_pid]->uci_format                          = wideband_cqi_rank2_2A;
      ulsch->harq_processes[harq_pid]->O                                   = sizeof_wideband_cqi_rank2_2A_5MHz;
      ulsch->harq_processes[harq_pid]->o_RI[0]                             = 1;
    } else {
      ulsch->harq_processes[harq_pid]->uci_format                          = wideband_cqi_rank1_2A;
      ulsch->harq_processes[harq_pid]->O                                   = sizeof_wideband_cqi_rank1_2A_5MHz;
      ulsch->harq_processes[harq_pid]->o_RI[0]                             = 0;
    }
    ...
    if (((phy_vars_ue->frame_tx % 100) == 0) || (phy_vars_ue->frame_tx < 10))
      print_CQI(ulsch->harq_processes[harq_pid]->o,
               ulsch->harq_processes[harq_pid]->uci_format,
               eNB_id,phy_vars_ue->lte_frame_parms.N_RB_DL);
  } else {
  ...
  msg("ulsch ra (UE): O        %d\n",ulsch->harq_processes[harq_pid]->O);
