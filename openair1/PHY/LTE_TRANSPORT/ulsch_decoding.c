                             ...
                             uint32_t *pulTBS)
{
  ...
  int16_t *ulsch_llr = phy_vars_eNB->lte_eNB_pusch_vars[UE_id][sched_subframe]->llr;
  ...
  uint16_t ausRiIdx[120]={0}; //xhd_oai_debug
  ...
  if( harq_pid_t != harq_pid )
  {
        printf("ulsch_decoding:harq_pid(%d-%d) mismatch sched_subframe=%d frame_rx=%d subframe_rx=%d\n",
            harq_pid_t,harq_pid,sched_subframe,phy_vars_eNB->proc[sched_subframe].frame_rx,subframe);
  }
  ...
  if (ulsch_harq->round == 0) 
  {
    ...
    ulsch_harq->TBS2=ulsch_harq->TBS;
    ulsch_harq->Kplus2=ulsch_harq->Kplus;
  ...
  if (Qprime * Q_m >= (G - Q_RI))
  {
    unsigned int Qprime_old = Qprime;
    unsigned int G_old = G;
    Qprime = (G - Q_RI)/Q_m;
    Qprime_CQI = Qprime;
    Q_CQI = Q_m * Qprime;

    G = 0;
    LOG_E(PHY,"ulsch_decoding, only control info: ulsch_decoding.c G < 0 (%d) : Q_RI %d, Q_CQI %d Q_m %d Qprime %d/%d nb_rb=%d Nsymb_pusch=%d O_RI=%d sumKr=%d %d %d %d %d\n",
        G_old,Q_RI,Q_CQI, Q_m, Qprime, Qprime_old, nb_rb, 
        ulsch_harq->Nsymb_pusch, ulsch_harq->O_RI,
        sumKr,
        ulsch_harq->Or1,ulsch_harq->Msc_initial,ulsch_harq->Nsymb_initial,ulsch->beta_offset_cqi_times8);
  ...
  if ((int)G < 0) {
    LOG_E(PHY,"FATAL: ulsch_decoding.c G < 0 (%d) : Q_RI %d, Q_CQI %d Q_m %d Qprime %d nb_rb=%d Nsymb_pusch=%d O_RI=%d sumKr=%d\n",
        G,Q_RI,Q_CQI, Q_m, Qprime, nb_rb, 
        ulsch_harq->Nsymb_pusch, ulsch_harq->O_RI,
        sumKr);
    return(-1);
  }
  ...
  */
  ...
  if (ulsch_harq->O_ACK > 2) {
    LOG_E(PHY,"ulsch_decoding: FATAL, ACK cannot be more than 2 bits yet ulsch_harq->O_ACK=%d\n", ulsch_harq->O_ACK);
    ulsch_harq->O_ACK = 2;
    return(-1);
  }
  ...
  if( len_ACK > MAX_ACK_PAYLOAD )
  {
      LOG_E(PHY,"ulsch_decoding: FATAL, len_ACK %d cannot be more than %d bits \n", len_ACK, MAX_ACK_PAYLOAD);
      return -1;
  }
    ...
    for (q=0; q<Q_m; q++) 
    {
      if (y[q+(Q_m*((r*Cmux) + columnset[j]))]!=0)
       ...
       print_info("ACK %d => %d (%d,%d,%d)i=%d/%d  Q_m=%d Cmux=%d Rmux_prime=%d len_ACK=%d\n",
           (q+(Q_m*i))%len_ACK,
           ulsch_harq->q_ACK[(q+(Q_m*i))%len_ACK],
  ...
  if( (subframe == 4) && (frame&0xf)== 0 && 0)  
  //if (ulsch_harq->O_ACK ) //xhd_oai_debug eNB  ULSCH after decoding
  {
        print_buff_excel(5,y,Cmux*Rmux_prime*Q_m,"ulsch_decoding: Y Cmux=%d Rmux_prime=%d Q_m=%d \n",
            Cmux,Rmux_prime,Q_m);
  }
  ...
  for (i=0; i<Qprime_RI; i++) {
    ...
    for (q=0; q<Q_m; q++)
    {
      ...
      ulsch_harq->q_RI[(q+(Q_m*i))%len_RI] += y[iIdx]; //[q+(Q_m*((r*Cmux) + columnset[j]))];
  ...
  */
  ...
  if (ulsch_harq->O_ACK == 1) 
  {
      print_info("ACK_HANDLE:  ulsch_decoding: O_ACK=%d ACK=%d %d q_ACK[0]=%d %d ulsch->bundling=%d wACK_idx=%d Nbundled=%d\n",
          ulsch_harq->O_ACK,
          ulsch_harq->o_ACK[0],ulsch_harq->o_ACK[1],
          ulsch_harq->q_ACK[0],ulsch_harq->q_ACK[1],
          ulsch->bundling,wACK_idx,Nbundled);
  }

  if( (subframe == 4) && (frame&0xf)== 0 && 0) //xhd_oai_debug eNB ulSch decoding ACK
  {
        print_buff(ulsch_harq->q_ACK,MAX_ACK_PAYLOAD,0,"ulsch_decoding: q_Ack:len=%d Qprime_ACK=%d Q_m=%d O_ACK=%d o_ACK[0]=%d %d\n",
            MAX_ACK_PAYLOAD,Qprime_ACK,Q_m,
            ulsch_harq->O_ACK,ulsch_harq->o_ACK[0],ulsch_harq->o_ACK[1]);
  }
  // RI
  ...
  if( (subframe == 4) && (frame&0xf)== 0 && 0) //xhd_oai_debug eNB ulSch encoding RI
  {
        print_info("ulsch_decoding: q_RI:len=%d [%d %d] at [%d %d %d %d] Rmux_prime=%d Cmux=%d Q_m=%d len_RI=%d Hpp=%d O_RI=%d o_RI[0]=%d\n",
            Qprime_RI,ulsch_harq->q_RI[0],ulsch_harq->q_RI[1],ausRiIdx[0],ausRiIdx[1],ausRiIdx[2],ausRiIdx[3],
            Rmux_prime,Cmux,Q_m,len_RI,Hpp,
  ...
  if( (subframe == 4) && (frame&0xf)== 0) //xhd_oai_debug eNB ulSch decoding CQI
  {
        print_buff(ulsch_harq->q,Qprime_CQI*Q_m,0,"ulsch_decoding: q:len=%d Qprime_CQI=%d Q_m=%d j=%d pHarq->O=%d o=%02x%02x%02x%02x %02x%02x%02x%02x\n",
            MAX_CQI_PAYLOAD,Qprime_CQI,Q_m,j, ulsch_harq->Or1,
            ulsch_harq->o[0],
            ulsch_harq->o[1],
            ulsch_harq->o[2],
            ulsch_harq->o[3],
  ...
  for (r=0; r<ulsch_harq->C; r++) {
    ...
    ulsch_harq->RTC[r] = generate_dummy_w(4+(Kr_bytes*8),
                                          (uint8_t*)&dummy_w[r][0],
                                          (r==0) ? ulsch_harq->F : 0); //xhd_oai_debug
    ...
    if( harq_pid == 4)
    {
        print_info("lte_rate_matching_turbo_rx_enb: frame=%d subframe=%d harq=%d, round=%d rvidx=%d mcs=%d q_m=%d clear=%d F=%d RTC=%d D=%d ND=%d\n", 
            frame0, subframe, harq_pid, ulsch_harq->round, ulsch_harq->rvidx, 
            ulsch_harq->mcs, get_Qm_ul(ulsch_harq->mcs),(ulsch_harq->round==0)?1:0,
            ulsch_harq->F, ulsch_harq->RTC[r],4+(Kr_bytes*8),((ulsch_harq->RTC[r]<<5)-(4+(Kr_bytes*8))));
    }
    
    if (lte_rate_matching_turbo_rx_enb(ulsch_harq->RTC[r],
                                   G,
                                   ulsch_harq->w[r],
                                   ulsch_harq->wc[r],
                                   (uint8_t*) &dummy_w[r][0],
    ...
    for (r=0; r<ulsch_harq->C; r++) {
      ...
      */
      ...
      ret = tc(&ulsch_harq->d[r][96],
               ...
               ulsch->max_turbo_iterations,//MAX_TURBO_ITERATIONS, xhd_oai_20users
               crc_type,
               (r==0) ? ulsch_harq->F : 0,
      ...
      //  "ulsch_decoding: after threegpplte_turbo_decoder r=%d Kr=%d(%d bytes) harq_pid=%d round=%d\n",
...
#endif
        if( (subframe == 4) && (frame&0xf)== 0 && 0) //xhd_oai_debug eNB decoding
        {
            print_buff_excel(1,&ulsch->harq_processes[harq_pid]->d[r][96],Kr,
              "ulsch_decoding: before threegpplte_turbo_decoder r=%d Kr=%d(%d bytes) harq_pid=%d round=%d\n",
              r, Kr,Kr>>3, harq_pid,ulsch->harq_processes[harq_pid]->round);
