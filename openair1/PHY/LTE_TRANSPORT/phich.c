...
uint8_t phich_subframe2_pusch_subframe(LTE_DL_FRAME_PARMS *frame_parms,uint8_t subframe,uint8_t flag)
{
  ...
  case 3:
    if ( (subframe == 8) || (subframe == 9) ) {
      ...
      LOG_E(PHY,"phich.c: phich_subframe2_pusch_subframe, illegal subframe %d for tdd_config %d flag=%d\n",
            subframe,frame_parms->tdd_config,flag);
      return(0);
    }
              ...
              uint8_t eNB_id)
{
  ...
  uint8_t subframe_send = phich_subframe2_pusch_subframe(frame_parms,subframe,4);
  ...
  uint32_t pre_frame = phich_frame2_pusch_frame(frame_parms,phy_vars_ue->frame_rx,subframe);
  uint32_t pre_subframe = phich_subframe2_pusch_subframe(frame_parms,subframe,3);
                           
  PERF_TIME(PERF_UE_rx_phich, PERF_START);
  ...
  if (ulsch->harq_processes[harq_pid]->status == ACTIVE) {
    ...
    if ((frame_parms->tdd_config == 0) && (frame_parms->frame_type == TDD) ) {
      pusch_subframe = phich_subframe2_pusch_subframe(frame_parms,subframe,0);
  ...
  PERF_TIME_LINE(PERF_UE_rx_phich, PERF_LINE);
  ...
  */
  ...
  {   
    ...
    //NACK
    ulsch->harq_processes[harq_pid]->HichNew = 2;
    if (phy_vars_ue->ulsch_ue_Msg3_active[eNB_id] == 1) 
    {
      ...
      if (ulsch->harq_processes[harq_pid]->round>=phy_vars_ue->lte_frame_parms.maxHARQ_Msg3Tx) {
        ...
        ulsch->harq_processes[harq_pid]->HichNew = 0;
      }
    } 
    else 
    {
      ...
      if( harq_pid == 4)
      {
          print_info("[UE  %d][PUSCH %d] Frame %d subframe %d PHICH, received NAK (%d) nseq %d, ngroup %d\n",
                phy_vars_ue->Mod_id,harq_pid,
                phy_vars_ue->frame_rx,
                subframe,
                HI16,
      ...
      UE_SendCountN(subframe_send,ulsch->harq_processes[harq_pid]->round);
      ...
      ulsch->harq_processes[harq_pid]->O    = 0;
  ...
  {  
    ...
    if (phy_vars_ue->ulsch_ue_Msg3_active[eNB_id] == 1) 
    {
      ...
      //ulsch->harq_processes[harq_pid]->roundHich = 0;
      // inform MAC?
      phy_vars_ue->ulsch_ue_Msg3_active[eNB_id] = 0;//2;
    ...
    else 
    {
      if( harq_pid == 4)
      {
          print_info("[UE  %d][PUSCH %d] Frame %d subframe %d PHICH, received ACK (%d) nseq %d, ngroup %d\n\n",
                phy_vars_ue->Mod_id,harq_pid,
                phy_vars_ue->frame_rx,
                subframe, HI16,
                nseq_PHICH,ngroup_PHICH);
      }
      UE_SendCountA(subframe_send,ulsch->harq_processes[harq_pid]->round);
      ulsch->harq_processes[harq_pid]->subframe_scheduling_flag =0;
      ulsch->harq_processes[harq_pid]->status = IDLE;
      ulsch->harq_processes[harq_pid]->round  = 0;
      ...
      //UE_SendCountA(subframe,ulsch->harq_processes[harq_pid]->round);

    }



    ulsch->harq_processes[harq_pid]->HichNew = 1;

  }
  PERF_TIME(PERF_UE_rx_phich, PERF_ONCE);
                        ...
                        uint8_t abstraction_flag)
{
  ...
  pusch_subframe = phich_subframe2_pusch_subframe(frame_parms,subframe,1);
  ...
  for (UE_id=0; UE_id<NUMBER_OF_UE_MAX; UE_id++) {
    if ((ulsch_eNB[UE_id])&&(ulsch_eNB[UE_id]->rnti>0)) {
      if (ulsch_eNB[UE_id]->harq_processes[harq_pid]->phich_active == 1) {
        ...
        //#endif
        //xhd_oai_20users generate phich
        if (ulsch_eNB[UE_id]->harq_processes[harq_pid]->crnti_flag == 1) 
        {
            LOG_D(PHY,"[eNB %d][PUSCH %d][RAPROC] Frame %d, subframe %d: Generating Msg3 PHICH for MAC CRNTI UE %d, ngroup_PHICH %d/%d, nseq_PHICH %d : HI %d, first_rb %d\n",
                  ...
                  ulsch_eNB[UE_id]->harq_processes[harq_pid]->first_rb);
            ulsch_eNB[UE_id]->harq_processes[harq_pid]->crnti_flag = 0;
        }
            ...
            (ulsch_eNB[UE_id]->harq_processes[harq_pid]->rar_alloc == 0) ) 
        {
          ...
          {
            #if 0
            LOG_W(PHY,"[UE %d][PUSCH %d] frame %d, subframe %d : PHICH NACK / (no format0 DCI) Setting subframe_scheduling_flag from %d\n",
                  UE_id,harq_pid,
                  phy_vars_eNB->proc[sched_subframe].frame_tx,subframe,
                  ulsch_eNB[UE_id]->harq_processes[harq_pid]->subframe_scheduling_flag);
          ...
          {
            if( ulsch_eNB[UE_id]->harq_processes[harq_pid]->subframe_scheduling_flag == 1)
            {
                LOG_W(PHY,"[UE %d][PUSCH %d] frame %d subframe %d PHICH ACK (no format0 DCI) Clearing subframe_scheduling_flag from %d, setting round from %d to 0\n",
                      UE_id,harq_pid,
                      phy_vars_eNB->proc[sched_subframe].frame_tx,subframe,
                      ulsch_eNB[UE_id]->harq_processes[harq_pid]->subframe_scheduling_flag,
