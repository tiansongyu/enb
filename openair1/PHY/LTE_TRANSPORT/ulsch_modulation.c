                      ...
                      int iUeId)
{
  ...
  //if( (subframe == 4) && (frame&0xf)== 0)
  if( ulsch->harq_processes[harq_pid]->O_ACK ) //xhd_oai_debug ue dft
  {
      print_buff_excel(3, ulsch->d,Msc_PUSCH,
           "ulsch_modulation: before dft_lte UE_id=%d frame=%d sub_frame=%d harq_pid=%d first_rb=%d nb_rb=%d size=%d  mcs=%d TBS=%d len=%d Q_m=%d\n",
           iUeId, frame, subframe,  harq_pid, first_rb, nb_rb, Msc_PUSCH,
           ulsch->harq_processes[harq_pid]->mcs,
           ulsch->harq_processes[harq_pid]->TBS>>3,
           Msymb,Q_m);
  }
  
  dft_lte(ulsch->z,ulsch->d,Msc_PUSCH,ulsch->Nsymb_pusch);

  if( (subframe == 4) && (frame&0xf)== 0 && 0) //xhd_oai_debug ue dft
  {
      print_buff_excel(0,ulsch->z,Msc_PUSCH,
         "ulsch_modulation: after dft_lte UE_id=%d frame=%d sub_frame=%d harq_pid=%d first_rb=%d nb_rb=%d size=%d  mcs=%d TBS=%d len=%d Q_m=%d\n",
         iUeId, frame, subframe, harq_pid, first_rb, nb_rb, Msc_PUSCH,
         ulsch->harq_processes[harq_pid]->mcs,
         ulsch->harq_processes[harq_pid]->TBS>>3,
         Msymb,Q_m);
