...
typedef struct {
  ...
  void (*initiate_ra_proc)(module_id_t Mod_id,int CC_id,frame_t frameP,uint16_t preamble,
      int16_t timing_offset,uint8_t sect_id,sub_frame_t subframe,uint8_t f_id, int16_t rnti);
  ...
  void (*rx_sdu)(module_id_t Mod_id,int CC_id,frame_t frameP, sub_frame_t sub_frameP,rnti_t rnti, 
      uint8_t *sdu,uint16_t sdu_len, int harq_pid,uint8_t *msg3_flag, rnti_t *crnti, uint8_t ucRound,
  uint8_t           ucFirstRb,
  uint8_t           ucRbNum);
  ...
  //xhd_oai_enb
  /// Function for MAC to get the UE stats from the PHY   
  void (*phy_remove_ue)(module_id_t Mod_id, uint8_t CC_id, rnti_t rnti);
  void (*phy_remain_ue)(module_id_t Mod_id, uint8_t CC_id, rnti_t rnti);
