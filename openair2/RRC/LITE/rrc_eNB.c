...
//-----------------------------------------------------------------------------
{
  ...
  //xhd_oai_multuser
  return 0; //rrc_transaction_identifier[enb_mod_idP];
}
/*------------------------------------------------------------------------------*/
...
rrc_eNB_get_ue_context_by_random(
  const protocol_ctxt_t* const ctxt_pP,
  const uint64_t               ue_identityP
)
//-----------------------------------------------------------------------------
...
extern int mac_remove_ue_by_rnti(module_id_t mod_idP, int rnti);
extern void cancel_ra_proc(module_id_t module_idP, int CC_id, frame_t frameP, rnti_t rnti);
...
//-----------------------------------------------------------------------------
{
  ...
  {
      //xhd_oai_multuser random id可能相同，不能作为判断是否同一个UE的依据
      #if 0
    RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head)) 
    {
      if (ue_context_p->ue_context.random_ue_identity == ue_identityP) 
      {
        LOG_I(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" rrc_eNB_get_next_free_ue_context, already exist rand UE id 0x" PRIx64 ", uid %u mme_ue_id:%x\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ue_identityP,
              ue_context_p->local_uid,
              ue_context_p->ue_context.mme_ue_id);
        //xhd_oai_multuser
        ucFound = TRUE;
        RB_REMOVE(rrc_ue_tree_s, &eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head, ue_context_p);

        
        //通知PHY、MAC、RLC、PDCP释放
        uint16_t rnti = ue_context_p->ue_context.rnti;
        
        protocol_ctxt_t ctxt;
        PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, 0, ENB_FLAG_YES, rnti, 0, 0,0);
       
        mac_xface->phy_remove_ue(0,0,rnti);
        mac_remove_ue_by_rnti(0, rnti);
        cancel_ra_proc(0, 0, 0, rnti);
    ...
    if( ucFound == FALSE )
    {
        ...
        LOG_I(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"rrc_eNB_get_next_free_ue_context: Created new UE context uid %u\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ue_context_p->local_uid);

        ue_context_p->ue_context.mme_ue_id = 0;
    }
...
static struct rrc_eNB_ue_context_s*
rrc_eNB_get_next_free_ue_context_tmsi(
  const protocol_ctxt_t* const ctxt_pP,
  const uint64_t               ue_tmsi
)
//-----------------------------------------------------------------------------
{
  struct rrc_eNB_ue_context_s*        ue_context_p = NULL;
  ...
  if (ue_context_p == NULL) 
  {
    RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head)) 
    {
      if (ue_context_p->ue_context.Initialue_identity_s_TMSI.m_tmsi == ue_tmsi
        || ue_context_p->ue_context.mme_ue_id == ue_tmsi) 
      {
        LOG_E(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"rrc_eNB_get_next_free_ue_context_tmsi: already exist rand UE id 0x%x, uid %u\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ue_tmsi,
              ue_context_p->local_uid);

        //xhd_oai_20users
        phy_set_delete_flag(ue_context_p->ue_id_rnti, ue_context_p->ue_id_rnti, ue_tmsi, ue_context_p->ue_context.random_ue_identity,3);
        printf("rrc_eNB_get_next_free_ue_context_tmsi: set delete flag for tmsi:%x rnti %x\n",ue_tmsi, ue_context_p->ue_id_rnti);
        //mac_xface->phy_remove_ue(0,0,ue_context_p->ue_id_rnti);
        //xhd_oai_multuser
        RB_REMOVE(rrc_ue_tree_s, &eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head, ue_context_p);
        ucFound = TRUE;
        break;
    ...
    if (ucFound == FALSE) 
    {
        ...
        LOG_I(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"rrc_eNB_get_next_free_ue_context_tmsi: Created new UE context uid %u\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ue_context_p->local_uid);
...
//xhd_oai_enb
void rrc_eNB_remove_ue_context_by_tmsi(
  const protocol_ctxt_t* const ctxt_pP,
  const uint64_t               ue_tmsi
)
//-----------------------------------------------------------------------------
{
  struct rrc_eNB_ue_context_s*        ue_context_p = NULL;
    ...
    RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head)) 
    {
      if (ue_context_p->ue_context.mme_ue_id == ue_tmsi
         && ue_context_p->ue_id_rnti != ctxt_pP->rnti) 
      {
           printf("rrc_eNB_remove_ue_context_by_tmsi:UE(RNTI=%d) removed for duplicated tmsi 0x%x of rnti %d\n",
                ue_context_p->ue_id_rnti, ue_tmsi, ctxt_pP->rnti);        

           mac_xface->phy_remove_ue(0,0,ue_context_p->ue_id_rnti);

           rrc_eNB_remove_ue_context(ctxt_pP, &eNB_rrc_inst[ctxt_pP->module_id], ue_context_p);
...
static struct rrc_eNB_ue_context_s*
rrc_eNB_get_ue_context_by_rnti(
  const protocol_ctxt_t* const ctxt_pP
)
//-----------------------------------------------------------------------------
{
  struct rrc_eNB_ue_context_s*        ue_context_p = NULL;
    ...
    RB_FOREACH(ue_context_p, rrc_ue_tree_s, &(eNB_rrc_inst[ctxt_pP->module_id].rrc_ue_head)) {
      if (ue_context_p->ue_id_rnti == ctxt_pP->rnti) {
        LOG_D(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT"rrc_eNB_get_ue_context_by_rnti: already exist rand UE id 0x%x, uid %u\n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ctxt_pP->rnti,
              ue_context_p->local_uid);
        return ue_context_p;
...
//-----------------------------------------------------------------------------
{
  ...
  if (NULL != ue_context_p) {
    ...
    LOG_D(RRC, "[eNB %d] Removing UE RNTI %x\n", enb_mod_idP, rntiP);
    ...
    //xhd_oai_enb
    LOG_E(RRC, "rrc_eNB_free_UE: rntiP=%x tmsi=%x random_id=%x delete_flag=%d\n",
        rntiP, 
        ue_context_p->ue_context.mme_ue_id,
        ue_context_p->ue_context.random_ue_identity,
        phy_get_delete_flag(rntiP));

    #if 0
    //mac_xface->phy_remove_ue(0,0,ctxt.rnti);
    phy_set_delete_flag(ue_context_p->ue_id_rnti, ue_context_p->ue_id_rnti, 
        ue_context_p->ue_context.mme_ue_id, 
        ue_context_p->ue_context.random_ue_identity,
        4);
    #endif
...
//-----------------------------------------------------------------------------
{
  ...
  if (NULL != ue_context_p) {
    ...
    Status = (ue_context_p->ue_context.Status>=RRC_RECONFIGURED)?2
        :((ue_context_p->ue_context.Status>=RRC_CONNECTED)?1:0);
    if(pRandomId != NULL )
    {
        *pRandomId = ue_context_p->ue_context.random_ue_identity;
    }
    if(pTmsi != NULL )
    {
        *pTmsi = ue_context_p->ue_context.mme_ue_id;
    }
...
//-----------------------------------------------------------------------------
void
rrc_eNB_process_RRCConnectionRequest(
  const protocol_ctxt_t* const ctxt_pP,
...
)
{
    ...
    if (ue_context_p != NULL) 
    {
        printf("rrc_eNB_process_RRCConnectionRequest:rnti[%x] already exists\n",ctxt_pP->rnti );
        ...
        MSC_LOG_RX_DISCARDED_MESSAGE(
          ...
          consumed);
        //xhd_oai_enb remove old context
        if(rrcConnectionRequest->ue_Identity.present == InitialUE_Identity_PR_s_TMSI)
        {
            rrc_eNB_remove_ue_context_by_tmsi(ctxt_pP, 
                BIT_STRING_to_uint32(&rrcConnectionRequest->ue_Identity.choice.s_TMSI.m_TMSI));
    ...
    else 
    {
        if(rrcConnectionRequest->ue_Identity.present == InitialUE_Identity_PR_randomValue)
        {
           ...
           */
                /*
                ue_context_p = rrc_eNB_get_ue_context_by_random(ctxt_pP, random_value);
                if (ue_context_p != NULL) {
                //AssertFatal(0 == 1, "TODO: remove UE fro MAC/PHY (how?)");
                printf("rrc_eNB_decode_ccch:UE already exists, rnti=%x UE random UE identity (0x%" PRIx64 ")\n",
                ue_context_p->ue_context.rnti,
                random_value);
                ...
                */
              LOG_W(RRC,"rrc_eNB_process_RRCConnectionRequest: UE random UE identity (0x%" PRIx64 ") \n",
                    random_value);

              ue_context_p = rrc_eNB_get_next_free_ue_context(ctxt_pP, random_value);
        }
        else if(rrcConnectionRequest->ue_Identity.present == InitialUE_Identity_PR_s_TMSI)
        {
            //xhd_oai_enb
            LOG_W(RRC,"rrc_eNB_process_RRCConnectionRequest: ue_Identity.present=%d m_TMSI=%x\n", 
                rrcConnectionRequest->ue_Identity.present,
                BIT_STRING_to_uint32(&rrcConnectionRequest->ue_Identity.choice.s_TMSI.m_TMSI));
            
            ue_context_p = rrc_eNB_get_next_free_ue_context_tmsi(ctxt_pP, 
                BIT_STRING_to_uint32(&rrcConnectionRequest->ue_Identity.choice.s_TMSI.m_TMSI));

        }
        else
        {
            ue_context_p = rrc_eNB_get_next_free_ue_context(ctxt_pP, 0);
        }
        LOG_D(RRC,
        ...
        if (ue_context_p != NULL) 
        {
          ...
          LOG_W(RRC, PROTOCOL_RRC_CTXT_UE_FMT" Accept new connection from UE random UE identity (0x%" PRIx64 ") MME code %u TMSI %x cause %u\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                ue_context_p->ue_context.random_ue_identity,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.mme_code,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.m_tmsi,
        ...
        else 
        {
          ...
          LOG_W(RRC, PROTOCOL_RRC_CTXT_UE_FMT" Can't create new context for UE random UE identity (0x%" PRIx64 ")\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                random_value);
          return -1;
        }
      ...
      LOG_D(RRC, PROTOCOL_RRC_CTXT_UE_FMT"CALLING RLC CONFIG SRB1 (rbid %d)\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
            Idx);
...
void
rrc_eNB_process_RRCConnectionReestablishmentRequest(
  const protocol_ctxt_t* const ctxt_pP,
...
)
{
    ...
    if (ue_context_p != NULL) 
    {
        printf("rrc_eNB_process_RRCConnectionReestablishmentRequest:rnti[%x] already exists\n",ctxt_pP->rnti );
        ...
        MSC_LOG_RX_DISCARDED_MESSAGE(
          ...
          Srb_info->Rx_buffer.Payload,
          consumed,
          MSC_AS_TIME_FMT" RRCConnectionRestablishmentRequest UE %x size %u (UE already in context)",
          MSC_AS_TIME_ARGS(ctxt_pP),
          ue_context_p->ue_context.rnti,
          consumed);
    } 

        memcpy(((uint8_t*) & crnti) ,
                 rrcConnectionReestablishmentRequest->ue_Identity.c_RNTI.buf,
                 rrcConnectionReestablishmentRequest->ue_Identity.c_RNTI.size);
        crnti = ((crnti&0xff) <<8)+((crnti>>8)&0xff);
        
        //printf("rrc_eNB_process_RRCConnectionReestablishmentRequest: rnti=%x crnti = %x  \n",
        //    ctxt_pP->rnti, crnti);

        new_rnti = ctxt_pP->rnti;
        //ctxt_pP->rnti = crnti;
        ...
        if (ue_context_p != NULL) 
        {
          ...
          //phy_set_delete_flag(new_rnti, crnti, ue_context_p->ue_context.mme_ue_id, ue_context_p->ue_context.random_ue_identity,5);

          /* Check s-TMSI presence in message */
          ue_context_p->ue_context.Initialue_identity_s_TMSI.presence = 0;


          ue_context_p->ue_context.establishment_cause =
            rrcConnectionReestablishmentRequest->reestablishmentCause;
          LOG_W(RRC, PROTOCOL_RRC_CTXT_UE_FMT"rrc_eNB_process_RRCConnectionReestablishmentRequest:  crnti %x random UE identity (0x%" PRIx64 ") MME code %u TMSI %x cause %u\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                crnti,
                ue_context_p->ue_context.random_ue_identity,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.mme_code,
        ...
        {
          phy_set_delete_flag(new_rnti, crnti, 0, 0,6);

          // no context available
          LOG_E(RRC, PROTOCOL_RRC_CTXT_UE_FMT" CRNTI(%x) not found!\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                crnti);
          return -1;
        }
...
//-----------------------------------------------------------------------------
{
  uint8_t                             buffer[100];
  uint8_t                             size;
  uint8_t                             ucTranId = rrc_eNB_get_next_transaction_identifier(ctxt_pP->module_id);

  LOG_W(RRC,"[MSG] RRC Security Mode Command:ciphering_algo:%d integrity_algo:%d ucTranId=%d\n",
  ...
  {
      LOG_I(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" Logical Channel DL-DCCH, Generate SecurityModeCommand (bytes %d)%02x %02x %02x\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
            size,
...
//-----------------------------------------------------------------------------
{
  ...
  printf("inside rrc_eNB_generate_RRCConnectionReestablishmentReject\n");
...
//-----------------------------------------------------------------------------
{
  ...
  *periodicBSR_Timer = MAC_MainConfig__ul_SCH_Config__periodicBSR_Timer_sf5;
...
//-----------------------------------------------------------------------------
{
  ...
  *mobilityInfo->additionalSpectrumEmission = 1;  //Check this value!
  ...
  LOG_D(RRC, "[eNB %d] CALLING RLC CONFIG SRB1 (rbid %d) for UE %x\n",
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_D(RRC, "rrc_eNB_process_RRCConnectionReconfigurationComplete: ciphering_algorithm:%d integrity_algorithm:%d\n",
    ue_context_pP->ue_context.ciphering_algorithm,
    ue_context_pP->ue_context.integrity_algorithm);
  ...
  if (DRB_configList != NULL) {
    for (i = 0; i < DRB_configList->list.count; i++) {  // num max DRB (11-3-8)
      if (DRB_configList->list.array[i]) {
        ...
        // for pre-ci tests
        my_printf("rrc_eNB_process_RRCConnectionReconfigurationComplete:local_uid=%d\n", ue_context_pP->local_uid);
        if( ue_context_pP->local_uid < NUMBER_OF_UE_MAX )
        {
        LOG_D(RRC,
...
#   endif
  ...
  for (i = 0; i < 8; i++) {
    ...
    printf( "%x.", ((uint8_t *) & ul_ccch_msg)[i]);
  }
  ...
  if (ul_ccch_msg->message.present == UL_CCCH_MessageType_PR_c1) {
    ...
    case UL_CCCH_MessageType__c1_PR_rrcConnectionReestablishmentRequest:
      ...
      */
      ...
      printf("rrc_eNB_decode_ccch: call rrc_eNB_generate_RRCConnectionReestablishmentReject start\n");
      rrc_eNB_generate_RRCConnectionReestablishmentReject(ctxt_pP,
                       rrc_eNB_get_ue_context(&eNB_rrc_inst[ctxt_pP->module_id], ctxt_pP->rnti),
                       CC_id);
      printf("rrc_eNB_decode_ccch: call rrc_eNB_generate_RRCConnectionReestablishmentReject finish\n");
      break;
    ...
    case UL_CCCH_MessageType__c1_PR_rrcConnectionRequest:
      ...
      if (ue_context_p != NULL) {
        printf("rrc_eNB_decode_ccch:rnti[%x] already exists\n",ctxt_pP->rnti );
      ...
      else 
      {
        ...
        if(rrcConnectionRequest->ue_Identity.present == InitialUE_Identity_PR_randomValue)
        {
           ...
           */
                ...
                */
              printf("rrc_eNB_decode_ccch: UE random UE identity (0x%" PRIx64 ") \n",
                    random_value);
        ...
        {
            //xhd_oai_enb
            printf("rrc_eNB_decode_ccch: ue_Identity.present=%d m_TMSI=%x\n", 
                rrcConnectionRequest->ue_Identity.present,
                BIT_STRING_to_uint32(&rrcConnectionRequest->ue_Identity.choice.s_TMSI.m_TMSI));
        ...
        if (ue_context_p != NULL) 
        {
          ...
          ue_context_p->ue_context.establishment_cause =
            rrcConnectionRequest->establishmentCause;
          LOG_I(RRC, PROTOCOL_RRC_CTXT_UE_FMT" Accept new connection from UE random UE identity (0x%" PRIx64 ") MME code %u TMSI %x cause %u\n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
                ue_context_p->ue_context.random_ue_identity,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.mme_code,
                ue_context_p->ue_context.Initialue_identity_s_TMSI.m_tmsi,
...
#   endif
  ...
  if (ul_ccch_msg->message.present == UL_CCCH_MessageType_PR_c1) {
    ...
    case UL_CCCH_MessageType__c1_PR_rrcConnectionReestablishmentRequest:
      ...
      rrc_eNB_process_RRCConnectionReestablishmentRequest(ctxt_pP, rrcConnectionReestablishmentRequest, Srb_info, CC_id, dec_rval.consumed);
    ...
    case UL_CCCH_MessageType__c1_PR_rrcConnectionRequest:
        ...
        rrc_eNB_process_RRCConnectionRequest(ctxt_pP, rrcConnectionRequest, Srb_info, CC_id, dec_rval.consumed);
...
//-----------------------------------------------------------------------------
int
rrc_eNB_crnti_ind(const protocol_ctxt_t* const ctxt_pP)
//-----------------------------------------------------------------------------
{
    ...
    if( ue_context_p == NULL )
    {
        ue_context_p = rrc_eNB_get_ue_context_by_rnti(ctxt_pP);

       if( ue_context_p == NULL )
       {
           printf("rrc_eNB_crnti_ind:get ue_context_p by rnti failed, rnti=%x \n",
               ctxt_pP->rnti);
           return -1;
       }
       else
       {
           printf("rrc_eNB_crnti_ind:get ue_context_p by rnti success, rnti=%x  \n",
               ctxt_pP->rnti);
       }
    }
    else
    {
          printf("rrc_eNB_crnti_ind:get ue_context_p success, rnti=%x/%x  mme_ue_id:%x ue_initial_id=%d eNB_ue_s1ap_id=0x%x \n",
              ctxt_pP->rnti, 
              ue_context_p->ue_context.rnti,
              ue_context_p->ue_context.mme_ue_id,
              ue_context_p->ue_context.ue_initial_id,
...
#   endif
  ...
  if( ue_context_p == NULL )
  {
     ...
     if( ue_context_p == NULL )
     {
         printf("rrc_eNB_decode_dcch:get ue_context_p by rnti failed, rnti=%x  message:%d %d\n",
             ctxt_pP->rnti, ul_dcch_msg->message.present,
             ul_dcch_msg->message.choice.c1.present);
         return -1;
     }
     else
     {
         printf("rrc_eNB_decode_dcch:get ue_context_p by rnti success, rnti=%x  message:%d %d\n",
             ctxt_pP->rnti, ul_dcch_msg->message.present,
             ul_dcch_msg->message.choice.c1.present);
     }
  }
  ...
  {
        my_printf("rrc_eNB_decode_dcch:get ue_context_p success, rnti=%x/%x  mme_ue_id:%d ue_initial_id=%d eNB_ue_s1ap_id=%d/0x%x message:%d %d\n",
            ctxt_pP->rnti, 
            ue_context_p->ue_context.rnti,
            ue_context_p->ue_context.mme_ue_id,
            ue_context_p->ue_context.ue_initial_id,
  ...
  if (ul_dcch_msg->message.present == UL_DCCH_MessageType_PR_c1) {
    ...
    case UL_DCCH_MessageType__c1_PR_rrcConnectionReconfigurationComplete:
      ...
      //xhd_oai_20users
      LOG_N(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" received RRCConnectionReconfigurationComplete\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
      ...
      if (ul_dcch_msg->message.choice.c1.choice.rrcConnectionReconfigurationComplete.criticalExtensions.
        ...
        LOG_W(RRC,
              PROTOCOL_RRC_CTXT_UE_FMT" mme_ue_id=%x UE State = RRC_RECONFIGURED \n",
              PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
              ue_context_p->ue_context.mme_ue_id);
...
#   endif
    ...
    case UL_DCCH_MessageType__c1_PR_rrcConnectionReestablishmentComplete:
      ...
      //xhd_oai_20users
      LOG_N(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT"  received rrcConnectionReestablishmentComplete\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
    ...
    case UL_DCCH_MessageType__c1_PR_rrcConnectionSetupComplete:
      ...
      //xhd_oai_20users
      LOG_N(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT"  received RRCConnectionSetupComplete\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
      ...
      if (ul_dcch_msg->message.choice.c1.choice.rrcConnectionSetupComplete.criticalExtensions.present ==
        ...
        if (ul_dcch_msg->message.choice.c1.choice.rrcConnectionSetupComplete.criticalExtensions.choice.c1.
          ...
          LOG_W(RRC, PROTOCOL_RRC_CTXT_UE_FMT" UE State = RRC_CONNECTED \n",
                PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
        }
      }
    ...
    case UL_DCCH_MessageType__c1_PR_securityModeComplete:
      ...
      LOG_N(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" received securityModeComplete\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
      LOG_D(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" RLC RB %02d --- RLC_DATA_IND %d bytes "
...
#warning "LG Removed rrc_eNB_generate_UECapabilityEnquiry after receiving securityModeFailure"
    ...
    case UL_DCCH_MessageType__c1_PR_ueCapabilityInformation:
      ...
      LOG_D(RRC,
            ...
            DCCH);
      LOG_N(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" received ueCapabilityInformation\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP));
      LOG_D(RRC,
            PROTOCOL_RRC_CTXT_UE_FMT" ***********************************************************\n",
            PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
