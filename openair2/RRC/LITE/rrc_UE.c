...
static int rrc_set_sub_state( module_id_t ue_mod_idP, Rrc_Sub_State_t subState )
{
  ...
  if (UE_rrc_inst[ue_mod_idP].RrcSubState != subState) {
      printf("@@@@@rrc_set_sub_state: subState %d ==> %d\n",
...
void rrc_ue_generate_RRCConnectionRequest( const protocol_ctxt_t* const ctxt_pP, const uint8_t eNB_index )
{
  ...
  printf("rrc_ue_generate_RRCConnectionRequest:payload_size=%d eNB_index=%d module_id=%d\n",
...
//-----------------------------------------------------------------------------
{
  ...
  printf("rrc_ue_process_radioResourceConfigDedicated...\n");
  ...
  if (radioResourceConfigDedicated->srb_ToAddModList) {
    ...
    printf("rrc_ue_process_radioResourceConfigDedicated...ciphering_algorithm%d integrity_algorithm:%d\n",
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_I(RRC,"[UE %d] security mode is %x \n",ctxt_pP->module_id, securityMode);
...
# if defined(DISABLE_ITTI_XER_PRINT)
  {
    MessageDef *msg_p;
    printf("@@@@@@ decode_BCCH_DLSCH_Message: 4 \n");
    ...
    itti_send_msg_to_task (TASK_UNKNOWN, ctxt_pP->instance, msg_p);
    printf("@@@@@@ decode_BCCH_DLSCH_Message: 5 \n");
...
# else
  {
    ...
    size_t      message_string_size;
    printf("@@@@@@ decode_BCCH_DLSCH_Message: 6 \n");

    if ((message_string_size = xer_sprint(message_string, sizeof(message_string), &asn_DEF_BCCH_DL_SCH_Message, (void *)bcch_message)) > 0) {
      MessageDef *msg_p;
      printf("@@@@@@ decode_BCCH_DLSCH_Message: 6.1 \n");
      ...
      itti_send_msg_to_task(TASK_UNKNOWN, ctxt_pP->instance, msg_p);
      printf("@@@@@@ decode_BCCH_DLSCH_Message: 6.2 \n");
...
# endif
  ...
  {
      printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.1 \n");
    switch (bcch_message->message.choice.c1.present)
    {
    case BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1:
      printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.2 \n");
      if ((ctxt_pP->frame % 2) == 0) 
      {
        // even frame
        printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.2.1 \n");
        if ((UE_rrc_inst[ctxt_pP->module_id].Info[eNB_index].SIStatus&1) == 0)
        {
          printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.2.2  eNB_index=%d\n", eNB_index);
          SystemInformationBlockType1_t* sib1 = UE_rrc_inst[ctxt_pP->module_id].sib1[eNB_index];
          memcpy( (void*)sib1,
                  (void*)&bcch_message->message.choice.c1.choice.systemInformationBlockType1,
                  sizeof(SystemInformationBlockType1_t) );
          LOG_D( RRC, "[UE %"PRIu8"] Decoding First SIB1\n", ctxt_pP->module_id );
          printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.2.3 \n");
          decode_SIB1( ctxt_pP, eNB_index, rsrq, rsrp );
          printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.2.4 \n");
        }
      }
    ...
    case BCCH_DL_SCH_MessageType__c1_PR_systemInformation:
        printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.3 \n");
    ...
    case BCCH_DL_SCH_MessageType__c1_PR_NOTHING:
        printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.4 \n");
    default:
        printf("@@@@@@ decode_BCCH_DLSCH_Message: 7.5 \n");
      break;
    }
  }
  ...
  if ((rrc_get_sub_state(ctxt_pP->module_id) == RRC_SUB_STATE_IDLE_SIB_COMPLETE)
     ...
     {
        //xhd_oai_plmn
         printf("@@@@@@ decode_BCCH_DLSCH_Message: 9 \n");
        rrc_ue_generate_RRCConnectionRequest(ctxt_pP, 0);
        rrc_set_sub_state( ctxt_pP->module_id, RRC_SUB_STATE_IDLE_CONNECTING );
    }
else
{
    printf("@@@@@@ decode_BCCH_DLSCH_Message: rrc state=%d/%d initialNasMsg.data=%llx\n",
        rrc_get_sub_state(ctxt_pP->module_id),
        RRC_SUB_STATE_IDLE_SIB_COMPLETE,
        UE_rrc_inst[ctxt_pP->module_id].initialNasMsg.data);
}
...
static int decode_SIB1( const protocol_ctxt_t* const ctxt_pP, const uint8_t eNB_index, const uint8_t rsrq, const uint8_t rsrp )
{
  ...
  printf("decode_SIB1 2\n");

#if defined(ENABLE_ITTI) && defined(ENABLE_USE_MME)
  {
    int cell_valid = 0;
    printf("decode_SIB1 3\n");
    ...
    if (sib1->cellAccessRelatedInfo.cellBarred == SystemInformationBlockType1__cellAccessRelatedInfo__cellBarred_notBarred) {
      ...
      printf("decode_SIB1 4 plmn_number=%d\n", plmn_number);
      ...
      for (plmn = 0; plmn < plmn_number; plmn++) {
        ...
        printf("decode_SIB UE_rrc_inst mcc=%d  %d %d   [%d]\n", 
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MCCdigit1,
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MCCdigit2,
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MCCdigit3,
           plmn_Identity->mcc );

        if( plmn_Identity->mcc != NULL )
        {
            printf("decode_SIB plmn_Identity mcc=%d  %d %d  \n", 
                *(plmn_Identity->mcc->list.array[0]),
                 *(plmn_Identity->mcc->list.array[1]),
                  *(plmn_Identity->mcc->list.array[2]));
        }
        printf("decode_SIB UE_rrc_inst mnc=%d  %d %d  \n", 
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MNCdigit1,
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MNCdigit2,
           UE_rrc_inst[ctxt_pP->module_id].plmnID.MNCdigit3);

        printf("decode_SIB plmn_Identity mcc=%d  %d %d  count = %d \n", 
            *(plmn_Identity->mnc.list.array[0]),
             *(plmn_Identity->mnc.list.array[1]),
              (plmn_Identity->mnc.list.count==3)?*(plmn_Identity->mnc.list.array[2]):888,
              plmn_Identity->mnc.list.count);
//#if 0
        if (
          (
...
#warning "ue_mod_idP ? for instance ?"
  ...
  if (new_sib == 1) {
    ...
    {
      printf("decode_SI: set sub state to RRC_SUB_STATE_IDLE_SIB_COMPLETE\n");
