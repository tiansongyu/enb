...
//------------------------------------------------------------------------------
{
    ...
    //printf("rrc_eNB_S1AP_get_ue_ids:ue_initial_id=%d eNB_ue_s1ap_id=%d(=0x%x)\n",
    //    ue_initial_id, eNB_ue_s1ap_id,eNB_ue_s1ap_id);
    ...
    //xhd_oai_multuser insert "else"
    if (ue_initial_id == UE_INITIAL_ID_INVALID) 
    {
        printf("rrc_eNB_S1AP_get_ue_ids:ue_initial_id is invalid rnti=%x\n",result->ue_rnti);
        return result;
    }
    ...
    {
        printf("rrc_eNB_S1AP_get_ue_ids:ue_initial_id do not found rnti=%x\n",result->ue_rnti);
        return result;
    }

    if (eNB_ue_s1ap_id <= 0) 
    {
        printf("rrc_eNB_S1AP_get_ue_ids:eNB_ue_s1ap_id invalid, use ue_initial_id rnti=%x\n",result->ue_rnti);
        return result;
    }
    ...
    if  (h_rc == HASH_TABLE_OK) 
    {
        //printf("rrc_eNB_S1AP_get_ue_ids:eNB_ue_s1ap_id and ue_initial_id are all valid rnti=%x\n",result->ue_rnti);
        return result;
    }
    ...
    if (NULL == result2) 
    {
        printf("rrc_eNB_S1AP_get_ue_ids:malloc faild rnti=%x\n",result->ue_rnti);
        return result;
    }
    ...
    *result2 = *result;
    ...
    //printf("rrc_eNB_S1AP_get_ue_ids:malloc success, rnti=%x\n",result->ue_rnti);
    return result;
}
//------------------------------------------------------------------------------
void
...
)
{
  ...
  // translation
  LOG_D(RRC,
        "[eNB %d] NAS security_capabilities.encryption_algorithms %x AS ciphering_algorithm %u NAS security_capabilities.integrity_algorithms %x AS integrity_algorithm %u\n",
        ctxt_pP->module_id,
  ...
  LOG_D (RRC, "[eNB %d][UE %x] Selected security algorithms (%x): cipher:%x, integrity:%x, %s\n",
         ctxt_pP->module_id,
...
//------------------------------------------------------------------------------
{
#if defined(ENABLE_ITTI)
  {
    ...
    //xhd_oai_multuser
    LOG_I(S1AP, "[eNB %d] rrc_eNB_send_S1AP_NAS_FIRST_REQ: RNTI=%x ue_initial_id %u\n",
            ctxt_pP->module_id, 
            ue_context_pP->ue_context.rnti,
            ue_context_pP->ue_context.ue_initial_id);
    ...
    /* Fill UE identities with available information */
    {
      ...
      if (ue_context_pP->ue_context.Initialue_identity_s_TMSI.presence) {
        ...
        LOG_I(S1AP, "[eNB %d] Build S1AP_NAS_FIRST_REQ with s_TMSI: MME code %u M-TMSI %x ue %x\n",
            ctxt_pP->module_id,
            S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.mme_code,
            S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.m_tmsi,
            ue_context_pP->ue_context.rnti);
      }
      else if(0 != ue_context_pP->ue_context.mme_ue_id)
      {
          S1AP_NAS_FIRST_REQ (message_p).ue_identity.presenceMask |= UE_IDENTITIES_s_tmsi;
          S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.mme_code = 1;
          S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.m_tmsi = ue_context_pP->ue_context.mme_ue_id;
          LOG_I(S1AP, "[eNB %d] Build S1AP_NAS_FIRST_REQ with s_TMSI from ue_context: MME code %u M-TMSI %x ue %x\n",
              ctxt_pP->module_id,
              S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.mme_code,
              S1AP_NAS_FIRST_REQ (message_p).ue_identity.s_tmsi.m_tmsi,
              ue_context_pP->ue_context.rnti);
...
//------------------------------------------------------------------------------
{
  ...
  mme_ue_id = S1AP_DOWNLINK_NAS (msg_p).mme_ue_id;
  
  ue_context_p = rrc_eNB_get_ue_context_from_s1ap_ids(instance, ue_initial_id, eNB_ue_s1ap_id);
  LOG_D(RRC, "[eNB %d] Received %s: ue_initial_id %d, eNB_ue_s1ap_id %d instance=%d\n",
        instance,
        msg_name,
        ue_initial_id,
        eNB_ue_s1ap_id,
  ...
  if (ue_context_p == NULL) {
    ...
    //xhd_oai_multuser
    printf("rrc_eNB_process_S1AP_DOWNLINK_NAS: rnti=%x random id=0x%"PRIx64" mme_ue_id %x ==> %x\n", 
        ue_context_p->ue_context.rnti,
        ue_context_p->ue_context.random_ue_identity,
        ue_context_p->ue_context.mme_ue_id, mme_ue_id);
    ...
    MSC_LOG_RX_MESSAGE(
      ...
      (const char *)NULL,
      0,
      MSC_AS_TIME_FMT" DOWNLINK-NAS UE initial id %u eNB_ue_s1ap_id %x",
      0,0,//MSC_AS_TIME_ARGS(ctxt_pP),
      ue_initial_id,
...
void rrc_eNB_Record_eNB_ue_s1ap_id(instance_t instance, uint16_t  ue_initial_id, uint32_t eNB_ue_s1ap_id)
{
  struct rrc_eNB_ue_context_s* ue_context_p = NULL;
  ...
  if (ue_context_p != NULL) 
  {
    ...
    LOG_I(RRC, "[eNB %d] rrc_eNB_Record_eNB_ue_s1ap_id success: ue_initial_id %d, eNB_ue_s1ap_id %d(0x%x)\n",
          instance, ue_initial_id, eNB_ue_s1ap_id);
  }
  else
  {
      LOG_I(RRC, "[eNB %d] rrc_eNB_Record_eNB_ue_s1ap_id fail: ue_initial_id %d, eNB_ue_s1ap_id %d(0x%x)\n",
            instance, ue_initial_id, eNB_ue_s1ap_id);
  }
}
...
int rrc_eNB_process_S1AP_INITIAL_CONTEXT_SETUP_REQ(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  ...
  LOG_I(RRC, "[eNB %d] Received %s: ue_initial_id %d, eNB_ue_s1ap_id %d(0x%x), nb_of_e_rabs %d\n",
        instance, msg_name, ue_initial_id, eNB_ue_s1ap_id, eNB_ue_s1ap_id,S1AP_INITIAL_CONTEXT_SETUP_REQ (msg_p).nb_of_e_rabs);

  if (ue_context_p == NULL) {
    /* Can not associate this message to an UE index, send a failure to S1AP and discard it! */
    MessageDef *msg_fail_p = NULL;

    LOG_W(RRC, "[eNB %d] In S1AP_INITIAL_CONTEXT_SETUP_REQ: unknown UE from S1AP ids (%x, %d(0x%x))\n", 
        instance, ue_initial_id, eNB_ue_s1ap_id, eNB_ue_s1ap_id);
    ...
    //xhd_oai_multuser 记录UE的TMSI
    ue_context_p->ue_context.mme_ue_id = S1AP_INITIAL_CONTEXT_SETUP_REQ (msg_p).mme_ue_id;
...

    {
...
#endif
      //xhd_oai_multuser 对于SR与TAU不更新密钥
      if( ue_context_p->ue_context.security_capabilities.integrity_algorithms == 0xc000 )
      {
          printf("@@@This is Service Request!\n");
          rrc_pdcp_config_security(
            &ctxt,
            ue_context_p,
      ...
      else if( ue_context_p->ue_context.security_capabilities.integrity_algorithms == 0xe000 )
      {
          printf("@@@This is Trace Area Update!\n");
          rrc_pdcp_config_security(
            &ctxt,
            ue_context_p,
            send_security_mode_command);
      ...
      else if( ue_context_p->ue_context.security_capabilities.integrity_algorithms == 0x8000 )
      {
          printf("@@@This is Initial Setup!\n");
          rrc_pdcp_config_security(
            &ctxt,
            ue_context_p,
            send_security_mode_command);
