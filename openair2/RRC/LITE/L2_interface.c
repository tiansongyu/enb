  ...
  if( enb_flagP == ENB_FLAG_YES) {
      ...
      if(Srb_info->Tx_buffer.payload_size>0) { //Fill buffer
...
#if defined(ENABLE_ITTI)
        {
          ...
          if (ccch_size > sdu_size) {
            LOG_E(RRC, "SDU larger than CCCH SDU buffer size (%d, %d) AAA", ccch_size, sdu_size);
            ccch_size = sdu_size;
          }
    ...
    if( (UE_rrc_inst[Mod_idP].Srb0[eNB_index].Tx_buffer.payload_size > 0) ) {
...
#if defined(ENABLE_ITTI)
      {
        ...
        if (ccch_size > sdu_size) {
          LOG_E(RRC, "SDU larger than CCCH SDU buffer size (%d, %d) BBB", ccch_size, sdu_size);
          ccch_size = sdu_size;
        }
...
//--------------------------------------------------------------------------
{
  ...
  if(eNB_flagP == ENB_FLAG_NO) {
    ...
    if((srb_idP & RAB_OFFSET) == CCCH) {
      if (sdu_lenP>0) {
...
#if defined(ENABLE_ITTI)
        {
          ...
          if (sdu_lenP > msg_sdu_size) {
            LOG_E(RRC, "SDU larger than CCCH SDU buffer size (%d, %d) CCC", sdu_size, msg_sdu_size);
            sdu_size = msg_sdu_size;
          } else {
            sdu_size =  sdu_lenP;
          }
...
#if defined(ENABLE_ITTI)
    {
      ...
      if (sdu_lenP > msg_sdu_size) {
        LOG_E(RRC, "SDU larger than CCCH SDU buffer size (%d, %d) DDD", sdu_lenP, msg_sdu_size);
        sdu_size = msg_sdu_size;
      } else {
        sdu_size = sdu_lenP;
      }
...
//------------------------------------------------------------------------------
{
  ...
  if (ctxt_pP->enb_flag == ENB_FLAG_NO) {
    LOG_D(RRC, "[UE %x] Frame %d: received a DCCH %d message on SRB %d with Size %d from eNB ???\n",
