...
void initiate_ra_proc2(module_id_t module_idP,int CC_id,frame_t frameP, 
    uint16_t preamble_index,int16_t timing_offset,uint8_t sect_id,sub_frame_t subframe,uint8_t f_id, int16_t rnti);
...
@param preamble_index index of the received RA request.
*/
void cancel_ra_proc2(module_id_t module_idP,int CC_id,frame_t frameP, uint16_t preamble_index);
...
*/
void rx_sdu(const module_id_t module_idP, const int CC_id,const frame_t frameP, const sub_frame_t subframeP, const rnti_t rnti, 
    uint8_t *sdu, const uint16_t sdu_len, const int harq_pid,uint8_t *msg3_flag,rnti_t *crnti,uint8_t ucRound,
  uint8_t           ucFirstRb,
  uint8_t           ucRbNum);
...
#define  UE_RNTI(mod_idP, ue_idP) UE_RNTI_base(mod_idP,  ue_idP, __FILE__,  __LINE__)
...
uint8_t UE_is_to_be_scheduled(module_id_t module_idP,int CC_id,uint8_t UE_id,unsigned char sched_subframe);
...
@returns 0 for no SR, 1 for SR
*/
void ue_get_sdu(module_id_t module_idP, int CC_id,frame_t frameP, sub_frame_t subframe, uint8_t eNB_index,uint8_t *ulsch_buffer,uint16_t buflen,uint8_t *access_mode, int *pSdu_len, int *pOffset);
...
int mac_init(void);
int add_new_ue(module_id_t Mod_id, int CC_id, rnti_t rnti,int harq_pid,int frameP, sub_frame_t subframeP);
...
void ulsch_scheduler_pre_processor(module_id_t module_idP, int frameP, sub_frame_t subframeP, unsigned char sched_subframe,uint16_t *first_rb, uint8_t  aggregattion, uint32_t *nCCE);
...
void update_ul_dci(module_id_t module_idP,uint8_t CC_id,rnti_t rnti,uint8_t dai, sub_frame_t subframeP);
void update_ul_dci50(module_id_t module_idP,uint8_t CC_id,rnti_t rnti,uint8_t dai, sub_frame_t subframeP);
void update_ul_dci100(module_id_t module_idP,uint8_t CC_id,rnti_t rnti,uint8_t dai, sub_frame_t subframeP);
