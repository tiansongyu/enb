...
//------------------------------------------------------------------------------
{
  ...
  for (i=0; i<num_sdu; i++) {
    ...
    if (rx_lcids[i] == CCCH) {
      ...
      LOG_I(MAC,"[UE %d] rnti %x Frame %d : DLSCH -> DL-CCCH, RRC message (eNB %d, %d bytes)\n",
            module_idP,
   ...
   , int *pSduLen, int *pOffset)
{
  ...
  if (UE_mac_inst[module_idP].scheduling_info.LCID_status[DCCH] == LCID_NOT_EMPTY) {
    ...
    sdu_lcids[0] = DCCH;
    LOG_D(MAC,"[UE %d] frame:%d subframe:%d TX Got %d bytes for DCCH\n",module_idP,frameP,subframe,sdu_lengths[0]);
    num_sdus = 1;
  ...
  if (UE_mac_inst[module_idP].scheduling_info.LCID_status[DCCH1] == LCID_NOT_EMPTY) {
    ...
    LOG_W(MAC,"[UE %d] TX Got %d bytes for DCCH1\n",module_idP,sdu_lengths[num_sdus]);
    num_sdus++;
  ...
  if ((UE_mac_inst[module_idP].scheduling_info.LCID_status[DTCH] == LCID_NOT_EMPTY) &&
    ...
    LOG_D(MAC,"[UE %d] frame:%d subframe:%d  TX Got %d bytes for DTCH\n",module_idP,frameP,subframe,sdu_lengths[num_sdus]);
  ...
  *pSduLen = sdu_length_total;
  ...
  {
      for (j=0; j<(buflen-payload_offset); j++) 
      {
        ulsch_buffer[payload_offset+j] = (char)(taus()&0xff);//0x00;
      }
  }
...
//------------------------------------------------------------------------------
{
  ...
  do {
    count++;
    //printf("ue_scheduler count=%d\n",count);
      LOG_RX_LINE();

    // Checks if a message has been sent to MAC sub-task
    itti_poll_msg_ue(TASK_MAC_UE, &msg_p);
    LOG_RX_LINE();
  ...
  if (UE_mac_inst[module_idP].RA_contention_resolution_timer_active == 1) {
    ...
    if( UE_mac_inst[module_idP].RA_contention_resolution_cnt % 10 == 0)
    {
