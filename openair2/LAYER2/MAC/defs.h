...
//xhd_oai_20users
//#define NB_RA_PROC_MAX 16
#define NB_RA_PROC_MAX NUMBER_OF_UE_MAX
...
typedef struct {
  ...
  uint64_t sdu_bytes_per_frame;//xhd_oai_test
  // total MAC pdu bytes
...
typedef struct {
  ...
  uint8_t DLSCH_DCI[8][(MAX_DCI_SIZE_BITS>>3)+3];
  ...
  //xhd_oai_multuser
  uint16_t sche_ul; // num_max_harq
  uint16_t first_rb_ul[8]; // num_max_harq
  uint16_t round_ul[8]; // num_max_harq
  uint16_t mcs[8]; // num_max_harq
  /// Number of Allocated RBs by the ulsch preprocessor
  uint8_t pre_allocated_nb_rb_ul[10];

  /// index of Allocated RBs by the ulsch preprocessor
  int8_t pre_allocated_rb_table_index_ul[10];

  /// total allocated RBs
  //int8_t total_allocated_rbs;

  /// pre-assigned MCS by the ulsch preprocessor
  uint8_t pre_assigned_mcs_ul[10];
  ...
  uint8_t ul_SR;
  uint8_t ul_SR_subframe;
  uint8_t ul_SR_idle[10];
  uint8_t ul_SR_idle_exe[10];
  uint8_t ul_SR_flag[8];
  ...
  uint32_t dl_buffer_total[10];
  /// total downlink pdus
  uint32_t dl_pdus_total[10];
...
typedef struct {
  ...
  uint16_t remain_1[MAX_NUM_CCs];
  uint16_t nb_rbs_required[MAX_NUM_CCs];
  uint16_t remain_1b[MAX_NUM_CCs];
  uint16_t nb_rbs_requiredb[MAX_NUM_CCs];

    // resource scheduling information
    uint8_t       harq_pid_ul[MAX_NUM_CCs];
    uint8_t       round_ul[MAX_NUM_CCs];

    uint8_t       max_round_ul;
...
typedef struct {
  ...
  DCI_PDU DCI_pdu[10]; //xhd_oai_60m
...
typedef struct {
  ...
  UE_info_t UE_info[NUMBER_OF_UE_MAX];
  UE_list_t UE_list[10];
