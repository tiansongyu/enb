                         ...
                         uint8_t            first_sync)   //init as MR
{
  ...
  printf("dl_phy_sync_success: AAA");
  if (first_sync==1) {
	  printf("dl_phy_sync_success: AAA1");
...
#endif
  {
	//printf("dl_phy_sync_success: BBB");
        if (first_sync==1) {
               printf("dl_phy_sync_success: BBB1");
               layer2_init_UE(module_idP);
               openair_rrc_ue_init(module_idP,eNB_index);
        } 
...
int mac_top_init(int eMBMS_active, char *uecap_xer, uint8_t cba_group_active, uint8_t HO_active)
{
  ...
  for (i=0; i<NB_eNB_INST; i++)
    for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
      ...
      {
            UE_template = (UE_TEMPLATE *)&eNB_mac_inst[i].UE_info[j].UE_template[CC_id];
            UE_template->rnti=0;
            // initiallize the eNB to UE statistics
            memset (&eNB_mac_inst[i].UE_info[j].eNB_UE_stats[CC_id],0,sizeof(eNB_UE_STATS));
      }
    }
...
int l2_init(LTE_DL_FRAME_PARMS *frame_parms,int eMBMS_active, char *uecap_xer,uint8_t cba_group_active, uint8_t HO_active)
{
  ...
  mac_xface->initiate_ra_proc          = initiate_ra_proc2;
  mac_xface->cancel_ra_proc            = cancel_ra_proc2;
  ...
  //xhd_oai_enb
  mac_xface->phy_remove_ue          = phy_remove_ue;
  mac_xface->phy_remain_ue          = phy_remain_ue;
