    ...
    for (i=0; i<NB_RA_PROC_MAX; i++) {
      ...
      if (RA_template[i].RA_active == TRUE) {
        ...
        if (RA_template[i].generate_rar == 1) {
          ...
          if (rrc_sdu_length>0) {
            LOG_D(MAC,"[eNB %d][RAPROC] CC_id %d Frame %d, subframeP %d: Generating Msg4 with RRC Piggyback (RA proc %d, RNTI %x)\n",
            ...
            offset = generate_dlsch_header((unsigned char*)eNB->UE_info[(unsigned char)UE_id].DLSCH_pdu[CC_id][subframeP&1].payload[0],
                                           1,                           //num_sdus
            ...
            memcpy((void*)&eNB->UE_info[(unsigned char)UE_id].DLSCH_pdu[CC_id][subframeP&1].payload[0][(unsigned char)offset],
                   &eNB->common_channels[CC_id].CCCH_pdu.payload[0],
                   rrc_sdu_length);

            if (opt_enabled==1) {
              trace_pdu(1, (uint8_t *)eNB->UE_info[(unsigned char)UE_id].DLSCH_pdu[CC_id][subframeP&1].payload[0],
                      ...
                      uint8_t f_id)
{
  ...
  printf("initiate_ra_proc: max user reached!\n");
...
void cancel_ra_proc(module_id_t module_idP, int CC_id, frame_t frameP, rnti_t rnti)
{
  ...
  //xhd_oai_60m
  //LOG_E(MAC,"[eNB %d][RAPROC] CC_id %d Frame %d Cancelling RA procedure for UE rnti %x\n",module_idP,CC_id,frameP,rnti);
...
void initiate_ra_proc2(module_id_t module_idP, int CC_id,frame_t frameP, 
                      uint16_t preamble_index,int16_t timing_offset,uint8_t sect_id,sub_frame_t subframeP,
                      uint8_t f_id, int16_t rnti)
{
    ...
    int16_t UE_Id = RNTI2UEID(rnti);
    if( UE_Id >= NB_RA_PROC_MAX )
    {
      printf("initiate_ra_proc2: max user reached!\n");
      return;
    }
    RA_template[UE_Id].RA_active=TRUE;
    RA_template[UE_Id].generate_rar=1;
    RA_template[UE_Id].generate_Msg4=0;
    RA_template[UE_Id].wait_ack_Msg4=0;
    RA_template[UE_Id].timing_offset=timing_offset;
    // Put in random rnti (to be replaced with proper procedure!!)
    RA_template[UE_Id].rnti = rnti;
    RA_template[UE_Id].RA_rnti = 1+subframeP+(10*f_id);
    RA_template[UE_Id].preamble_index = preamble_index;
    LOG_D(MAC,"[eNB %d][RAPROC] CC_id %d Frame %d Activating RAR generation for process %d, rnti %x, RA_active %d\n",
        module_idP,CC_id,frameP,UE_Id,RA_template[UE_Id].rnti,
        RA_template[UE_Id].RA_active);

    return;
}

void cancel_ra_proc2(module_id_t module_idP, int CC_id, frame_t frameP, rnti_t rnti)
{
    unsigned char i;
    RA_TEMPLATE *RA_template = (RA_TEMPLATE *)&eNB_mac_inst[module_idP].common_channels[CC_id].RA_template[0];
    ...
    MSC_LOG_EVENT(MSC_PHY_ENB, "RA Cancelling procedure ue %"PRIx16" ", rnti);
    //LOG_E(MAC,"[eNB %d][RAPROC] CC_id %d Frame %d Cancelling RA procedure for UE rnti %x\n",module_idP,CC_id,frameP,rnti);
    int UE_Id = RNTI2UEID(rnti);
    if( UE_Id > NB_RA_PROC_MAX )
    {
        printf("cancel_ra_proc2: too big UE_Id of rnti %x\n",rnti);
        return;
    }

    RA_template[UE_Id].RA_active=FALSE;
    RA_template[UE_Id].generate_rar=0;
    RA_template[UE_Id].generate_Msg4=0;
    RA_template[UE_Id].wait_ack_Msg4=0;
    RA_template[UE_Id].timing_offset=0;
    RA_template[UE_Id].RRC_timer=20;
    RA_template[UE_Id].rnti = 0;
}

void cancel_ra_proc_of_others(module_id_t module_idP, int CC_id,  rnti_t rnti)
{
  unsigned char i;
  RA_TEMPLATE *RA_template = (RA_TEMPLATE *)&eNB_mac_inst[module_idP].common_channels[CC_id].RA_template[0];
  ...
  for (i=0; i<NB_RA_PROC_MAX; i++) {
    if (rnti != RA_template[i].rnti 
       && RA_template[i].RA_active==TRUE) 
  {
      printf("cancel_ra_proc_of_others, cancel ra proc of rnti %x for rnti %x\n",RA_template[i].rnti, rnti);
      RA_template[i].RA_active=FALSE;
      RA_template[i].generate_rar=0;
      RA_template[i].generate_Msg4=0;
      RA_template[i].wait_ack_Msg4=0;
...
void clear_ra_proc(module_id_t module_idP, int CC_id)
{
  unsigned char i;
  RA_TEMPLATE *RA_template = (RA_TEMPLATE *)&eNB_mac_inst[module_idP].common_channels[CC_id].RA_template[0];

  printf("clear_ra_proc\n");
