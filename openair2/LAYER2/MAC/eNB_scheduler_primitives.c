...
//------------------------------------------------------------------------------
{
  ...
  return(&eNB_mac_inst[module_idP].common_channels[CC_id].DCI_pdu[subframeP]);
...
//------------------------------------------------------------------------------
{
  ...
  int UE_id= RNTI2UEID(rntiP);
  UE_info_t *UE_info = &eNB_mac_inst[mod_idP].UE_info[UE_id];
  
  if( UE_id < NUMBER_OF_UE_MAX)
  {
      if (UE_info->UE_template[UE_PCCID(mod_idP,UE_id)].rnti==rntiP) 
      {
        return(UE_id);
      }
  }
...
#if 0
//------------------------------------------------------------------------------
int find_UE_id_by_old_crnti(module_id_t mod_idP, rnti_t rntiP)
//------------------------------------------------------------------------------
{
  ...
  for (UE_id=UE_list->head; UE_id>=0; UE_id=UE_list->next[UE_id]) {
    if (UE_list->UE_template[UE_PCCID(mod_idP,UE_id)][UE_id].oldRnti==rntiP) {
      return(UE_id);
    }
  }
...
//------------------------------------------------------------------------------
int UE_num_active_CC(UE_info_t *infoP)
//------------------------------------------------------------------------------
{
  return(infoP->numactiveCCs);
}
...
//------------------------------------------------------------------------------
{
  return(eNB_mac_inst[mod_idP].UE_info[ue_idP].pCC_id);
}

//------------------------------------------------------------------------------
rnti_t UE_RNTI_base(module_id_t mod_idP, int ue_idP, char *file, int line)
//------------------------------------------------------------------------------
{

  rnti_t rnti = eNB_mac_inst[mod_idP].UE_info[ue_idP].UE_template[UE_PCCID(mod_idP,ue_idP)].rnti;

  if (rnti>0) {
    return (rnti);
  }

  LOG_E(MAC,"[eNB %d] Couldn't find RNTI for UE %d  file:%s line:%d\n",mod_idP,ue_idP,file,line);
  //display_backtrace();
  return(NOT_A_RNTI);
}
...
//------------------------------------------------------------------------------
{
  return(eNB_mac_inst[mod_idP].UE_info[ue_idP].active);
}
...
{
  //xhd_oai_multuser
  //xhd_oai_ue aggregation
  unsigned char aggregation=0;//0; //1; //2;
...
#if 0
int add_new_ue_single(module_id_t mod_idP, int cc_idP, rnti_t rntiP,int harq_pidP)
{
  int UE_id;
  int j;
  ...
  //xhd_oai_enb
  mac_xface->phy_remain_ue(mod_idP,cc_idP,rntiP);
  ...
  if (UE_list->avail>=0) {
    ...
    UE_list->avail = 0; //UE_list->next[UE_list->avail];
    UE_list->next[UE_id] = -1; //UE_list->head;
    UE_list->next_ul[UE_id] = -1; //UE_list->head_ul;
    UE_list->head = UE_id;
...
void mac_dump_ue( uint8_t *msg)
{
    UE_list_t *UE_list = &eNB_mac_inst[0].UE_list;
    int j = 0;

    printf("%s DL :",msg);
    for (int i=UE_list->head; i>=0; i=UE_list->next[i]) 
    {
      //printf("%d(0x%x) ",i,UE_list->UE_template[0][i].rnti);
      printf("%d ",i);
      if( j++ > 20)
      {
          break;
    ...
    printf(" UL :");
    j = 0;
    for (int i=UE_list->head_ul; i>=0; i=UE_list->next_ul[i]) 
    {
      printf("%d ",i);
      if( j++ > 20)
      {
...
int add_new_ue(module_id_t mod_idP, int cc_idP, rnti_t rntiP,int harq_pidP, int frameP, sub_frame_t subframeP)
{
  int UE_id;
  int j;
  ...
  //mac_dump_ue("######add_new_ue:  before ");

  //xhd_oai_20users
  UE_id = RNTI2UEID(rntiP); //get_phy_ueid_by_rnti(rntiP);
  if( UE_id >= NUMBER_OF_UE_MAX )
  {
      return(-1);
  }
  UE_info_t *UE_info = &eNB_mac_inst[mod_idP].UE_info[UE_id];
  if( UE_info->active == TRUE)
  {
      mac_remove_ue(mod_idP, UE_id,0,0);
      //mac_dump_ue("######add_new_ue:  clean ");
      //return UE_id;
  }
  ...
  //  UE_id = UE_list->avail;
    ...
    //UE_list->head = UE_id;
    //UE_list->head_ul = UE_id;
    UE_info->UE_template[cc_idP].rnti       = rntiP;
    UE_info->UE_template[cc_idP].configured = FALSE;
    UE_info->numactiveCCs                   = 1;
    UE_info->numactiveULCCs                 = 1;
    UE_info->pCC_id                         = cc_idP;
    UE_info->ordered_CCids[0]               = cc_idP;
    UE_info->ordered_ULCCids[0]             = cc_idP;
    //UE_info->num_UEs++;
    ...
    for (j=0; j<8; j++) {
      UE_info->UE_template[cc_idP].oldNDI[j]    = (j==0)?1:0;   // 1 because first transmission is with format1A (Msg4) for harq_pid 0
      UE_info->UE_template[cc_idP].oldNDI_UL[j] = (j==harq_pidP)?0:1; // 1st transmission is with Msg3;
    }

    eNB_ulsch_info[mod_idP][cc_idP][UE_id].status = S_UL_WAITING;
    eNB_dlsch_info[mod_idP][cc_idP][UE_id].status = S_DL_WAITING;

    //mac_dump_ue("######add_new_ue:  after ");
...
//------------------------------------------------------------------------------
{
  ...
  rnti_t  rnti;
  UE_info_t *UE_info = &eNB_mac_inst[mod_idP].UE_info[ue_idP];
  int pCC_id = UE_PCCID(mod_idP,ue_idP);

  rnti = UE_info->UE_template[pCC_id].rnti;
  LOG_D(MAC,"Removing UE %d from Primary CC_id %d (rnti %x)\n",ue_idP,pCC_id, rnti);
  //dump_ue_list(UE_list,0);

  // clear all remaining pending transmissions
  UE_info->UE_template[pCC_id].bsr_info[LCGID0]  = 0;
  UE_info->UE_template[pCC_id].bsr_info[LCGID1]  = 0;
  UE_info->UE_template[pCC_id].bsr_info[LCGID2]  = 0;
  UE_info->UE_template[pCC_id].bsr_info[LCGID3]  = 0;
  UE_info->UE_template[pCC_id].bsr_zero_flag = 0;

  UE_info->UE_template[pCC_id].ul_SR             = 0;
  UE_info->UE_template[pCC_id].rnti              = NOT_A_RNTI;
  UE_info->UE_template[pCC_id].ul_active         = FALSE;
  ...
  printf("######mac_remove_ue:ue %d is to be removed\n",ue_idP);
  mac_dump_ue("######mac_remove_ue: before ");
  ...
  //mac_dump_ue("######mac_remove_ue: after ");
...
//------------------------------------------------------------------------------
int mac_remove_ue_by_rnti(module_id_t mod_idP, int rnti)
//------------------------------------------------------------------------------
{
  ...
  ue_idP = RNTI2UEID(rnti); //get_phy_ueid_by_rnti(rntiP);
  if( ue_idP >= NUMBER_OF_UE_MAX )
  {
      return(-1);
  }
  ...
  printf("######mac_remove_ue_by_rnti:ue %d is to be removed\n",ue_idP);
  mac_dump_ue("######mac_remove_ue_by_rnti: before ");
  for (i=UE_list->head; i>=0; i=UE_list->next[i]) {
    if (i == ue_idP) {
      // link prev to next in Active list
      //if (prev==UE_list->head)
  ...
  mac_dump_ue("######mac_remove_ue_by_rnti: after ");

  if (ret == 0) {
    return (0);
  }

  LOG_E(MAC,"error in mac_remove_ue_by_rnti(), could not find previous to %d in UE_list, should never happen, Dumping UE list\n",ue_idP);
  dump_ue_list(UE_list,0);
  //mac_xface->macphy_exit("mac_remove_ue: Problem in UE_list");
  return(-1);
  #endif
...
void swap_UEs(UE_list_t *listP,int nodeiP, int nodejP, int ul_flag)
{
  ...
  if ((prev_i<0) || (prev_j<0)) {
      LOG_E(MAC,"swap_UEs:nodeiP=%d nodejP=%d in ue list %s failed!\n",
            nodeiP, nodejP,
            (ul_flag == 0)? "DL" : "UL");
      mac_dump_ue("######prev: ");
...
void SR_indication(module_id_t mod_idP, int cc_idP, frame_t frameP, rnti_t rntiP, sub_frame_t subframeP)
{
  ...
  if (UE_id  != -1) {
    ...
    UE_info->UE_template[cc_idP].ul_SR = 1;
    UE_info->UE_template[cc_idP].ul_SR_subframe = subframeP;
    UE_info->UE_template[cc_idP].ul_active = TRUE;
  } else {
...
//xhd_oai_20users
void SR_indication_idle(module_id_t mod_idP, int cc_idP, frame_t frameP, rnti_t rntiP, sub_frame_t subframeP)
{
  ...
  {
      UE_info_t *UE_info = &eNB_mac_inst[mod_idP].UE_info[UE_id];
      if( UE_info->UE_template[cc_idP].ul_SR_idle_exe[subframeP] != 1)
      {
          UE_info->UE_template[cc_idP].ul_SR_idle[(subframeP+4)%10] = 1;
          phy_add_count_subframe(rntiP,255,1,(subframeP+4)%10);
      }
      else
      {
          UE_info->UE_template[cc_idP].ul_SR_idle[(subframeP+4)%10] = 0;
          phy_add_count_subframe(rntiP,255,2,(subframeP+4)%10);
      }
  }
}
...
void add_ue_spec_dci(DCI_PDU *DCI_pdu,void *pdu,rnti_t rnti,unsigned char dci_size_bytes,unsigned char aggregation,
                          unsigned char dci_size_bits,unsigned char dci_fmt,uint8_t ra_flag)
{

  int dci_count = DCI_pdu->Num_common_dci+DCI_pdu->Num_ue_spec_dci;
  
  memcpy(&DCI_pdu->dci_alloc[dci_count].dci_pdu[0],pdu,dci_size_bytes);
  
  DCI_pdu->dci_alloc[dci_count].dci_length = dci_size_bits;
  DCI_pdu->dci_alloc[dci_count].L          = aggregation;
  DCI_pdu->dci_alloc[dci_count].rnti       = rnti;
  DCI_pdu->dci_alloc[dci_count].format     = dci_fmt;
  DCI_pdu->dci_alloc[dci_count].ra_flag    = ra_flag;

  DCI_pdu->Num_ue_spec_dci++;

  return;
  
  uint16_t usUeId = get_phy_ueid_by_rnti(rnti);
  if( usUeId <= 2)
  {
      if( PHY_vars_eNB_g[0][0]->ulsch_eNB[usUeId]->print_rb_flag < 100 )
      {
          LOG_I(MAC,"add ue specific dci format %d for PHY UeId %d rnti %x Num_common_dci=%d Num_ue_spec_dci=%d dci_size_bytes=%d\n",
            dci_fmt,usUeId,rnti, 
            DCI_pdu->Num_common_dci,
            DCI_pdu->Num_ue_spec_dci,
            dci_size_bytes);
...
uint8_t UE_is_to_be_scheduled(module_id_t module_idP,int CC_id,uint8_t UE_id,unsigned char subframe)
{

  UE_TEMPLATE *UE_template = &eNB_mac_inst[module_idP].UE_info[UE_id].UE_template[CC_id];

  //  LOG_D(MAC,"[eNB %d][PUSCH] Frame %d subframeP %d Scheduling UE %d\n",module_idP,rnti,frameP,subframeP,
  //  UE_id);
  static int count = 0;
  UE_template->ul_SR_idle_exe[subframe] = 0;
  ...
  { 
    ...
    if( ++count == 1024)
    {
        printf("UE_is_to_be_scheduled: UE_id=%d subframe=%d ul_SR_subframe=%d\n", 
            UE_id, subframe, UE_template->ul_SR_subframe);
        count = 0;
    }
    return(1);
  } 
  else if (UE_template->ul_SR_idle[subframe]>0 ) //xhd_oai_20users
  { 
    // uplink scheduling request
    if( ++count == 1024)
    {
        printf("UE_is_to_be_scheduled: 22222 UE_id=%d subframe=%d ul_SR_subframe=%d\n", 
            UE_id, subframe, UE_template->ul_SR_subframe);
        count = 0;
    }
    UE_template->ul_SR_idle_exe[subframe]=1;
    return(1);
  } 
  else 
  {
...
// This has to be updated to include BSR information
uint8_t UE_is_to_be_scheduled_SR(module_id_t module_idP,int CC_id,uint8_t UE_id,unsigned char subframe)
{
  ...
  if(count ++ == 1000)
  {
      LOG_I(MAC,"[eNB %d]  UE %d CC id %d  subframe =%d ul_SR=%d\n",
            module_idP, CC_id, UE_id, subframe, UE_template->ul_SR);
      count = 0;
  }
  #endif
...
// This has to be updated to include BSR information
uint8_t UE_is_to_be_scheduled_SR_Idle(module_id_t module_idP,int CC_id,uint8_t UE_id,unsigned char subframe, unsigned char sched_subframe)
{
  ...
  //  LOG_D(MAC,"[eNB %d][PUSCH] Frame %d subframeP %d Scheduling UE %d\n",module_idP,rnti,frameP,subframeP,
  //  UE_id);
  if (UE_template->ul_SR_idle[subframe]>0 ) //xhd_oai_20users
  { 
    // uplink scheduling request
    UE_template->ul_SR_idle_exe[sched_subframe]=1;
    UE_template->ul_SR_idle[subframe] = 0;
    return(1);
  } 
  else 
  {
...
uint32_t allocate_prbs_sub(int nb_rb, uint8_t *rballoc)
{
  ...
  //xhd_oai_multuser
  //如果RB分配不是连续的，这样的处理是否有问题?
  nb_rb = mac_xface->lte_frame_parms->N_RB_DL; //only for test
