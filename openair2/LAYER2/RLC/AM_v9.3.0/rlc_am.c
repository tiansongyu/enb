  ...
  case RLC_DATA_TRANSFER_READY_STATE:
    ...
    if (rlc_pP->first_retrans_pdu_sn >= 0) 
    {
      ...
      while ((rlc_pP->nb_bytes_requested_by_mac > 2) &&
        ...
        if ((tx_data_pdu_management->header_and_payload_size <= rlc_pP->nb_bytes_requested_by_mac) 
            && (tx_data_pdu_management->retx_count >= 0)
            && (tx_data_pdu_management->nack_so_start == 0) 
            && (tx_data_pdu_management->nack_so_stop == 0x7FFF)) 
        {
          ...
          if( tx_data_pdu_management->retx_count <= 3 )
          {
              rlc_send_set[tx_data_pdu_management->retx_count +1]++;
          }
        ...
        else if ((tx_data_pdu_management->retx_count >= 0) 
            && (rlc_pP->nb_bytes_requested_by_mac >= RLC_AM_MIN_SEGMENT_SIZE_REQUEST)) 
        {
    ...
    if ((rlc_pP->nb_bytes_requested_by_mac > 2) && (rlc_pP->vt_s != rlc_pP->vt_ms)) 
    {
      ...
      if (rlc_pP->pdus_to_mac_layer.head != NULL) 
      {
        ...
        //xhd_oai_multuser RLC send count
        phy_add_count(ctxt_pP->rnti, 14);
  ...
  struct mac_status_ind        tx_statusP)
{
  ...
  if( rlc->input_sdus == NULL )
  {
  ...
  else  if ((rlc->input_sdus[rlc->current_sdu_index].mem_block != NULL) && (status_resp.buffer_occupancy_in_bytes)) {
    ...
    status_resp.head_sdu_creation_time = (diff_time > 0 ) ? (uint32_t) diff_time :  (uint32_t)(1024 + diff_time);
...
//xhd_oai_50
int sdu_recv_count = 0;
int sdu_recv_bytes = 0;
extern void phy_add_count(uint16_t rnti, uint16_t usCountId);
  ...
  mem_block_t * const         sdu_pP)
{
  ...
  if ((l_rlc_p->input_sdus[l_rlc_p->next_sdu_index].mem_block == NULL) &&
      ...
      (((l_rlc_p->next_sdu_index + 1) % RLC_AM_SDU_CONTROL_BUFFER_SIZE) 
...
#   endif
    ...
    //xhd_oai_50
    sdu_recv_count++;
    sdu_recv_bytes+=data_size;

    //xhd_oai_multuser SDU Receive count from gtpu
    //phy_add_count(ctxt_pP->rnti, 12);
    ...
    LOG_D(RLC, PROTOCOL_RLC_AM_CTXT_FMT" RLC_AM_DATA_REQ size %d Bytes,  NB SDU %d current_sdu_index=%d next_sdu_index=%d conf %d mui %d\n",
          PROTOCOL_RLC_AM_CTXT_ARGS(ctxt_pP,l_rlc_p),
          data_size,
          l_rlc_p->nb_sdu,
          l_rlc_p->current_sdu_index,
  ...
  else 
  {
	...
	//xhd_oai_50
	//SDU缓存满，可能是下行不畅，丢弃新来的包即可，不必要退出程序
	#if 0
