...
//xhd_oai_50
int rlc_send_count = 0;
int rlc_resend_count = 0;
int rlc_ack_count = 0;
int rlc_nack_count = 0;
int rlc_timeout_count = 0;
int rlc_reack_count = 0;
int rlc_renack_count = 0;
int rlc_cancel_count = 0;
int rlc_send_set[5] = {0};
int rlc_ack_set[5] = {0};
int rlc_nack_set[5] = {0};
int rlc_timeout_set[5] = {0};

int sdu_send_count;
int sdu_send_bytes;
int sdu_cancel_count;
int sdu_cancel_bytes;
  ...
  int timeout_flag)
{
  ...
  if (mb_p != NULL) {
	...
	if(rlc_pP->pdu_retrans_buffer[snP].retx_count<5)
	{
        if( timeout_flag )
        {
    		rlc_timeout_set[rlc_pP->pdu_retrans_buffer[snP].retx_count]++;
		}
		else
		{
    		rlc_nack_set[rlc_pP->pdu_retrans_buffer[snP].retx_count]++;
		}
	}

    if (rlc_pP->pdu_retrans_buffer[snP].retx_count >= rlc_pP->max_retx_threshold) 
	{
	  //xhd_oai_50 ´óÓÚ×î´óÖØ´«´ÎÊý
	  rlc_cancel_count++;
      ...
      for (pdu_sdu_index = 0; pdu_sdu_index < rlc_pP->pdu_retrans_buffer[snP].nb_sdus; pdu_sdu_index++) {
        ...
        if (rlc_pP->input_sdus[sdu_index].nb_pdus_ack == rlc_pP->input_sdus[sdu_index].nb_pdus) {
          ...
          sdu_cancel_count ++;
          sdu_cancel_bytes += rlc_pP->input_sdus[sdu_index].sdu_size;
  ...
  const rlc_sn_t snP)
{
  ...
  if ((rlc_pP->pdu_retrans_buffer[snP].flags.ack == 0) && (mb_p != NULL)) 
  {
	...
	{
      rlc_pP->retrans_num_bytes_to_retransmit -= rlc_pP->pdu_retrans_buffer[snP].header_and_payload_size;
	  if(rlc_pP->pdu_retrans_buffer[snP].retx_count<4)
	  { 
	  	  rlc_ack_set[rlc_pP->pdu_retrans_buffer[snP].retx_count+1]++;
	  }
	  rlc_reack_count++;
    }
	else
    ...
    for (pdu_sdu_index = 0; pdu_sdu_index < rlc_pP->pdu_retrans_buffer[snP].nb_sdus; pdu_sdu_index++) {
          ...
          (rlc_pP->input_sdus[sdu_index].sdu_remaining_size == 0)) 
      {
        ...
        sdu_send_count ++;
        sdu_send_bytes += rlc_pP->input_sdus[sdu_index].sdu_size;

        //xhd_oai_multuser SDU send success
        //phy_add_count(ctxt_pP->rnti, 13);
    ...
    //xhd_oai_50, ÖÃÉÏACK±êÖ¾£¬Ó¦´ð´ÎÊýµÝÔö
    rlc_pP->pdu_retrans_buffer[snP].flags.ack = 1;

    if (snP == rlc_pP->vt_a) 
	{
      //rlc_pP->pdu_retrans_buffer[snP].flags.ack = 1;
      //Èç¹û¸ÕºÃµÈÓÚvt_a£¬ÔòÐèÒªÒÆ¶¯´°¿Ú£¬¿ÉÄÜÊÇÁ¬ÐøÒÆ¶¯ºÃ¼¸¸ö
      do {
    ...
    if (snP == rlc_pP->first_retrans_pdu_sn) {
      do {
        ...
        if (rlc_pP->pdu_retrans_buffer[rlc_pP->first_retrans_pdu_sn].retx_count >= 0) {
		  //ÕÒµ½Ò»¸öÐèÒªÖØ´«µÄPDU£¬¾Í²»ÐèÒªÔÙ¼ÌÐø²éÕÒÁË
  ...
  rlc_am_entity_t* const rlc_pP)
{
  ...
  while (sn != sn_end) 
  {
    if (rlc_pP->pdu_retrans_buffer[sn].mem_block != NULL) 
    {
      ...
      if (rlc_pP->pdu_retrans_buffer[sn].header_and_payload_size <= rlc_pP->nb_bytes_requested_by_mac) 
      {
...
#endif
        rlc_am_nack_pdu (ctxt_pP, rlc_pP, sn, 0, 0x7FFF, 1);
        ...
        //xhd_oai_50
        rlc_resend_count++;
        if(rlc_pP->pdu_retrans_buffer[sn].retx_count<4)
        {
            rlc_send_set[rlc_pP->pdu_retrans_buffer[sn].retx_count+1]++;
        }
  ...
  if (found_pdu) 
  {
    ...
    /*Èç¹ûRLC Size´óÓÚMACÇëÇóSize£¬Ôò½øÐÐ·Ö¶Î´¦Àí*/
...
#endif
      rlc_am_nack_pdu (ctxt_pP, rlc_pP, found_pdu_sn, 0, 0x7FFF, 1);
