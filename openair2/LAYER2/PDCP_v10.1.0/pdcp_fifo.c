...
uint32_t ulIcmpSend=0;
uint32_t ulIcmpRecv=0;
uint8_t aulLast[100]={0};
uint8_t ulIcmpRecvLast = 0;
uint8_t ulReceivFlg = 0;

void IcmpSend(char *msg, uint8_t *pData)
{
    uint32_t ulNum = (pData[26]<<8)+(pData[27]);
        
    ulIcmpSend++;
    if( ulNum > 1 && ulReceivFlg == 0 && aulLast[(ulNum-1)%100]==1 )
    {
        ulIcmpRecvLast--;
        aulLast[(ulNum-1)%100]=0;
    }

    ulReceivFlg = 0;

    if( ulIcmpSend % 100 == 101)
    {
        LOG_W(PDCP, "%s ICMP [%d.%d.%d.%d ==> %d.%d.%d.%d] SEQ=%d Total %d/%d Recent %d/%d  \n", 
            msg,
            pData[12],pData[13],pData[14],pData[15],
            pData[16],pData[17],pData[18],pData[19],
            ulNum,
            ulIcmpRecv,ulIcmpSend,ulIcmpRecvLast,(ulNum>100)?100:ulNum
            );
    }
    print_info("AAAAA %s ICMP SEQ=%d Total %d/%d Recent %d/%d\n", 
        msg,ulNum,
        ulIcmpRecv,ulIcmpSend,ulIcmpRecvLast,(ulNum>100)?100:ulNum);

}
void IcmpRecv(char *msg, uint8_t *pData)
{
    uint32_t ulNum = (pData[26]<<8)+(pData[27]);

    ulIcmpRecv++;
    if( aulLast[ulNum%100]==0)
    {
        ulIcmpRecvLast++;
        aulLast[ulNum%100]=1;
    }
    ...
    if( ulIcmpSend % 100 == 0 )
    {
        LOG_N(PDCP, "%s [%d.%d.%d.%d ==> %d.%d.%d.%d] ICMP SEQ=%d Sum %d Succ %d Fail %d Recent %d/%d\n", 
             msg,
            pData[12],pData[13],pData[14],pData[15],
            pData[16],pData[17],pData[18],pData[19],
             ulNum,ulIcmpSend,
             ulIcmpRecv,ulIcmpSend-ulIcmpRecv,ulIcmpRecvLast,(ulNum>100)?100:ulNum);
    }
    print_info("AAAAA %s ICMP SEQ=%d Succ %d Fail %d Recent %d/%d\n", 
         msg,ulNum,
         ulIcmpRecv,ulIcmpSend-ulIcmpRecv,ulIcmpRecvLast,(ulNum>100)?100:ulNum);
...
//#endif
  ...
  while (sdu_p && cont) {
    ...
    if (ctxt_pP->enb_flag) {
      ...
      //xhd_oai_miss ENB pdcp send to GTPU ???
      if( bytes_wrote == 84)
      {
          LOG_W(PDCP,"pdcp_fifo_flush_sdus: PDCP SDU send to GTPU, ICMP SEQ=%d\n", 
              (sdu_p->data[sizeof (pdcp_data_ind_header_t)+26]<<8)
              +sdu_p->data[sizeof (pdcp_data_ind_header_t)+27]);
      }
      gtpv1u_new_data_req(
      ...
      //printf("pdcp_fifo_flush_sdus: data size=%d\n", ((pdcp_data_ind_header_t *)(sdu_p->data))->data_size);
    ...
    {
        printf("pdcp_fifo_flush_sdus, UE does not use GTPU\n");
    }
...
//#ifdef PDCP_USE_NETLINK
//#ifdef LINUX
      my_printf("pdcp_fifo_flush_sdus: PDCP_USE_NETLINK 1\n");
...
//#ifdef LINUX
          ...
          nas_nlh_tx->nlmsg_len += pdcp_output_sdu_bytes_to_write+sizeof(pdcp_data_ind_header_t);
          nas_iov_tx.iov_len = nas_nlh_tx->nlmsg_len+sizeof(pdcp_data_ind_header_t);
          ret = sendmsg(nas_sock_fd,&nas_msg_tx,0);

          if( pdcp_output_sdu_bytes_to_write == 84)
          {

              LOG_N(PDCP,"pdcp_fifo_flush_sdus: PDCP SDU send to NETLINK, ret=%d  ICMP SEQ=%d\n", 
                  ret, 
                  (sdu_p->data[sizeof (pdcp_data_ind_header_t)+26]<<8)
                  +sdu_p->data[sizeof (pdcp_data_ind_header_t)+27]);
          }
          if (ret<0) 
          {
            LOG_I(PDCP, "[PDCP_FIFOS] sendmsg returns %d (errno: %d)\n", ret, errno);
...
//-----------------------------------------------------------------------------
int pdcp_fifo_flush_sdus_ue(const protocol_ctxt_t* const  ctxt_pP)
{
  //-----------------------------------------------------------------------------
  ...
  while (sdu_p && cont) {
    ...
    if (!pdcp_output_sdu_bytes_to_write) 
    {
      ...
      my_printf("pdcp_fifo_flush_sdus_ue: PDCP_USE_NETLINK 1\n");
      ...
      if (bytes_wrote > 0) 
      {
        ...
        { 
          ...
          if( pdcp_output_sdu_bytes_to_write == 84)
          {
              //xhd_oai_debug ICMP Respose Received!
              IcmpRecv("pdcp_fifo_flush_sdus: PDCP SDU send to NETLINK ", 
                            &sdu_p->data[sizeof (pdcp_data_ind_header_t)]);
          }
...
# else /* PDCP_USE_NETLINK_QUEUES*/
  ...
  while (len > 0) {
    ...
    if (len<=0) {
      ...
      //printf("pdcp_fifo_read_input_sdus: len=%d\n", len);
...
#warning CODE TO BE REVIEWED, ONLY WORK FOR SIMPLE TOPOLOGY CASES
              for (ue_id = 0; ue_id < NB_UE_INST; ue_id++) {
                if (oai_emulation.info.eNB_ue_module_id_to_rnti[ctxt_cpy.module_id][ue_id] != NOT_A_RNTI) {
                  ...
                  LOG_I(PDCP, "[FRAME %5u][eNB][IP][INSTANCE %u][RB %u][--- PDCP_DATA_REQ / %d Bytes --->][PDCP][MOD %u][UE %u][RB DEFAULT_RAB_ID %u]\n",
                        ctxt.frame,
                        pdcp_read_header_g.inst,
                        pdcp_read_header_g.rb_id,
                        pdcp_read_header_g.data_size,
          ...
          { 
            LOG_D(PDCP, "Forcing send on DEFAULT_RAB_ID rab_id=%d\n",rab_id);
            if (rab_id != 0) 
            {
              rab_id = rab_id % maxDRB;
              key = PDCP_COLL_KEY_VALUE(ctxt.module_id, ctxt.rnti, ctxt.enb_flag, rab_id, SRB_FLAG_NO);
              ...
              if (h_rc == HASH_TABLE_OK) {
                ...
                //xhd_oai_miss UE PDCP receive from NETLINK
                if( pdcp_read_header_g.data_size == 84)
                {
                    //xhd_oai_debug ICMP Request Send!
                   
                    IcmpSend("pdcp_fifo_read_input_sdus: PDCP SDU receive from NETLINK", NLMSG_DATA(nas_nlh_rx));
            ...
            {
              LOG_D(PDCP, "Forcing send on DEFAULT_RAB_ID\n");
              LOG_D(PDCP, "[FRAME %5u][UE][IP][INSTANCE %u][RB %u][--- PDCP_DATA_REQ / %d Bytes --->][PDCP][MOD %u][UE %u][RB DEFAULT_RAB_ID %u]\n",
                    ctxt.frame,
                    pdcp_read_header_g.inst,
                    pdcp_read_header_g.rb_id,
                    pdcp_read_header_g.data_size,
            ...
            if( pdcp_read_header_g.data_size == 84)
            {
                LOG_W(PDCP,"pdcp_fifo_read_input_sdus: BBB PDCP SDU receive from NETLINK,  ICMP SEQ=%d\n", 
                    (((unsigned char *)NLMSG_DATA(nas_nlh_rx))[26]<<8)
                    +((unsigned char *)NLMSG_DATA(nas_nlh_rx))[27]);
            }
