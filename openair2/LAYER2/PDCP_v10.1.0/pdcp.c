...
//-----------------------------------------------------------------------------
{
  ...
  else 
  {
    ...
    if (pdcp_pdu_p != NULL) {
      ...
      if( srb_flagP && 0)
      {
          printf("pdcp_data_req:security_activated=%d cipheringAlgorithm=%d integrityProtAlgorithm=%d current_sn=%d\n",
              pdcp_p->security_activated,
...
//-----------------------------------------------------------------------------
{
  ...
  if ((TRUE == ctxt_pP->enb_flag) && (FALSE == srb_flagP)) {
    ...
    if( sdu_buffer_sizeP - payload_offset == 84)
    {
        //xhd_oai_debug
       
        IcmpSend("pdcp_data_ind: PDCP send message to GTPU", &sdu_buffer_pP->data[payload_offset]);
    }
...
//-----------------------------------------------------------------------------
{
  ...
  do {
    // Checks if a message has been sent to PDCP sub-task
    itti_poll_msg_ue (ctxt_pP->enb_flag ? TASK_PDCP_ENB : TASK_PDCP_UE, &msg_p);
...
# endif
  ...
  pdcp_fifo_flush_sdus_ue(ctxt_pP);
