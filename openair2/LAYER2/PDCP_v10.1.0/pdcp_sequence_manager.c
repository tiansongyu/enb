  ...
  { // DRB
    ...
    case RLC_MODE_AM:
           ...
           (pdcp_entity->last_submitted_pdcp_rx_sn - seq_num < reordering_window)  )) 
      {
		...
		if( (count++) % 100 == 0)
		{
	        LOG_W(PDCP, "Out of the reordering window (Incoming SN:%d, Expected SN:%d, LastSN:%d): discard this PDCP SDU\n",
	              seq_num, pdcp_entity->next_pdcp_rx_sn,pdcp_entity->last_submitted_pdcp_rx_sn);
		}
        return FALSE;
      } 
	  else if (pdcp_entity->next_pdcp_rx_sn - seq_num > reordering_window) 
	  {
	  	//½ÓÊÕ°üµÄÐòºÅ±ÈÆÚÍûÖµÐÂ
        pdcp_entity->rx_hfn++;
