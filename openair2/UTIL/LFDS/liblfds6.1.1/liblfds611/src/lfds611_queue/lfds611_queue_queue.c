...
int lfds611_queue_dequeue( struct lfds611_queue_state *qs, void **user_data )
{
  ...
  do {
    ...
    */
    ...
    if( dequeue[LFDS611_QUEUE_POINTER] == qs->dequeue[LFDS611_QUEUE_POINTER] 
        and dequeue[LFDS611_QUEUE_COUNTER] == qs->dequeue[LFDS611_QUEUE_COUNTER] ) 
    {
  ...
  //printf("lfds611_queue_dequeue:count=%d\n",count);
  if( cas_result == 1 )
...
int lfds611_queue_dequeue_ue( struct lfds611_queue_state *qs, void **user_data )
{
  LFDS611_ALIGN(LFDS611_ALIGN_DOUBLE_POINTER) struct lfds611_queue_element
      *enqueue[LFDS611_QUEUE_PAC_SIZE],
      *dequeue[LFDS611_QUEUE_PAC_SIZE],
