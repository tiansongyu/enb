...
//xhd_oai_enb
int gtpu_total_size = 0;
int gtpu_total_count = 0;
  ...
  NwGtpv1uUlpApiT   *pUlpApi)
{
  ...
  case NW_GTPV1U_ULP_API_RECV_TPDU: {
        ...
        )
    {
        ...
        printf("DATA:%d.%d.%d.%d ==> %d.%d.%d.%d TUN:%x\n",
...
#warning "LG eps bearer mapping to DRB id to do (offset -4)"
      ...
      if( buffer_len == 84)
      {
          ...
          IcmpRecv("pdcp_data_req from GTPU ", buffer);
      }
      ...
      if(result != TRUE)
      {
          LOG_E(GTPU, "PDCP data request failed!\n");
          return NW_GTPV1U_FAILURE;
      }
    } 
    else 
